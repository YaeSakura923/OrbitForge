...
本指南介绍了 OrbitForge 项目中可用的开发脚本命令。

## 构建与开发

- `pnpm build` - 构建所有工作区包并创建最终的捆绑包
- `pnpm build-examples` - 构建示例项目
- `pnpm build-bundle` - 构建 OrbitForge 主捆绑包
...
> **注意**: 在执行发布命令前，请确保已设置 `NPM_TOKEN` 环境变量，并且只有 `@orbitforge/OrbitForge` 和 `@orbitforge/orbitforge-react` 会被发布，其他包已设置为私有。
