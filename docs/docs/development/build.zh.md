...
本指南将介绍如何构建 OrbitForge 项目源码。

## 构建系统概览

OrbitForge 使用现代化的构建工具链：
...
对于捆绑包（如 @orbitforge/OrbitForge），构建输出位于 `dist/` 目录：
- .module.js: ES 模块格式
- .cjs.js: CommonJS 格式
- .d.ts: 类型定义文件
...
- `@orbitforge/orbitforge-examples/webpack.config.ts`: 示例项目配置
- `@orbitforge/OrbitForge/webpack.config.js`: 核心库配置
...
# 构建特定包及其依赖
pnpm --filter @orbitforge/orbitforge-map-controls... build
...
```bash
# 检查构建文件
ls -la @orbitforge/orbitforge-mapview/lib/
