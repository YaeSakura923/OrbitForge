...
import { EXAMPLE_CATEGORIES } from "@orbitforge/orbitforge-examples/src/example-categories";

// 动态导入所有示例配置
const examples: Example[] = [];

// 通过模块导入方式动态获取示例列表和配置
// 仅扫描@orbitforge/orbitforge-examples/src目录下的示例文件夹
const exampleContext = require.context("@orbitforge/orbitforge-examples/src", true, /config\.ts$/);
...
exampleContext.keys().forEach((configKey: string) => {
    try {
        // 确保只处理@orbitforge/orbitforge-examples/src目录下的config.ts文件
        if (configKey.startsWith("./") && configKey.includes("/config.ts")) {
            ...
            if (exampleId && exampleId !== "src" && !addedExampleIds.has(exampleId)) {
                ...
                let code = "";
                try {
                    const jsModule = require(`!!raw-loader!@orbitforge/orbitforge-examples/src/${exampleId}/index.js`);
