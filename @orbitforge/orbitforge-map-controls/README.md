...
`@orbitforge/orbitforge-map-controls` 是 orbitforge 库的地图控制器模块，实现了地图上下文中常用的一组默认相机功能。该模块提供相机交互控制（平移、缩放、旋转）、动画、坐标转换等功能。

## 安装

```bash
npm install @orbitforge/orbitforge-map-controls
```
...
import { MapView, GeoCoordinates, mercatorProjection } from "@orbitforge/orbitforge-mapview";
...
```typescript
import { MapControlsUI, MapControls } from "@orbitforge/orbitforge-map-controls";
...
import { 
    ...
    ControlPoint 
} from "@orbitforge/orbitforge-map-controls";
...
```typescript
import { EventNames } from "@orbitforge/orbitforge-map-controls";
...
```typescript
import { LongPressHandler } from "@orbitforge/orbitforge-map-controls";
