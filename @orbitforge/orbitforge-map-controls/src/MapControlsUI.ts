...
import { mercatorProjection, ProjectionType, sphereProjection } from "@orbitforge/orbitforge-geoutils";
import { MapViewEventNames } from "@orbitforge/orbitforge-mapview";
...
export class MapControlsUI {
    ...
    constructor(readonly controls: MapControls, options: MapControlsUIOptions = {
        ...
        container.className = "orbitforge-controls";
        this.domElement.appendChild(container);

        // Create compass button (circular)
        const compassButton = document.createElement("button");
        compassButton.id = "orbitforge-controls-button_compass";
        compassButton.title = "Reset North";
        compassButton.classList.add("orbitforge-controls-button", "compass");
        const compass = document.createElement("span");
        compass.id = "orbitforge-controls_compass";
        ...
        toggleButton.classList.add("orbitforge-controls-toggle");
        container.appendChild(toggleButton);

        // Create expanded controls container (initially hidden)
        const expandedContainer = document.createElement("div");
        expandedContainer.className = "orbitforge-controls-expanded";
        ...
        zoomInButton.classList.add("orbitforge-controls-button", "expanded");
        ...
        if (options.zoomLevel === "show") {
            ...
            this.m_zoomLevelElement.classList.add("orbitforge-controls_zoom-level");
            ...
            input.type = "number";
            input.step = "0.1";
            input.classList.add("orbitforge-controls_zoom-level");
            controls.mapView.addEventListener(
                MapViewEventNames.Render,
                this.m_onMapViewRenderEvent
            );
        ...
        zoomOutButton.classList.add("orbitforge-controls-button", "expanded");
        expandedContainer.appendChild(zoomOutButton);

        // Create tilt button
        const tiltButton = document.createElement("button");
        tiltButton.innerText = "3D";
        tiltButton.id = "orbitforge-controls_tilt-button-ui";
        tiltButton.title = "Toggle tilt";
        tiltButton.classList.add("orbitforge-controls-button", "expanded");
        ...
        if (options.screenshotButton !== false) {
            ...
            screenshotButton.id = "orbitforge-controls_screenshot-button";
            screenshotButton.title = this.getScreenshotTooltip();
            screenshotButton.classList.add("orbitforge-controls-button", "expanded");
        ...
        if (options.projectionSwitch) {
            ...
            switcher.id = "orbitforge-controls_switch_projection";
            switcher.classList.add("orbitforge-controls-button", "expanded");
        ...
        this.domElement.className = "orbitforge-controls";
        ...
        if (options.geoCenterDisplay !== false) {
            ...
            geoCenterContainer.className = "orbitforge-geo-center-display";
            this.m_geoCenterElement = document.createElement("div");
            this.m_geoCenterElement.className = "orbitforge-geo-center-text";
    ...
    private normalizeScreenshotConfig(config: boolean | ScreenshotConfig | undefined): ScreenshotConfig {
        ...
        if (typeof config === 'boolean') {
            ...
            return {
                ...
                height: 512,
                quality: 0.9,
                filename: 'orbitforge-screenshot-{timestamp}_{width}x{height}.png'
            };
        }
        ...
        return {
            ...
            filename: config?.filename || 'orbitforge-screenshot-{timestamp}_{width}x{height}.png'
        };
    }
    ...
    dispose() {
        ...
        if (this.m_geoCenterElement !== null) {
            ...
            const geoCenterDisplay = document.querySelector('.orbitforge-geo-center-display');
    ...
    private initStyle() {
        if (document.getElementById("here-orbitforge-controls.map-controls-ui-styles") !== null) {
            return;
        }
        const style = document.createElement("style");
        style.id = "here-orbitforge-controls.map-controls-ui-styles";
...
function getTextStyle() {
    return `
        ...
        .orbitforge-geo-center-display:hover {
            background: rgba(55, 65, 85, 0.95);
            transform: scale(1.02);
        }
        
        /* Circular compass button */
        .orbitforge-controls-button.compass {
            display: flex;
            align-items: center;
        ...
        .orbitforge-controls-button.compass:hover {
            background-color: rgba(55, 65, 85, 0.95);
            transform: scale(1.05);
        }
        
        .orbitforge-controls-button.compass:active {
            background-color: rgba(55, 175, 170, 0.95);
            transform: scale(0.95);
        }
        
        #orbitforge-controls_compass {
            pointer-events: none;
            position: absolute;
            top: 0;
            left: 0;
        ...
        #orbitforge-controls_compass::after {
            content: " ";
            position: absolute;
            left: 50%;
            margin-left: -3px;
        ...
        #orbitforge-controls_compass::before {
            content: " ";
            position: absolute;
            left: 50%;
            margin-left: -3px;
        ...
        .orbitforge-controls-toggle:hover {
            background-color: rgba(55, 65, 85, 0.95);
            transform: scale(1.05);
        }
        
        .orbitforge-controls-toggle:active {
            background-color: rgba(55, 175, 170, 0.95);
            transform: scale(0.95);
        }
        
        /* Expanded controls container */
        .orbitforge-controls-expanded {
            display: none;
        ...
        .orbitforge-controls-button.expanded {
            display: flex;
            align-items: center;
            justify-content: center;
            background-color: rgba(39, 45, 55, 0.85);
        ...
        .orbitforge-controls-expanded:not(.collapsed) .orbitforge-controls-button.expanded {
            opacity: 1;
            transform: translateX(0);
        }
        
        .orbitforge-controls-button.expanded:last-child {
            margin-left: 0;
        }
        
        .orbitforge-controls-button.expanded:hover {
            background-color: rgba(55, 65, 85, 0.95);
            transform: scale(1.05) translateX(0);
        }
        
        .orbitforge-controls-button.expanded:active {
            background-color: rgba(55, 175, 170, 0.95);
            transform: scale(0.95) translateX(0);
        }
        
        #orbitforge-controls_tilt-button-ui.expanded {
            font-size: 16px;
        }

        /* Screenshot button specific styles */
        #orbitforge-controls_screenshot-button.expanded {
            font-size: 18px;
        }
        
        .orbitforge-controls_screenshot_svg {
            width: 20px;
            height: 20px;
            stroke: #d4d5d7;
            fill: none;
        ...
        .orbitforge-controls-button:hover .orbitforge-controls_screenshot_svg {
            stroke: #fff;
        }
        
        /* Loading spinner for screenshot */
        .orbitforge-controls_loading_svg {
            width: 20px;
            height: 20px;
        ...
        /* Zoom level display */
        .orbitforge-controls_zoom-level {
            display: block;
        ...
        .orbitforge-controls-expanded:not(.collapsed) .orbitforge-controls_zoom-level {
            opacity: 1;
            transform: translateX(0);
        }
        
        .orbitforge-controls_zoom-level:last-child {
            margin-left: 0;
        }
        
        .orbitforge-controls_zoom-level:focus {
            box-shadow: 0 0 0 2px rgba(55, 175, 170, 0.6);
        }
        
        input.orbitforge-controls_zoom-level::-webkit-outer-spin-button,
        input.orbitforge-controls_zoom-level::-webkit-inner-spin-button {
            -webkit-appearance: none;
            margin: 0;
        }
        
        input.orbitforge-controls_zoom-level[type=number] {
            -moz-appearance: textfield;
        }
        
        /* Projection switch button */
        #orbitforge-controls_switch_projection {
            margin-left: 6px;
        }
        
        .orbitforge-controls-switch_svg {
            width: 25px;
            height: 25px;
            stroke: #d4d5d7;
            fill: #d4d5d7;
        ...
        .orbitforge-controls-button:hover .orbitforge-controls-switch_svg {
            stroke: #fff;
            fill: #fff;
        }
    `;
...
function getFlatMapSVG() {
    return `
    <svg class="orbitforge-controls_switch_svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function getScreenshotSVG() {
    return `
    <svg class="orbitforge-controls_screenshot_svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function getLoadingSVG() {
    return `
    <svg class="orbitforge-controls_loading_svg" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
