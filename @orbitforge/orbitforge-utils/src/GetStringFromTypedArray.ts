...
type DecodeFunction = (view: Uint8Array) => string;

interface GetStringFromTypedArrayFunction {
    (uint8Array: Uint8Array, byteOffset?: number, byteLength?: number): string;
    decode: DecodeFunction;
    decodeWithTextDecoder: (view: Uint8Array) => string;
    decodeWithFromCharCode: (view: Uint8Array) => string;
}
 ...
 * @private
 */
const getStringFromTypedArray: GetStringFromTypedArrayFunction = function (
...
getStringFromTypedArray.decodeWithTextDecoder = function (view: Uint8Array): string {
    const decoder = new TextDecoder("utf-8");
    return decoder.decode(view);
};

getStringFromTypedArray.decodeWithFromCharCode = function (view: Uint8Array): string {
    let result = "";
...
function utf8Handler(utfBytes: Uint8Array): number[] {
    let codePoint = 0;
    let bytesSeen = 0;
    let bytesNeeded = 0;
