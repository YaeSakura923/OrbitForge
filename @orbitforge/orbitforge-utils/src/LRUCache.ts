...
type ComputeMemoryUsageCallback<T> = (item: T) => number;

class LRUCache<T> {
    // options
    public minSize: number;
    public maxSize: number;
    public minBytesSize: number;
    public maxBytesSize: number;
    public unloadPercent: number;
    public autoMarkUnused: boolean;

    // data structures
    private readonly itemSet: Map<T, number>; // Maps items to their last used timestamp
    private readonly itemList: T[];
    private readonly usedSet: Set<T>;
    private readonly callbacks: Map<T, RemoveCallback<T>>;
    private unloadingHandle: number;
    protected cachedBytes: number;
    protected bytesMap: Map<T, number>;
    private readonly loadedSet: Set<T>;
    private scheduled: boolean;

    private _unloadPriorityCallback: UnloadPriorityCallback<T> | null;
    public computeMemoryUsageCallback: ComputeMemoryUsageCallback<T>;
    ...
    constructor() {
        ...
        this.computeMemoryUsageCallback = () => 0;
    }

    get unloadPriorityCallback(): UnloadPriorityCallback<T> | null {
        return this._unloadPriorityCallback;
    }

    set unloadPriorityCallback(cb: (a: T, b?: T) => number) {
        if (cb.length === 1) {
            console.warn(
                'LRUCache: "unloadPriorityCallback" function has been changed to take two arguments.'
            );
            this._unloadPriorityCallback = (a: T, b: T) => {
                const valA = cb(a);
                const valB = cb(b);
            ...
            this._unloadPriorityCallback = cb as UnloadPriorityCallback<T>;
        }
    }

    private get defaultPriorityCallback(): (item: T) => number {
        return (item: T) => this.itemSet.get(item) || 0;
    }
    ...
    getMemoryUsage(item: T): number | null {
        return this.bytesMap.get(item) ?? null;
    }

    add(item: T, removeCb: RemoveCallback<T>): boolean {
    ...
    // are marked as used.
    setLoaded(item: T, value: boolean): void {
    ...
    unloadUnusedContent(): void {
        ...
        const hasNodesToUnload =
            (excessNodes > 0 && unused > 0) || (unloaded && itemList.length > maxSize);
        const hasBytesToUnload =
            (unused && this.cachedBytes > minBytesSize) ||
            (unloaded && this.cachedBytes > maxBytesSize);
        ...
        if (hasBytesToUnload || hasNodesToUnload) {
            ...
            const maxBytesUnload = Math.max(
                unloadPercent * excessBytes,
                unloadPercent * minBytesSize
            );
            ...
            needsRerun =
                removedNodes < excessNodes || (removedBytes < excessBytes && removedNodes < unused);
            needsRerun = needsRerun && removedNodes > 0;
        }

        if (needsRerun) {
            this.unloadingHandle = requestAnimationFrame(() => {
                this.scheduleUnload();
            });
        }
    }
