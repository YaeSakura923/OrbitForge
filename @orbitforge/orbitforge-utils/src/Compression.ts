...
export function octEncodeInRange(vector: Vector3, rangeMax: number, result: Vector2): Vector2 {
    const vector3 = scratchVector3.copy(vector);

    assert(Math.abs(vector3.lengthSq() - 1.0) <= 1e-6); // Replaced _MathUtils.EPSILON6 with 1e-6
...
export function octPackFloat(encoded: Vector2): number {
    const vector2 = scratchVector2.copy(encoded);
