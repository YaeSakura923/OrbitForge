...
function extractCameraParams(matrix: THREE.Matrix4): { position: THREE.Vector3, lookAt: THREE.Vector3, up: THREE.Vector3 } {
...
function createUI() {
    ...
    button.onclick = () => {
        const tileKeyVal = (document.getElementById('tilekey-input') as HTMLInputElement).value;
        const minAltVal = (document.getElementById('min-altitude-input') as HTMLInputElement).value;
        const maxAltVal = (document.getElementById('max-altitude-input') as HTMLInputElement).value;
...
function createGeoBoxWithAltitudes(tileKey: TileKey, minAltitude: number, maxAltitude: number): GeoBox {
...
function runTestWithTileKeyAndAltitudes(tileKeyStr: string, minAltitude: number, maxAltitude: number) {
    try {
        // Parse input TileKey
        let tileKey: TileKey;
        ...
        const resultsDiv = document.getElementById('test-results')!;
        resultsDiv.innerHTML = `<p style="color: red;">Error: ${(error as Error).message}</p>`;
        console.error('Test execution error:', error);
    }
}

// Old function, kept for compatibility
function runTestWithTileKey(tileKeyStr: string) {
