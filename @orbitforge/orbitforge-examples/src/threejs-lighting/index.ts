 ...
 * @param mapView Map view instance
 */
const enableShadowMapping = (mapView: MapView): void => {
...
class LightsManager extends THREE.Object3D {
    ...
    // Spotlight related properties
    public spotLightTarget: THREE.Object3D;
    public spotLightTexture: THREE.Texture;
    ...
    // Toggle light switch
    public toggleLight(lightName: string, visible: boolean) {
    ...
    // Set light intensity
    public setLightIntensity(lightName: string, intensity: number) {
        const light = this.lights[lightName];
        if (light) {
    ...
    // Set light color
    public setLightColor(lightName: string, color: string) {
        const light = this.lights[lightName];
        if (light) {
    ...
    public moveLight(lightName: string, x: number, y: number, z: number) {
        const light = this.lights[lightName];
        if (light) {
 ...
 * @param mapView Map view instance
 */
const addLightManagerToMap = (mapView: MapView): void => {
