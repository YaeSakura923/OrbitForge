...
const PROJECT_CONFIG = {
    ECOLOGICAL_FARMING_3DTILES_URL: "http://192.168.1.18/orbitforge-examples/data/%E7%94%9F%E6%80%81%E5%85%BB%E6%AE%96%E5%9B%AD%E5%8C%BA/3dtile/tileset.json",
    ECOLOGICAL_FARMING_TERRAIN_URL: "http://192.168.1.18/orbitforge-examples/data/%E7%94%9F%E6%80%81%E5%85%BB%E6%AE%96%E5%9B%AD%E5%8C%BA/terrain/{z}/{x}/{y}.png",
 ...
 * @param mapView Map view instance
 */
const addEcologicalFarmingDataSource = async (mapView: MapView): Promise<void> => {
 ...
 * @param mapView Map view instance
 */
const addWaterFeatures = async (mapView: MapView): Promise<void> => {
    try {
        // Download water GeoJSON data
        const features: Feature[] = await TransferManager.instance().downloadJson(PROJECT_CONFIG.WATER_GEOJSON_PATH);
        ...
        features.forEach((feature) => {
            ...
            // Create polygon mesh
            const mesh = new PolygonMesh(feature, sphereProjection, waterMaterial) as MapAnchor<PolygonMesh>;

            // Set altitude
            (mesh.anchor as GeoCoordinates).altitude = (feature.properties.altitude || 30);
