...
import { type Definitions, type Theme } from "@orbitforge/orbitforge-datasource-protocol";
import {
    type JsonExpr,
    Expr,
    isJsonExpr,
...
import { loadTestResource } from "@orbitforge/orbitforge-test-utils";
import Ajv from "ajv";
...
describe("Berlin Theme", function () {
    ...
    before(async () => {
        ...
        const theme = await loadTestResource(
            "@orbitforge/orbitforge-datasource-protocol",
            "./theme.schema.json",
            "json"
        );
    ...
    themes.forEach(themePath => {
        ...
        describe(`${baseName}`, function () {
            let theme: Theme;
            before(async () => {
                theme = await loadTestResource("@orbitforge/orbitforge-map-theme", themePath, "json");
            });
