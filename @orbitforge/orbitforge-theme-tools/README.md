...
```
npx orbitforge-theme-optimizer --help
```
...
To minimize the http requests for a production application the `orbitforge-theme-optimizer` will combine all included files into one.
Additionaly whitespaces can be removed withe the `--minify` option.

```
npx orbitforge-theme-optimizer --in 'my-theme-file.json' --minify
```
