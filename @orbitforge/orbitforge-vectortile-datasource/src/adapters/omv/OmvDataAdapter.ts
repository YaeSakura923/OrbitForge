...
import { type ILogger, assert, LoggerManager } from "@orbitforge/orbitforge-utils";
...
import {
    type OmvFeatureFilter,
    type OmvFeatureModifier,
    ComposedDataFilter,
...
function roundUpPolygonCoordinates(geometry: IPolygonGeometry[], layerExtents: number) {
    geometry.forEach(polygon => {
        polygon.rings.forEach(r => {
            roundUpCoordinates(r, layerExtents);
        });
    });
}
...
function roundUpLineCoordinates(geometry: ILineGeometry[], layerExtents: number) {
    geometry.forEach(line => {
        roundUpCoordinates(line.positions, layerExtents);
    });
}
