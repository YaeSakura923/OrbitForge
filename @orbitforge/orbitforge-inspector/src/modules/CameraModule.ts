...
export class CameraModule {
    ...
    updateData(data: CameraData): void {
        ...
        // Position
        data.position = `${camera.position.x.toFixed(2)},${camera.position.y.toFixed(
            2
        )},${camera.position.z.toFixed(2)}`;

        // Target
        data.target = `${target.latitude.toFixed(6)},${target.longitude.toFixed(6)},${
            target.altitude?.toFixed(2) || 0
        }`;
