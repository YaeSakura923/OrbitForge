...
export class VisibleTileSetModule {
    ...
    private updateTileKeyDetails(data: VisibleTileSetData, dataSourceTileList: any[]): void {
        try {
            const details: string[] = [];

            dataSourceTileList.forEach((list: any, index: number) => {
                const dataSourceName =
                    list.dataSource && list.dataSource.name
                ...
                if (list.visibleTiles && list.visibleTiles.length > 0) {
                    ...
                    sampleTiles.forEach((tile: any) => {
                        if (tile && tile.tileKey) {
                            const level =
                                tile.tileKey.level !== undefined ? tile.tileKey.level : "?";
                            const row = tile.tileKey.row !== undefined ? tile.tileKey.row : "?";
                            const column =
                                tile.tileKey.column !== undefined ? tile.tileKey.column : "?";
    ...
    private updateTileKeyMortonCodes(data: VisibleTileSetData, dataSourceTileList: any[]): void {
        try {
            const mortonCodes: string[] = [];
    ...
    private updateTileKeyDistribution(data: VisibleTileSetData, dataSourceTileList: any[]): void {
        try {
    ...
    private updatePerformanceImpact(data: VisibleTileSetData, dataSourceTileList: any[]): void {
        try {
            const impact: string[] = [];
            let totalTiles = 0;
            let totalLoading = 0;
            ...
            impact.push(
                `Loading Ratio: ${
                    totalTiles > 0 ? ((totalLoading / totalTiles) * 100).toFixed(2) : 0
                }%`
            );
            ...
            if (data.cacheSize > 0 && data.cacheCapacity > 0) {
                impact.push(
                    `Cache Usage: ${data.cacheUsage}% (${data.cacheSize}/${data.cacheCapacity})`
                );
            }
    ...
    bindControls(folder: GUI, data: VisibleTileSetData): void {
        ...
        const distributionElement =
            distributionController.domElement.querySelector("input") ||
        ...
        const impactElement =
            impactController.domElement.querySelector("input") ||
