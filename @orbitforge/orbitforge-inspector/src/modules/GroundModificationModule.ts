...
import { type DataSource } from "@orbitforge/orbitforge-mapview";
import { type GroundModificationOptions } from "@orbitforge/orbitforge-terrain-datasource";
...
export class GroundModificationModule {
    ...
    updateData(data: GroundModificationData): void {
        ...
        const terrainSource = this.getTerrainSource();
        if (terrainSource) {
            const options: GroundModificationOptions = {
                model: data.model,
                sigma2: data.sigma2,
                alpha: data.alpha,
     ...
     */
    private getTerrainSource(): any | undefined {
        // 遍历所有数据源查找地形源
     ...
     */
    private isTerrainSource(dataSource: DataSource): boolean {
        return (
            dataSource &&
