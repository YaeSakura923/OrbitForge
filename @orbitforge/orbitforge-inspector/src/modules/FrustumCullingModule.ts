...
import { type MapView, type ElevationProvider, type ElevationRangeSource } from "@orbitforge/orbitforge-mapview";
import { TileKey, geographicTerrainStandardTiling, normalizedEquirectangularProjection, GeoBox, OrientedBox3, GeoCoordinates } from "@orbitforge/orbitforge-geoutils";
import { GUI } from "dat.gui";

/**
 * Frustum culling debugging module for OrbitForge inspector
 * Provides tools to test frustum culling with elevation data
 */
export class FrustumCullingModule {
    private readonly gui: GUI;
    private readonly mapView: MapView;
    private readonly frustumFolder: GUI;
    private readonly elevationProvider: ElevationProvider;
    private readonly elevationRangeSource: ElevationRangeSource;
        ...
        const queryControls = {
            ...
            testFrustumCulling: () => {
                try {
                    ...
                    const levelCheckResults: { level: number; tileKey: TileKey; intersects: boolean; geoBox: GeoBox; orientedBox: OrientedBox3 }[] = [];
                    ...
                    while (currentTileKey.level >= 0) {
                        ...
                        const geoBoxWithElevation = new GeoBox(
                            new GeoCoordinates(geoBox.southWest.latitude, geoBox.southWest.longitude, minElevation),
                            new GeoCoordinates(geoBox.northEast.latitude, geoBox.northEast.longitude, maxElevation)
                        );
                    ...
                    alert(`Error testing frustum culling: ${(error as Error).message}`);
                }
            }
        };
