...
import { PerformanceStatistics } from "@orbitforge/orbitforge-mapview/Statistics";
// Properly import dat.GUI types
...
export class EnhancedTileModule {
    ...
    private updateTileKeyInfo(data: EnhancedTileData, visibleTileSet: any): void {
        try {
            ...
            if (dataSourceTileList.length > 0) {
                const tileCounts = dataSourceTileList.map((list: any) =>
                    list.visibleTiles ? list.visibleTiles.length : 0
                );

                if (tileCounts.length > 0) {
                    data.averageTilesPerDataSource =
                        tileCounts.reduce((a: number, b: number) => a + b, 0) / tileCounts.length;
                    data.maxTilesPerDataSource = Math.max(...tileCounts);
                    data.minTilesPerDataSource = Math.min(...tileCounts);
                }

                // Collect tile key information
                const tileKeyInfo: string[] = [];
                dataSourceTileList.forEach((list: any, index: number) => {
                    ...
                    tileKeyInfo.push(
                        `${dataSourceName}: ${visibleCount} visible, ${renderedCount} rendered, ${loadingCount} loading`
                    );
                    ...
                    if (list.visibleTiles && list.visibleTiles.length > 0) {
                        ...
                        sampleTiles.forEach((tile: any) => {
                            if (tile && tile.tileKey) {
                                const mortonCode = tile.tileKey.mortonCode
                                    ? tile.tileKey.mortonCode(
                                          tile.dataSource.getTilingScheme().mortonTileEncoding
                                      )
                                    : "N/A";
                                const level =
                                    tile.tileKey.level !== undefined ? tile.tileKey.level : "N/A";
                                const row =
                                    tile.tileKey.row !== undefined ? tile.tileKey.row : "N/A";
                                const column =
                                    tile.tileKey.column !== undefined ? tile.tileKey.column : "N/A";

                                tileKeyInfo.push(
                                    `  Tile(${level},${row},${column}) morton:${mortonCode}`
                                );
                            }
                        });
                        ...
                        if (list.visibleTiles.length > 3) {
                            tileKeyInfo.push(
                                `  ... and ${list.visibleTiles.length - 3} more tiles`
                            );
                        }
                    }
                });
    ...
    private updateTileKeyDetails(data: EnhancedTileData, visibleTileSet: any): void {
        try {
            const dataSourceTileList = visibleTileSet.dataSourceTileList || [];
            ...
            dataSourceTileList.forEach((list: any, index: number) => {
                ...
                if (list.visibleTiles && list.visibleTiles.length > 0) {
                    ...
                    list.visibleTiles.forEach((tile: any) => {
                        if (tile && tile.tileKey) {
                    ...
                    Object.keys(levelGroups)
                        ...
                        .forEach(level => {
                            ...
                            tiles.slice(0, 3).forEach((tile: any) => {
                                if (tile.tileKey) {
    ...
    private updateTileKeyMortonCodes(data: EnhancedTileData, visibleTileSet: any): void {
        try {
            const dataSourceTileList = visibleTileSet.dataSourceTileList || [];
            const mortonInfo: string[] = [];
    ...
    private updateTileKeyLevels(data: EnhancedTileData, visibleTileSet: any): void {
        try {
            const dataSourceTileList = visibleTileSet.dataSourceTileList || [];
    ...
    private updatePerformanceMetrics(
        ...
        try {
            ...
            dataSourceTileList.forEach((list: any) => {
            ...
            metrics.push(
                `Tiles - Visible: ${totalVisible}, Rendered: ${totalRendered}, Loading: ${totalLoading}`
            );
            ...
            if (data.maxCacheSize > 0) {
                ...
                metrics.push(
                    `Cache Usage: ${cacheUsage.toFixed(2)}% (${data.cacheSize}/${
                        data.maxCacheSize
                    })`
                );
            ...
            if (stats && stats.frames) {
                ...
                if (frameTime > 16.67) {
                    metrics.push(
                        `⚠️ Frame time exceeds 60fps target (${(frameTime - 16.67).toFixed(
                            2
                        )}ms over)`
                    );
                }
    ...
    bindControls(folder: GUI, data: EnhancedTileData): void {
        ...
        const tileKeysElement =
            tileKeysController.domElement.querySelector("input") ||
        ...
        const detailsElement =
            detailsController.domElement.querySelector("input") ||
        ...
        const mortonElement =
            mortonController.domElement.querySelector("input") ||
        ...
        const levelsElement =
            levelsController.domElement.querySelector("input") ||
        ...
        const metricsElement =
            metricsController.domElement.querySelector("input") ||
