...
import { TileKey, GeoCoordinates } from "@orbitforge/orbitforge-geoutils";
import { GUI } from "dat.gui";

/**
 * Elevation debugging module for OrbitForge inspector
 * Provides tools to debug elevation provider and elevation range source
 */
export class ElevationModule {
    private readonly gui: GUI;
    private readonly mapView: MapView;
    private readonly elevationFolder: GUI;
    private readonly rangeFolder: GUI;
            ...
            queryElevation: () => {
                if (elevationProvider) {
                    try {
                        ...
                        alert(`Error querying elevation: ${(error as Error).message}`);
                    }
                }
            }
        };
            ...
            queryRange: async () => {
                try {
                    ...
                    const tileKey = TileKey.fromRowColumnLevel(
                        rangeQueryControls.row,
                        rangeQueryControls.column,
                        rangeQueryControls.level
                    );
                    ...
                    alert(`Error querying elevation range: ${(error as Error).message}`);
                }
            }
        };
