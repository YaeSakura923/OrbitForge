...
地面修改控制面板是 orbitforge-inspector 的一个新模块，用于控制 GroundModificationManager 中的 kriging 插值选项。该面板允许用户实时调整克里金插值参数，以改变地形修改的视觉效果。
...
要在项目中使用地面修改控制面板，首先需要确保已安装 `@orbitforge/orbitforge-inspector` 包：

```bash
pnpm add @orbitforge/orbitforge-inspector
```
