...
function transformTexCoords(materialIndex: number, gltfData: GLTFWithBuffers): void {
    ...
    const processedTexCoords: Array<[number, number]> = [];
...
function transformPrimitives(
    ...
    processedTexCoords: Array<[number, number]>
) {
    ...
    if (Number.isFinite(texCoordAccessor)) {
        ...
        if (accessor && accessor.bufferView) {
            ...
            if (bufferView) {
                ...
                const { arrayBuffer, byteOffset: bufferByteOffset } =
...
function updateGltf(
    ...
    buffers: Array<{ arrayBuffer: ArrayBuffer; byteOffset: number; byteLength: number }>,
