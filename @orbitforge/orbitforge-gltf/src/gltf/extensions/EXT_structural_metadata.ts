...
import type { GLTF, GLTFMeshPrimitive, GLTFTextureInfoMetadata } from "../types/gltf-json-schema";
import {
    ...
    if (propertyTextures && json.meshes) {
        ...
        for (const mesh of json.meshes) {
            for (const primitive of mesh.primitives) {
                processPrimitivePropertyTextures(
                    scenegraph,
                    propertyTextures,
                    primitive,
                    extension
...
function processPropertyTable(
    ...
    for (const propertyName in schemaClass.properties) {
        ...
        const propertyTableProperty:
            | GLTF_EXT_structural_metadata_PropertyTable_Property
            | undefined = propertyTable.properties?.[propertyName];
...
function parseVariableLengthArrayENUM(params: {
    ...
    for (let index = 0; index < numberOfElements; index++) {
        ...
        const array: string[] = getEnumsArray(
            valuesData,
            typedArrayOffset,
            elementCount,
            enumEntry
