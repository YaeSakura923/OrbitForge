...
import type { BigTypedArray, ImageType, TypedArray } from "@orbitforge/orbitforge-utils";
import { getImageData } from "@orbitforge/orbitforge-utils";

import { type GLTFScenegraph } from "../../api/gltf-scenegraph";
import { getComponentTypeFromArray } from "../../gltf-utils/gltf-utils";
import type { GLTFMeshPrimitive, GLTFTextureInfoMetadata } from "../../types/gltf-json-schema";
...
export function convertRawBufferToMetadataArray(
    ...
    return new (ArrayType as any)(buffer, offset, length);
}
...
export function getPrimitiveTextureData(
  ...
  */
    ...
    const textureCoordinates: TypedArray =
...
function getImageValueByCoordinates(
    ...
    let components = 1;
    if (mimeType && (mimeType.includes("image/jpeg") || mimeType.includes("image/png"))) {
        components = 4;
    }
...
export function parseVariableLengthArrayNumeric(
    ...
    for (let index = 0; index < numberOfElements; index++) {
        ...
        attributeValueArray.push(
            valuesData.slice(typedArrayOffset, typedArrayOffset + elementCount)
        );
    }
    ...
    if (stringOffsets) {
        ...
        for (let index = 0; index < numberOfElements; index++) {
            ...
            if (stringByteSize + stringOffset <= valuesDataBytes.length) {
                const stringData = valuesDataBytes.subarray(
                    stringOffset,
                    stringByteSize + stringOffset
                );
