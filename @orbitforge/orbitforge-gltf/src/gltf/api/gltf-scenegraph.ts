...
import { copyToArray, getBinaryImageMetadata, padToNBytes } from "@orbitforge/orbitforge-utils";
...
export class GLTFScenegraph {
    ...
    hasExtension(extensionName: string): boolean {
        ...
        const isRequiredExtension = this.getRequiredExtensions().find(
    ...
    getObjectExtension<T = Extension>(
    ...
    getSkin(index: number): GLTFSkin {
        return this.getObject("skins", index) as unknown as GLTFSkin;
    }

    getMesh(index: number): GLTFMesh {
        return this.getObject("meshes", index) as unknown as GLTFMesh;
    }
    ...
    getAccessor(index: number): GLTFAccessor {
        return this.getObject("accessors", index) as unknown as GLTFAccessor;
    }
    ...
    getBufferView(index: number | object): GLTFBufferView {
        return this.getObject("bufferViews", index) as unknown as GLTFBufferView;
    }

    getBuffer(index: number): GLTFBuffer {
        return this.getObject("buffers", index) as unknown as GLTFBuffer;
    }
    ...
    getObject(array: string, index: number | object): Record<string, unknown> {
        ...
        const object = this.json[array] && (this.json[array] as Array<{}>)[index];
        if (!object) {
     ...
     */
    addApplicationData(key: string, data: object): this {
        this.json[key] = data;
        return this;
    }
     ...
     */
    addExtraData(key: string, data: object): this {
        this.json.extras = this.json.extras || {};
        (this.json.extras as Record<string, unknown>)[key] = data;
        return this;
    }

    addObjectExtension(object: object, extensionName: string, data: object): this {
        // @ts-ignore
    ...
    createBinaryChunk(): void {
        // Calculate actual total size needed
        ...
        if (this.json?.buffers?.[0]) {
            this.json.buffers[0].byteLength = actualByteLength;
        } else {
            this.json.buffers = [{ byteLength: actualByteLength }];
        }
