...
class GLTFPostProcessor {
    ...
    _resolveTree(gltf: GLTF, options = {}): GLTFPostprocessed {
        ...
        if (gltf.bufferViews) {
            json.bufferViews = gltf.bufferViews.map((bufView, i) =>
                this._resolveBufferView(bufView, i)
            );
        }
        if (gltf.images) {
        ...
        if (gltf.accessors) {
            json.accessors = gltf.accessors.map((accessor, i) =>
                this._resolveAccessor(accessor, i)
            );
        }
        if (gltf.materials) {
            json.materials = gltf.materials.map((material, i) =>
                this._resolveMaterial(material, i)
            );
        }
        if (gltf.meshes) {
    ...
    _resolveMaterial(gltfMaterial: GLTFMaterial, index: number): GLTFMaterialPostprocessed {
        ...
        if (material.pbrMetallicRoughness) {
            ...
            if (mr.metallicRoughnessTexture) {
                ...
                mr.metallicRoughnessTexture.texture = this.getTexture(
    ...
    _getValueFromInterleavedBuffer(
        ...
        for (let i = 0; i < count; i++) {
            ...
            result.set(
                new Uint8Array(
                    buffer.arrayBuffer.slice(elementOffset, elementOffset + bytesPerElement)
                ),
                i * bytesPerElement
            );
        }
        ...
        return {
            ...
            sampler:
                typeof gltfTexture.sampler === "number"
                    ...
                    : makeDefaultSampler(),
            source:
                typeof gltfTexture.source === "number"
                    ? this.getImage(gltfTexture.source)
                    : undefined
        };
    }
    ...
    _resolveImage(gltfImage: GLTFImage, index: number): GLTFImagePostprocessed {
        const image: GLTFImagePostprocessed = {
            ...
            image: null!,
            bufferView:
                gltfImage.bufferView !== undefined
                    ? this.getBufferView(gltfImage.bufferView)
                    : undefined
        };
