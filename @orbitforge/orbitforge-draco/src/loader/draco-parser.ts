...
export default class DracoParser {
    ...
    parseSync(arrayBuffer: ArrayBuffer, options: DracoParseOptions = {}): DracoMesh {
        ...
        try {
            ...
            // Pass additional attribute mapping options
            const schema = getDracoSchema(geometry, loaderData);
    ...
    _getMeshData(
        ...
        for (const [name, attribute] of Object.entries(attributes)) {
            geometry.setAttribute(name.toLowerCase(), attribute);
        }
            ...
            switch (options.topology) {
                case "triangle-strip":
                    indices = new THREE.BufferAttribute(
                        this._getTriangleStripIndices(dracoGeometry),
                        1
                    );
                ...
                default:
                    indices = new THREE.BufferAttribute(
                        this._getTriangleListIndices(dracoGeometry),
                        1
                    );
                    geometry.setIndex(indices);
                    break;
    ...
    private _getMeshAttributes(
        ...
        dracoGeometry: Mesh | PointCloud,
        options: DracoParseOptions
    ): { [attributeName: string]: THREE.BufferAttribute } {
        ...
        for (const loaderAttribute of Object.values(loaderData.attributes)) {
            ...
            if (values) {
                // Convert attribute names to Three.js specification
                const threeName =
                    DRACO_TO_GLTF_ATTRIBUTE_NAME_MAP[attributeName]?.toLowerCase() ||
                    attributeName.toLowerCase();
                attributes[threeName] = new THREE.BufferAttribute(values.value, values.size);
            }
        }
    ...
    _getTriangleStripIndices(dracoGeometry: Mesh) {
        ...
        try {
            /* const numStrips = */ this.decoder.GetTriangleStripsFromMesh(
                dracoGeometry,
                dracoArray
            );
    ...
    _getAttributeValues(
    ...
    ): { value: THREE.TypedArray; size: number } | null {
        ...
        let value: THREE.TypedArray;

        const ptr = this.draco._malloc(byteLength);
        try {
            const dracoAttribute = this.decoder.GetAttribute(
                dracoGeometry,
