...
```
npm install orbitforge-atlas-tools
```

in directory of your choice you will have two applications available:

-   orbitforge-atlas-generator
-   orbitforge-sprites-generator
...
```
npx orbitforge-atlas-generator --help
npx orbitforge-sprites-generator --help
```
...
> **Important note**
>
> If you are not sure to install entire package via **npm**, but have cloned repository you may still run in from root package directory `@orbitforge/orbitforge-atlas-tools`. Just install
> dependencies localy and use **yarn run** command instead of **npx**:
>
> ```
> yarn
> yarn run orbitforge-atlas-generator --help
> ```
>
...
Although the **orbitforge-atlas-generator** tool is flexible enough for most use cases, it may be neccessary to
...
For this purpose you may use **orbitforge-sprites-generator**. The tool which performs images post-processing with
same configuration rules as **orbitforge-atlas-generator** (see: [Simple configuration options](#simple-configuration-options)), but instead of merging all images into atlas, it outputs
...
```
npx orbitforge-sprites-generator -i *.png -o 'intermediate' -c 'resizeConf.json'
```

2. Sub-set B (ico0.svg, ico2.svg ...) preprocessing:

```
npx orbitforge-sprites-generator -i *.svg -o 'intermediate' -c 'resizeConf.json'
```

3. Merging outputs from 1 and 2 into single sprite atlas:

```
npx orbitforge-atlas-generator -i 'intermediate/*' -o 'atlas'
```
...
Since the icons that come with **orbitforge** have a license that
...
The process of generating them is not difficult, and orbitforge-atlas-tools will help in doing so.
...
**orbitforge-atlas-tools** firstly download package using manager of your choice, for example:
...
Some simple backgrounds are already prepared in the **orbitforge-atlas-tools** package directory
under `resources-dev/backgrounds` for your convinience. It should reside at `node_modules/@orbitforge/orbitforge-atlas-tools/` sub-folder of your installation directory.

In order to simplify paths you may copy it to you current folder or create sym-link to it (you do not need to do it if your work in the root package directory: `@orbitforge/orbitforge-atlas-tools`):

```
cp -R node_modules/@orbitforge/orbitforge-atlas-tools/resources-dev resources-dev
```

or

```
ln -s node_modules/@orbitforge/orbitforge-atlas-tools/resources-dev resources-dev
```
...
To solve this problem you may create atlas in three steps. Firstly prepare bigger version of maki icons, processing them with **orbitforge-sprites-generator**, next do the same with smaller icons sizes and then merge them all together
with **orbitforge-atlas-generator** yet without any special configuration, thought you have already post-processed icons
to final shape.
...
```
npx orbitforge-sprites-generator -i "resources-tmp/icons/*-15.svg" -o "resources-tmp/sprites_day" -c "resources-dev/configs/maki-day-15.json" -v
```
...
```
npx orbitforge-sprites-generator -i "resources-tmp/icons/*-11.svg" -o "resources-tmp/sprites_day" -c "resources-dev/configs/maki-day-11.json" -v
```
...
```
npx orbitforge-atlas-generator -i "resources-tmp/sprites_day/*" -o "resources/maki_icons_day" -v
```
...
```
yarn run orbitforge-sprites-generator -i "resources-tmp/icons/*-11.svg" -o "resources-tmp/sprites_night" -c "resources-dev/configs/maki-night-11.json" -v

yarn run orbitforge-sprites-generator -i "resources-tmp/icons/*-15.svg" -o "resources-tmp/sprites_night" -c "resources-dev/configs/maki-night-15.json" -v

npx orbitforge-atlas-generator -i "resources-tmp/sprites_night/*" -o "resources/maki_icons_night" -v
```
...
The sprite atlas generated by **orbitforge-atlas-generator** uses SpriteSmith notation, that looks like this:
