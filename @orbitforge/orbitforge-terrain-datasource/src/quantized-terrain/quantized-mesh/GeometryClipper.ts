...
export class GeometryClipper {
    ...
    protected forEachSplitPermutation(callback: () => void): void {
        ...
        const runPermutations = (index = 0): void => {
    ...
    clip(mesh: Mesh, range: Range | null = null): Mesh {
    ...
    protected getClippedData(
        ...
        function pushTriangle(tri: ClipTriangle, geometry: BufferGeometry): void {
            for (let i = 0; i < 3; i++) {
    ...
    protected constructMesh(
        ...
        const indexBuffer =
            attributes.position.length / 3 > 65535
        ...
        for (const key in attributes) {
            ...
            const cons = new (attr.array.constructor as new (arr: number[]) => typeof attr.array)(
                attributes[key]
            );
        ...
        // new mesh
        const result = new Mesh(geometry, (sourceMesh.material as Material).clone());
    ...
    protected splitTriangle(
        ...
        if (edgeIndices.length !== 2) {
            ...
            const minBound = Math.min(tri.clipValues.a, tri.clipValues.b, tri.clipValues.c);
...
class ClipTriangle implements VertexData {
    indices: Indices;
    clipValues: ClipValues;
    barycoord: Triangle;
    ...
    initFromTriangle(other: ClipTriangle): this {
        return this.initFromIndices(other.indices.a, other.indices.b, other.indices.c);
    }
    ...
    lerpVertexFromEdge(
        ...
        this.clipValues[targetVertex] = MathUtils.lerp(
            other.clipValues[e0],
            other.clipValues[e1],
            alpha
        );
...
function readInterpolatedAttribute(
...
export function hashVertex(...args: number[]): string {
    const scalar = 1e6;
    const additive = 0.5;
    let result = "";
