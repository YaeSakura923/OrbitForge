...
function getSubprojectTsconfigPathsPlugins(projectRoot: string): TsconfigPathsPlugin[] {
    try {
        ...
        // Filter out subprojects under the @orbitforge scope (excluding private packages and root project)
        const orbitforgePackages = packages.filter((pkg: any) =>
            pkg.name.startsWith('@orbitforge/') &&
            pkg.path !== projectRoot
        );

        // Create TsconfigPathsPlugin configuration for each subproject
        return orbitforgePackages.map((pkg: any) => {
...
export function createDecoderConfig(config?: FlywaveWebpackConfig): Configuration {
    ...
    return {
        ...
        output: {
            path: outputPath,
            filename: process.env.NODE_ENV === "production" ? "orbitforge-decoders.min.js" : "orbitforge-decoders.js"
        }
    };
}
...
export function addWebpackConfig(
    ...
    const mainEntry = orbitforgeConfig === undefined ? undefined : orbitforgeConfig.mainEntry;
        ...
        WebpackMergeMatchLoader(
            {
                ...
                plugins: createPlugins(
                    orbitforgeConfig === undefined ? undefined : orbitforgeConfig.htmlTemplate
                ),
                stats: {
                    all: false,
                    timings: true,
    ...
    if (orbitforgeConfig !== undefined && orbitforgeConfig.decoderEntry !== undefined) {
        const decoderConfig = orbitforgeConfig.decoderEntry.endsWith(".ts")
            ? WebpackMergeMatchLoader(baseConfig, typescriptConfig)
            : baseConfig;
        bundles.push(
            WebpackMergeMatchLoader(
                {
                    target: "webworker",
                    entry: {
                        "orbitforge.decoder": orbitforgeConfig.decoderEntry
                    },
                    ...decoderConfig
                },
                userConfig
...
function createPlugins(htmlTemplate?: string): WebpackPluginInstance[] {
    const plugins = [
        new CopyWebpackPlugin({
            patterns: [
                {
                    from: `../orbitforge-map-theme/resources`,
                    to: "resources",
                    toType: "dir"
                }
            ]
