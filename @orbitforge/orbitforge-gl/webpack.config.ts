...
const mapThemePath = path.dirname(require.resolve("@orbitforge/orbitforge-map-theme/package.json"));
...
const decoderConfig = merge(createDecoderConfig({
    ...
    output: {
        ...
        filename: `orbitforge-decoders${getFilenameSuffix()}`
    }
});
...
const outputFormats = [
    {
        name: "commonjs",
        filename: `orbitforge.cjs${getFilenameSuffix()}`,
        library: {
    ...
    {
        name: "module",
        filename: `orbitforge.module${getFilenameSuffix()}`, 
        experiments: {
            outputModule: true
        },
        library: {
...
const browserConfigs = outputFormats.map(format => {
    ...
    const config = format.experiments
        ? merge(createBrowserConfig(orbitforgeConfig), configBase, { experiments: format.experiments })
        : merge(createBrowserConfig(orbitforgeConfig), configBase);

    // Add copy plugin for each configuration
    const assets = createAssetsConfig(orbitforgeConfig);
    config.plugins = [
