...
import { type TileEncoding, type TileKey } from "./TileKey";
...
export class TilingScheme {
    ...
    get mortonTileEncoding(): TileEncoding {
        return this.subdivisionScheme.mortonTileEncoding;
    }
    ...
    getTileKeyGeoOrigin(tileKey: TileKey): GeoCoordinatesLike {
        return this.boundingBoxGenerator.getTileKeyGeoOrigin(tileKey);
    }
    ...
    isEqualTilingScheme(target: TilingScheme): boolean {
        return (
            this.subdivisionScheme == target.subdivisionScheme &&
            this.projection == target.projection
        );
    }
}
