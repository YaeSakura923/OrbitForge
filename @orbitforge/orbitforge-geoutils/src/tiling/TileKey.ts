...
export class TileKey {
     ...
     */
    static fromMortonCode(quadKey64: number, encoding?: TileEncoding): TileKey {
        const effectiveEncoding = encoding ?? TileEncoding.QUAD_TREE;
        let level = 0;
        let row = 0;
        let column = 0;
        ...
        if (effectiveEncoding === TileEncoding.HALF_QUAD_TREE) {
            level -= 1;
        }
    ...
    private m_tileEncoding?: TileEncoding;
     ...
     */
    mortonCode(encoding: TileEncoding = TileEncoding.QUAD_TREE): number {
        const effectiveEncoding = encoding;

        if (this.m_tileEncoding !== effectiveEncoding) {
            this.m_mortonCode = undefined;
        }

        this.m_tileEncoding = effectiveEncoding;
        ...
        if (this.m_mortonCode === undefined) {
            ...
            for (let i = 0; i < level; ++i) {
                if (column & 0x1) result += powerOfTwo[2 * i];
                if (row & 0x1) result += powerOfTwo[2 * i + 1];
                column >>>= 1;
                row >>>= 1;
            }
    ...
    static fromArray(array: ArrayLike<number>) {
        return new TileKey(array[0], array[1], array[2]);
    }
}
