...
import { type GeoCoordLike, geoCoordLikeToGeoCoordinatesLike } from "./GeoCoordLike";
import { type GeoPointLike } from "./GeoPointLike";
import { type GeoPolygonLike } from "./GeoPolygonLike";
...
export class GeoPolygon implements GeoPolygonLike {
    ...
    public toJSON(): GeoPolygonJSON {
        return {
            type: "Polygon",
            coordinates: this.m_coordinates.map(coord => {
                return [coord.longitude, coord.latitude, coord.altitude];
            })
        };
    }

    public static fromJSON(json: GeoPolygonJSON): GeoPolygon {
        return new GeoPolygon(
            json.coordinates.map(coord => {
                return new GeoCoordinates(coord[1], coord[0], coord[2]);
            }) as GeoPolygonCoordinates
        );
    }
}
