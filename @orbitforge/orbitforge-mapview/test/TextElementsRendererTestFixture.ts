...
import { type Theme, MapEnv } from "@orbitforge/orbitforge-datasource-protocol";
import { identityProjection, TileKey, TileKeyUtils } from "@orbitforge/orbitforge-geoutils";
import { silenceLoggingAroundFunction } from "@orbitforge/orbitforge-test-utils";
import { type FontCatalog, type TextCanvas } from "@orbitforge/orbitforge-text-canvas";
...
import { type TextCanvasFactory } from "../src/text/TextCanvasFactory";
import { type TextElement } from "../src/text/TextElement";
import { TextElementsRenderer } from "../src/text/TextElementsRenderer";
import { type TextElementsRendererOptions } from "../src/text/TextElementsRendererOptions";
import { TextElementType } from "../src/text/TextElementType";
import { type ViewState } from "../src/text/ViewState";
import { type Tile } from "../src/Tile";
import { type DataSourceTileList } from "../src/VisibleTileSet";
import { FakeOmvDataSource } from "./FakeOmvDataSource";
import {
    type ElevationProviderStub,
...
export class TestFixture {
    ...
    async setUp(): Promise<void> {
        ...
        await this.m_textRenderer.waitLoaded();
    }
    ...
    private checkTextElementNotRendered(textElement: TextElement, lineMarkerIndex?: number) {
        if (textElement.type === TextElementType.PathLabel) {
            this.checkPathTextNotRendered(textElement);
            return;
        } else {
            // PathLabel or LineMarker
            if (textElement.poiInfo !== undefined) {
                this.checkPoiNotRendered(textElement, lineMarkerIndex);
                return;
            } else {
