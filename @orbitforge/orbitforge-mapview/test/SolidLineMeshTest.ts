...
import { SolidLineMaterial } from "@orbitforge/orbitforge-materials";
...
class BufferGeometryBuilder {
    ...
    protected readonly m_attributes: Record<string, THREE.BufferAttribute> = {};

    constructor(positions: number[]) {
        this.withPosition(positions);
    }

    withAttribute(name: string, values: number[], itemSize: number): this {
        this.m_attributes[name] = new THREE.BufferAttribute(new Float32Array(values), itemSize);
        return this;
    }

    withPosition(values: number[]): this {
        return this.withAttribute("position", values, 3);
    }

    withIndices(values: number[]): this {
        this.indices = new THREE.BufferAttribute(new Uint32Array(values), 1);
        return this;
    }

    withBitangent(values: number[]): this {
        return this.withAttribute("biTangent", values, 3);
    }

    withUv(values: number[]): this {
        return this.withAttribute("uv", values, 2);
    }

    withNormal(values: number[]): this {
        return this.withAttribute("normal", values, 3);
    }

    withGroups(groups: Array<{ start: number; count: number; materialIndex?: number }>): this {
...
class SolidLineGeometryBuilder extends BufferGeometryBuilder {
    ...
    addPolyline(polyline: number[], polylineBitangents: number[]): this {
        const { positions, bitangents } = SolidLineGeometryBuilder.computePositionsAndBitangents(
            polyline,
            polylineBitangents
        );
    ...
    /** @override */
    withUv(polylineUvs: number[]): this {
        super.withUv(SolidLineGeometryBuilder.replicateToExtrudedVertices(polylineUvs, 2));
        return this;
    }

    /** @override */
    withNormal(polylineNormals: number[]): this {
        super.withNormal(SolidLineGeometryBuilder.replicateToExtrudedVertices(polylineNormals, 3));
        return this;
    }
}
...
class SolidLineMeshBuilder {
    ...
    addFeature(polyline: number[], bitangents: number[]): this {
    ...
    addGroup(startFeature: number, endFeature: number, material?: THREE.Material): this {
        let materialIndex = 0;
        if (material) {
