...
import { MathUtils } from "@orbitforge/orbitforge-geoutils/math/MathUtils";
import { EarthConstants } from "@orbitforge/orbitforge-geoutils/projection/EarthConstants";
import { assert, DOMUtils, LoggerManager } from "@orbitforge/orbitforge-utils";
 ...
 */
export const TERRAIN_ZOOM_LEVEL = 5;
 ...
 * {@link @orbitforge/orbitforge-map-controls#MapControls} never matches
 * exactly the target due to the precision loss caused by the
 * conversion from zoom level to camera distance (done in
 * {@link (MapView.lookAt:WITH_PARAMS)} and {@link @orbitforge/orbitforge-map-controls#MapControls})
...
export namespace MapViewUtils {
    ...
    function computeClampedDeltaTilt(
        ...
        if (projection.type === sphereProjection.type) {
    ...
    export function getWorldTargetFromCamera(
        ...
        if (projection.type === ProjectionType.Planar) {
            const result = rayCaster.ray.intersectPlane(groundPlane, targetWorldPos);
            if (elevation !== undefined) {
    ...
    export function getTargetAndDistance(
        ...
        if (target !== null) {
            ...
            return { target: fallbackTarget, distance, altitude: elevation, final };
        }
    }

    /**
     * @internal
     * Computes the target using depth buffer information for more accurate results.
     * @param mapView - The MapView instance to access depth buffer.
     * @param camera - The camera whose target will be computed.
     * @param projection - The world space projection.
     ...
     */
    export async function getWorldTargetFromDepthBuffer(
        mapView: MapView,
        ...
        // Create 3D coordinate with the actual depth value
        const ndcWithDepth = new THREE.Vector3(ndcCenter.x, ndcCenter.y, depth);
    ...
    /**
     * @internal
     * Asynchronously computes the target for a given camera and the distance between them,
     * using depth buffer information when available for more accurate results.
     * @param projection - The world space projection.
     * @param camera - The camera whose target will be computed.
     * @param mapView - The MapView instance to access depth buffer.
     * @param elevationProvider - If provided, elevation at the camera position will be used.
     ...
     * provider was passed but the elevation was not available yet.
     */
    export async function getTargetAndDistanceAsync(
        projection: Projection,
        camera: THREE.Camera,
        mapView: MapView,
        elevationProvider?: ElevationProvider
    ): Promise<{ target: THREE.Vector3; distance: number; final: boolean }> {
        const cameraPitch = extractAttitude({ projection }, camera).pitch;
        ...
        // Try to use depth buffer for more accurate target calculation
        ...
        // If depth buffer method didn't work or isn't available, use the original method
        if (!target && cameraPitch < MAX_TILT_RAD) {
            target = getWorldTargetFromCamera(camera, projection, elevation);
        }
   ...
   export function getCameraPositionFromTargetCoordinates(
     ...
     * @internal
     *
     * Return `GeoPoints` bounding {@link @orbitforge/orbitforge-geoutils#GeoBox}
     * applicable for {@link getFitBoundsDistance}.
     *
     * @returns {@link @orbitforge/orbitforge-geoutils#GeoCoordinates} set that covers `box`
     */
    ...
    export function rayCastWorldCoordinates(
        ...
        let sphere = mapView.projection as SphereProjection;
     ...
     * @param projection - The {@link @orbitforge/orbitforge-geoutils#Projection} used to
     ...
     *
     * @param projection - The {@link @orbitforge/orbitforge-geoutils#Projection} used when
    ...
    /**
     * @deprecated Use {@link @orbitforge/orbitforge-utils#DOMUtils.getBrowserLanguages}
     */
...
export namespace TileOffsetUtils {
    /**
     * @deprecated Use {@link @orbitforge/orbitforge-geoutils#TileKeyUtils.getKeyForTileKeyAndOffset}.
     */
    ...
    /**
     * @deprecated Use {@link @orbitforge/orbitforge-geoutils#TileKeyUtils.getParentKeyFromKey}.
     */
