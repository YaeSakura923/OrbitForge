...
import { LRUCache } from "@orbitforge/orbitforge-lrucache";
import { type TaskQueue, assert, MathUtils } from "@orbitforge/orbitforge-utils";
...
import { type FrustumIntersection, TileKeyEntry } from "./FrustumIntersection";
import { type TileGeometryManager } from "./geometry/TileGeometryManager";
...
// flickering.
const compareDistances = (a: TileKeyEntry, b: TileKeyEntry, tileingScheme: TilingScheme) => {
...
export class VisibleTileSet {
    ...
    updateRenderList(
        ...
        for (const { dataSource, visibleTileKeys } of visibleTileKeysResult.tileKeys) {
            visibleTileKeys.sort((a, b) => compareDistances(a, b, dataSource.getTilingScheme()));
            ...
            dataSource.onTouchTiledComplete();
        }
    ...
    /**
     * Dispose tiles that are marked for removal by {@link @orbitforge/orbitforge-lrucache#LRUCache} algorithm.
     */
    disposePendingTiles() {
    ...
    forEachCachedTile(fun: (tile: Tile) => void, dataSource?: DataSource): void {
        this.m_dataSourceCache.forEach((tile, _) => {
            fun(tile);
        }, dataSource);
    }
    ...
    private processVisibleTiles(
        ...
        for (
            ...
            if (processDependentTiles) {
                ...
                // problem.
                const mortonTileEncoding = dataSource.getTilingScheme().mortonTileEncoding;
                for (const tileKey of tile.dependencies) {
                    if (
                        visibleTileKeys.find(
                            tileKeyEntry =>
                                tileKeyEntry.tileKey.mortonCode(mortonTileEncoding) ===
                                tileKey.mortonCode(mortonTileEncoding)
                        ) === undefined &&
    ...
    private findDown(
        ...
        const tileKey = TileKey.fromMortonCode(
            mortonCode,
            dataSource.getTilingScheme().mortonTileEncoding
        );
        ...
        for (const childTileKey of tilingScheme.getSubTileKeys(tileKey)) {
            const childTileCode = TileKeyUtils.getKeyForTileKeyAndOffset(
                ...
                offset,
                undefined,
                tilingScheme.mortonTileEncoding
            );
            const childTile = this.m_dataSourceCache.get(
                childTileKey.mortonCode(tilingScheme.mortonTileEncoding),
                offset,
                dataSource
            );
    ...
    private findUp(
        ...
        const parentTileKey = parentTile
            ? parentTile.tileKey
            : TileKey.fromMortonCode(mortonCode, dataSource.getTilingScheme().mortonTileEncoding);
    ...
    private getTileImpl(
        ...
        let tile = tileCache.get(tileKey.mortonCode(mortonTileEncoding), offset, dataSource);
        ...
        if (tile !== undefined) {
            ...
            tile.offset = offset;
            touchTile(tile);
            tileCache.set(tileKey.mortonCode(mortonTileEncoding), offset, dataSource, tile);
        }
        return tile;
    }
    ...
    private markDataSourceTilesDirty(
        ...
        const retainedTiles = new Set<TileCacheId>();
