...
import { type Projection, EarthConstants, EllipsoidProjection, ProjectionType, SphereProjection } from "@orbitforge/orbitforge-geoutils";
import { assert } from "@orbitforge/orbitforge-utils";
...
namespace SphericalProj {
    ...
    export function getProjSphereIntersectionDistance(
        ...
        R: number,
        projection: SphereProjection | EllipsoidProjection
    ): number | undefined {
...
export interface ClipPlanesEvaluator {
     ...
     * @param useLogarithmicDepth - Whether logarithmic depth buffer is enabled.
     *
     */
    evaluateClipPlanes(
        camera: THREE.Camera,
        projection: Projection,
        elevationProvider: ElevationProvider | undefined,
...
export class TiltViewClipPlanesEvaluator extends TopViewClipPlanesEvaluator {
    ...
    private computeNearDistSphericalProj(
        ...
        if (cameraBelowMaxElevation) {
            ...
            return useLogarithmicDepth ? 0.001 : 0;
        }
