...
import {
    type Env,
    type FlatTheme,
    type FontCatalogConfig,
    type PostEffects,
    type TextStyleDefinition,
    type Theme,
    type Value,
    MapEnv
} from "@orbitforge/orbitforge-datasource-protocol";
import { type ViewRanges } from "@orbitforge/orbitforge-datasource-protocol/ViewRanges";
import {
    ...
    OrientedBox3,
    ProjectionType
} from "@orbitforge/orbitforge-geoutils";
import { type GeoCoordLike } from "@orbitforge/orbitforge-geoutils/coordinates/GeoCoordLike";
import {
    type TaskQueue,
    type UriResolver,
    assert,
    ...
    LogLevel,
    PerformanceTimer
} from "@orbitforge/orbitforge-utils";
...
import { TerrainDataSource, type DataSource } from "./DataSource";
...
import { type FrameStats, PerformanceStatistics } from "./Statistics";
...
import { type TextElementsRendererOptions } from "./text/TextElementsRendererOptions";
import { type Tile } from "./Tile";
import { TileObjectRenderer } from "./TileObjectsRenderer";
import { MapViewUtils } from "./Utils";
import {
    type VisibleTileSetOptions,
...
export interface MapViewOptions extends TextElementsRendererOptions, Partial<LookAtParams> {
    ...
    /**
     * The {@link @orbitforge/orbitforge-datasource-protocol#Theme} used by Mapview.
     *
     ...
     * ```
     *
     * @see {@link @orbitforge/orbitforge-utils#UriResolver}
     * @See {@link @orbitforge/orbitforge-utils#PrefixMapUriResolver}
     */
...
export interface LookAtParams {
     ...
     *
     * * if `bounds` is {@link @orbitforge/orbitforge-geoutils#GeoBox}, then `lookAt`
     *   use {@link LookAtParams.target} or `bounds.target` and
     *   ensure whole box is visible
     *
     * * if `bounds` is {@link @orbitforge/orbitforge-geoutils#GeoPolygon}, then `lookAt`
     *   use `bounds.getCentroid()` and ensure whole polygon is visible
     *
     * * if `bounds` is {@link @orbitforge/orbitforge-geoutils#GeoBoxExtentLike},
...
export class MapView extends EventDispatcher {
    ...
    // Resize observer for automatic canvas resize detection
    private m_resizeObserver?: ResizeObserver;
    ...
    private readonly m_taskScheduler: MapViewTaskScheduler;
    ...
    constructor(options: MapViewOptions) {
        ...
        if (options.addBackgroundDatasource === undefined) {
            options.addBackgroundDatasource = MapViewDefaults.addBackgroundDatasource;
        }
        ...
        // Set up ResizeObserver to automatically detect canvas size changes
    ...
    /**
     * Sets up a ResizeObserver to automatically detect canvas size changes
     */
    private setupResizeObserver(): void {
        ...
        if (typeof ResizeObserver !== "undefined") {
            ...
            this.m_resizeObserver.observe(this.canvas);
        }
    }
    ...
    /**
     * {@link @orbitforge/orbitforge-utils#UriResolver} used to resolve application/deployment
     ...
     *
     * @param projection - The {@link @orbitforge/orbitforge-geoutils#Projection} instance to use.
     */
     ...
     *  - if the tilt is high and projection is {@link @orbitforge/orbitforge-geoutils#sphereProjection}`
    ...
    async addDataSource(dataSource: DataSource): Promise<void> {
        ...
        try {
            ...
            if (theme !== undefined && theme.styles !== undefined) {
                await dataSource.setThemeFromBase(theme);
            }
     ...
     * | ------               | ------      | --------
     * | {@link @orbitforge/orbitforge-geoutils#GeoBox}           | _defined_   | `params.target` is used
     * | {@link @orbitforge/orbitforge-geoutils#GeoBox}           | `undefined` | `bounds.center` is used as new `target`
     * | {@link @orbitforge/orbitforge-geoutils#GeoBoxExtentLike} | `undefined` | current `MapView.target` is used
     * | {@link @orbitforge/orbitforge-geoutils#GeoBoxExtentLike} | _defined_   | `params.target` is used
     ...
     *
     * @param pos - The position as a {@link @orbitforge/orbitforge-geoutils#GeoCoordLike} or
    ...
    /**
     * Returns the {@link @orbitforge/orbitforge-geoutils#GeoCoordinates} from the
    ...
    async setElevationSource(
        elevationSource: TerrainDataSource,
        elevationRangeSource: ElevationRangeSource = elevationSource.getElevationRangeSource(),
        elevationProvider: ElevationProvider = elevationSource.getElevationProvider()
    ) {
    ...
    private updateCameras(viewRanges?: ViewRanges) {
        ...
        this.m_viewRanges.maximum = this.m_viewRanges.maximum;
    ...
    /**
     * Apply minimum camera height constraint to prevent the camera from getting too close to the ground.
     */
    private applyMinCameraHeightConstraint(altitude?: number): void {
        ...
        // Get camera geographic coordinates
        const geoCoords = this.projection.unprojectPoint(cameraPos);

        const actualDistance = geoCoords.altitude - altitude;

        // Apply minCameraHeight limit
        if (actualDistance < this.minCameraHeight) {
            const correction = this.minCameraHeight - actualDistance;

            // For adjustment direction, use vertical direction as simplification
            const normal = this.projection.surfaceNormal(cameraPos, cache.vector3[0]).negate();
    ...
    protected updateLookAtSettings() {
        ...
        // Apply minCameraHeight constraint
        this.applyMinCameraHeightConstraint(altitude);
    ...
    private createVisibleTileSet(): VisibleTileSet {
        ...
        const enableMixedLod = this.m_enableMixedLod === undefined ? true : this.m_enableMixedLod;
