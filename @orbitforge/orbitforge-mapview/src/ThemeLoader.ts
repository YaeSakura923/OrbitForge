...
import "@orbitforge/orbitforge-fetch";

import { isJsonExpr } from "@orbitforge/orbitforge-datasource-protocol";
import {
    type Definitions,
    ...
    getDefinitionValue,
    isJsonExprReference
} from "@orbitforge/orbitforge-datasource-protocol/Theme";
import {
    type IContextLogger,
    type ISimpleChannel,
    type UriResolver,
...
/**
 * Options to customize {@link @orbitforge/orbitforge-datasource-protocol#Theme} loading process.
 *
...
export interface ThemeLoadOptions {
     ...
     * through [{@link @orbitforge/orbitforge-datasource-protocol#Theme}s `extends` property.
     *
...
export class ThemeLoader {
    /**
     * Loads a {@link @orbitforge/orbitforge-datasource-protocol#Theme} from a
     ...
     * providing {@link @orbitforge/orbitforge-utils#UriResolver} using // {@link ThemeLoadOptions.uriResolver}
     * option.
     *
     * @param theme - {@link @orbitforge/orbitforge-datasource-protocol#Theme} instance or theme URL
     *                to the theme.
     ...
     *
     * Loads a {@link @orbitforge/orbitforge-datasource-protocol#Theme} from a remote resource,
    ...
    /**
     * Resolves all {@link @orbitforge/orbitforge-datasource-protocol#Theme}'s relatives URLs
     * to full URL using the {@link @orbitforge/orbitforge-datasource-protocol#Theme}'s URL
     * (see: https://www.w3.org/TR/WD-html40-970917/htmlweb.html#h-5.1.2).
     *
     * This method mutates original `theme` instance.
     *
     * @param theme - The {@link @orbitforge/orbitforge-datasource-protocol#Theme} to resolve.
     */
    ...
    /**
     * Expand all `ref` expressions in {@link @orbitforge/orbitforge-datasource-protocol#Theme}
     ...
     * its base {@link @orbitforge/orbitforge-datasource-protocol#Theme}.
     *
     * @param theme - {@link @orbitforge/orbitforge-datasource-protocol#Theme} object
     * @param options - Optional, a {@link ThemeLoadOptions} objects
     *                  containing any custom settings for
     *                  this load request.
     */
    ...
    private static resolveResources(theme: Theme, childUrlResolver: UriResolver) {
        ...
        // Resolve environment map URLs
        if (theme.environment) {
            // Check if it's a cube map (has urls array)
            if (Array.isArray((theme.environment as EnvironmentMapDefinition).urls)) {
                const environmentMap = theme.environment as EnvironmentMapDefinition;
                const urls = environmentMap.urls;
                const resolvedUrls: [string, string, string, string, string, string] = [
                    childUrlResolver.resolveUri(urls[0]),
                    childUrlResolver.resolveUri(urls[1]),
                    childUrlResolver.resolveUri(urls[2]),
                    childUrlResolver.resolveUri(urls[3]),
                    childUrlResolver.resolveUri(urls[4]),
                    childUrlResolver.resolveUri(urls[5])
                ];
                environmentMap.urls = resolvedUrls;
            }
            // Check if it's an equirectangular map (has url property)
            else if ((theme.environment as EquirectangularEnvironmentMapDefinition).url) {
                const environmentMap = theme.environment as EquirectangularEnvironmentMapDefinition;
                environmentMap.url = childUrlResolver.resolveUri(environmentMap.url);
            }
        }
