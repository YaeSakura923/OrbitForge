...
import { type MemoryUsage } from "@orbitforge/orbitforge-text-canvas";
import { Math2D } from "@orbitforge/orbitforge-utils";
...
export class BoxBufferMesh extends THREE.Mesh {
    ...
    get isEmpty(): boolean {
        if (this.geometry === undefined) {
            ...
            return bufferGeometry.index === null || bufferGeometry.index.array.length === 0;
        }
    }
}
...
export class BoxBuffer {
    /**
     * {@link @here/FLYWAVE-datasource-protocol#BufferAttribute} holding the `BoxBuffer` position data.
     */
    private m_positionAttribute?: THREE.BufferAttribute;

    /**
     * {@link @here/FLYWAVE-datasource-protocol#BufferAttribute} holding the `BoxBuffer` color data.
     */
    private m_colorAttribute?: THREE.BufferAttribute;

    /**
     * {@link @here/FLYWAVE-datasource-protocol#BufferAttribute} holding the `BoxBuffer` uv data.
     */
    private m_uvAttribute?: THREE.BufferAttribute;

    /**
     * {@link @here/FLYWAVE-datasource-protocol#BufferAttribute} holding the `BoxBuffer` index data.
     */
    private m_indexAttribute?: THREE.BufferAttribute;

    // Track the actual number of elements used (replaces the read-only count property)
    private m_positionCount: number = 0;
    private m_colorCount: number = 0;
    private m_uvCount: number = 0;
     ...
     * {@link @here/FLYWAVE-datasource-protocol#BufferAttribute}s.
     */
    ...
    reset() {
        if (this.m_positionAttribute !== undefined) {
            ...
            this.m_geometry.drawRange.start = 0;
            this.m_geometry.drawRange.count = 0;
        }
    }
    ...
    canAddElements(glyphCount = 1): boolean {
        ...
        if (
            this.m_indexCount + glyphCount * NUM_INDICES_PER_ELEMENT >=
    ...
    saveState(): State {
        const state: State = {
            positionAttributeCount: this.m_positionCount,
            colorAttributeCount: this.m_colorCount,
            uvAttributeCount: this.m_uvCount,
            indexAttributeCount: this.m_indexCount,
    ...
    restoreState(state: State) {
        this.m_positionCount = state.positionAttributeCount;
        this.m_colorCount = state.colorAttributeCount;
        this.m_uvCount = state.uvAttributeCount;
        this.m_indexCount = state.indexAttributeCount;
        this.m_pickInfos!.length = state.pickInfoCount;

        // Resize arrays to match the saved state if necessary
        this.resizeBufferToCount(
            state.positionAttributeCount,
            state.colorAttributeCount,
            state.uvAttributeCount,
            state.indexAttributeCount
        );
    }
    ...
    addBox(
        ...
        const r = Math.round(color.r * opacity);
        const g = Math.round(color.g * opacity);
        const b = Math.round(color.b * opacity);
        const a = Math.round(opacity);
        ...
        const baseIndex = this.m_indexCount;
        ...
        this.m_positionCount += NUM_VERTICES_PER_ELEMENT;
        this.m_colorCount += NUM_VERTICES_PER_ELEMENT;
        this.m_uvCount += NUM_VERTICES_PER_ELEMENT;
        this.m_indexCount += NUM_INDICES_PER_ELEMENT;
    ...
    updateBufferGeometry() {
        ...
        if (this.m_positionCount > 0) {
            ...
            positionAttribute.addUpdateRange(0, this.m_positionCount * positionAttribute.itemSize);
        }

        if (this.m_colorCount > 0) {
            colorAttribute.needsUpdate = true;
            colorAttribute.addUpdateRange(0, this.m_colorCount * colorAttribute.itemSize);
        }

        if (this.m_uvCount > 0) {
            uvAttribute.needsUpdate = true;
            uvAttribute.addUpdateRange(0, this.m_uvCount * uvAttribute.itemSize);
        }

        if (this.m_indexCount > 0) {
            indexAttribute.needsUpdate = true;
            this.m_geometry.setDrawRange(0, this.m_indexCount);
        }

        if (this.m_geometry !== undefined) {
            this.m_geometry.clearGroups();
            this.m_geometry.addGroup(0, this.m_indexCount);
        }
    }
    ...
    cleanUp() {
        ...
        if (this.m_indexCount === 0 && this.size > START_BOX_BUFFER_SIZE) {
    ...
    /**
     * Creates a new {@link @here/FLYWAVE-datasource-protocol#Geometry} object
    ...
    updateMemoryUsage(info: MemoryUsage) {
        const numBytes =
            this.m_positionCount * NUM_POSITION_VALUES_PER_VERTEX * NUM_BYTES_PER_FLOAT +
            this.m_colorCount * NUM_COLOR_VALUES_PER_VERTEX +
            this.m_uvCount * NUM_UV_VALUES_PER_VERTEX * NUM_BYTES_PER_FLOAT +
            this.m_indexCount * NUM_BYTES_PER_INT32; // May be UInt16, so we overestimate
    ...
    private isPixelTransparent(
        ...
        // Fix the type checking for image dimensions
        ...
        if (image instanceof SVGImageElement) {
            ...
            // Default values if we can't determine dimensions
            width = 1;
            height = 1;
        }
        const x = width * u;
    ...
    private resizeBuffer(newSize: number) {
        ...
        if (this.m_positionAttribute !== undefined && this.m_positionAttribute.array.length > 0) {
            ...
            const copyCount = Math.min(
                newPositionArray.length,
                this.m_positionAttribute.array.length
            );
            newPositionArray.set(this.m_positionAttribute.array.slice(0, copyCount));
        }
        ...
        if (this.m_colorAttribute !== undefined && this.m_colorAttribute.array.length > 0) {
            // Copy existing data
            const copyCount = Math.min(newColorArray.length, this.m_colorAttribute.array.length);
            newColorArray.set(this.m_colorAttribute.array.slice(0, copyCount));
        }
        ...
        if (this.m_uvAttribute !== undefined && this.m_uvAttribute.array.length > 0) {
            // Copy existing data
            const copyCount = Math.min(newUvArray.length, this.m_uvAttribute.array.length);
            newUvArray.set(this.m_uvAttribute.array.slice(0, copyCount));
        }
        ...
        if (this.m_indexAttribute !== undefined && this.m_indexAttribute.array.length > 0) {
            // Copy existing data
            const copyCount = Math.min(newIndexArray.length, this.m_indexAttribute.array.length);
            newIndexArray.set(this.m_indexAttribute.array.slice(0, copyCount));
        }
    ...
    /**
     * Resize buffers to match specific counts
     */
    private resizeBufferToCount(
        posCount: number,
        colorCount: number,
        uvCount: number,
        indexCount: number
    ) {
        // Ensure arrays are large enough to hold the specified counts
        if (this.m_positionAttribute) {
            const requiredPosLength = posCount * this.m_positionAttribute.itemSize;
            if (this.m_positionAttribute.array.length < requiredPosLength) {
                const newPositionArray = new Float32Array(requiredPosLength);
                newPositionArray.set(
                    this.m_positionAttribute.array.slice(
                        0,
                        Math.min(newPositionArray.length, this.m_positionAttribute.array.length)
                    )
                );
        ...
        if (this.m_colorAttribute) {
            const requiredColorLength = colorCount * this.m_colorAttribute.itemSize;
            if (this.m_colorAttribute.array.length < requiredColorLength) {
                const newColorArray = new Uint8Array(requiredColorLength);
                newColorArray.set(
                    this.m_colorAttribute.array.slice(
                        0,
                        Math.min(newColorArray.length, this.m_colorAttribute.array.length)
                    )
                );
        ...
        if (this.m_uvAttribute) {
            const requiredUvLength = uvCount * this.m_uvAttribute.itemSize;
            if (this.m_uvAttribute.array.length < requiredUvLength) {
                const newUvArray = new Float32Array(requiredUvLength);
                newUvArray.set(
                    this.m_uvAttribute.array.slice(
                        0,
                        Math.min(newUvArray.length, this.m_uvAttribute.array.length)
                    )
                );
        ...
        if (this.m_indexAttribute) {
            const requiredIndexLength = indexCount * this.m_indexAttribute.itemSize;
            if (this.m_indexAttribute.array.length < requiredIndexLength) {
                const newIndexArray =
                    this.m_indexAttribute.array instanceof Uint32Array
                        ? new Uint32Array(requiredIndexLength)
                        : new Uint16Array(requiredIndexLength);
                newIndexArray.set(
                    this.m_indexAttribute.array.slice(
                        0,
                        Math.min(newIndexArray.length, this.m_indexAttribute.array.length)
                    )
                );
