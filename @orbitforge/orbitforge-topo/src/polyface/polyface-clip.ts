...
import { type Range2d, Range1d, Range3d } from "../geometry3d/range";
...
export class PolyfaceClip {
                ...
                if (contour !== undefined) {
                    if (clipper) {
                        ...
                        if (destination.builderA) {
                            contour.emitFacets(destination.builderA, true);
                        }
                        if (destination.builderB) {
                            contour.emitFacets(destination.builderB, false);
                        }
                    }
                }
            }
    ...
    public static drapeRegion(
        ...
        if (sweepVector && contour.localToWorld.matrix.dotColumnZ(sweepVector) < 0.0) {
            sweep = sweepVector.scale(-1);
        } // avoid inverted clipper
