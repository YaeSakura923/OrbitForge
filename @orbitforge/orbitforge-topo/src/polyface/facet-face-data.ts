...
import { IndexedPolyfaceVisitor } from "./indexed-polyface-visitor";
...
export class FacetFaceData {
    private readonly _paramDistanceRange: Range2d;
    private readonly _paramRange: Range2d;
            ...
            paramDelta.x === 0
                ...
                : this._paramDistanceRange.low.x +
                  ((x - this._paramRange.low.x) *
                      (this._paramDistanceRange.high.x - this._paramDistanceRange.low.x)) /
                      paramDelta.x;
        result.y =
            paramDelta.y === 0.0
                ? y
                : this.paramDistanceRange.low.y +
                  ((y - this._paramRange.low.y) *
                      (this._paramDistanceRange.high.y - this._paramDistanceRange.low.y)) /
                      paramDelta.y;
        return result;
    }
    ...
    public convertParamXYToNormalized(x: number, y: number, result?: Point2d): Point2d {
        result = result ? result : Point2d.create();
        const paramDelta = this._paramRange.high.minus(this._paramRange.low);
        result.x = paramDelta.x === 0.0 ? x : (x - this._paramRange.low.x) / paramDelta.x;
        result.y = paramDelta.y === 0.0 ? y : (y - this._paramRange.low.y) / paramDelta.y;
        return result;
    }
