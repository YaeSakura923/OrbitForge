...
export class GriddedRaggedRange2dSet<T> implements Range2dSearchInterface<T> {
    private readonly _range: Range2d;
    private readonly _numXEdge: number;
    private readonly _numYEdge: number;
    /** Each grid block is a simple linear search set */
    private readonly _rangesInBlock: Array<Array<OptionalRange2dSearchInterface<T>>>;
    ...
    public conditionalInsert(range: Range2d | Range3d | LowAndHighXY, tag: T): boolean {
        if (Range2d.isNull(range)) return false;
    ...
    public totalRange(result?: Range2d): Range2d {
        ...
        this.visitChildren(0, (_depth, child) => {
            const childRange = (GriddedRaggedRange2dSet._workRange = child.totalRange(
                GriddedRaggedRange2dSet._workRange
            ));
