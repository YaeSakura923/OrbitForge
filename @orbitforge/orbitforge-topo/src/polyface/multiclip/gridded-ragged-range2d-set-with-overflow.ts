...
import { GriddedRaggedRange2dSet } from "./gridded-ragged-range2d-set";
import { LinearSearchRange2dArray } from "./linear-search-range2d-array";
...
export class GriddedRaggedRange2dSetWithOverflow<T> implements Range2dSearchInterface<T> {
    private readonly _gridSet: GriddedRaggedRange2dSet<T>;
    private readonly _overflowSet: Range2dSearchInterface<T>;
    private static _workRange?: Range2d;
    private constructor(
        gridSet: GriddedRaggedRange2dSet<T>,
        overflowSet: Range2dSearchInterface<T>
    ) {
    ...
    public totalRange(result?: Range2d): Range2d {
        ...
        else result = Range2d.createNull();
        this.visitChildren(0, (_depth, child) => {
            const childRange = (GriddedRaggedRange2dSetWithOverflow._workRange = child.totalRange(
                GriddedRaggedRange2dSetWithOverflow._workRange
            ));
    ...
    public visitChildren(
        initialDepth: number,
        handler: (depth: number, child: Range2dSearchInterface<T>) => void
    ) {
