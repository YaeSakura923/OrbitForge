...
export class SweepLineStringToFacetContext {
    private readonly _spacePoints: GrowableXYZArray;
    private readonly _spacePointsRange: Range3d;
    private readonly _numSpacePoints: number;
    ...
    private readonly _segmentPoint0 = Point3d.create();
    private readonly _segmentPoint1 = Point3d.create();
    private readonly _localSegmentPoint0 = Point3d.create();
    private readonly _localSegmentPoint1 = Point3d.create();
    private readonly _clipFractions = Segment1d.create(0, 1);
    private readonly _localFrame = Transform.createIdentity();
    private readonly _polygonRange = Range3d.create();
...
export class EdgeClipData {
    ...
    private readonly _crossingPoints: Point3d[];
    ...
    public static createPointPointSweep(
        pointA: Point3d,
        pointB: Point3d,
        sweep: Vector3d
    ): EdgeClipData | undefined {
    ...
    public processPolygon(
        polygon: Point3d[] | GrowableXYZArray,
        announceEdge: (pointA: Point3d, pointB: Point3d) => void
    ) {
        ...
        if (Array.isArray(polygon)) {
            ...
            IndexedXYZCollectionPolygonOps.polygonPlaneCrossings(
                this.edgePlane,
                polygon,
                this._crossingPoints
            );
        ...
        if (this._crossingPoints.length === 2) {
            ...
            this.clip.announceClippedSegmentIntervals(
                0,
                1,
                this._crossingPoints[0],
                this._crossingPoints[1],
                (f0: number, f1: number) => {
                    announceEdge(
...
export class ClipSweptLineStringContext {
    private readonly _edgeClippers: EdgeClipData[];
    private readonly _localToWorld?: Transform;
    private readonly _worldToLocal?: Transform;
    private readonly _localRange?: Range3d;
    private constructor(
        edgeData: EdgeClipData[],
        localData:
            | undefined
            | { localToWorld: Transform; worldToLocal: Transform; localRange: Range3d }
    ) {
    ...
    public static create(
        ...
        sweepVector: Vector3d | undefined
    ): ClipSweptLineStringContext | undefined {
        if (sweepVector === undefined) sweepVector = Vector3d.create(0, 0, 1);
        if (xyz.length > 1) {
            ...
            if (localToWorldMatrix === undefined) localToWorldMatrix = Matrix3d.createIdentity();
    ...
    public processPolygon(
        ...
        if (this._worldToLocal !== undefined && this._localRange !== undefined) {
            ...
            if (!polygonRange.intersectsRangeXY(this._localRange)) return;
        }
