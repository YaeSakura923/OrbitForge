...
export class OffsetMeshContext {
    ...
    private readonly _basePolyface: IndexedPolyface;
    private readonly _baseGraph: HalfEdgeGraph;
    /** "Exterior" side of a bare edge of the mesh */
    public get exteriorMask(): HalfEdgeMask {
        return this._exteriorMask;
    }

    private readonly _exteriorMask: HalfEdgeMask;

    /** Mask indicating a a sector's coordinates have been reassigned at offset distance. */
    private readonly _offsetCoordinatesReassigned: HalfEdgeMask;
    ...
    private readonly _breakMaskA: HalfEdgeMask;
    ...
    private readonly _breakMaskB: HalfEdgeMask;
    ...
    private readonly _insideOfChamferFace: HalfEdgeMask;
    ...
    private readonly _outsideOfChamferFace: HalfEdgeMask;
    ...
    private readonly _insideChamferSling: HalfEdgeMask;
    ...
    private readonly _outsideEndOfChamferFace: HalfEdgeMask;
    ...
    private readonly _smoothRadiansBetweenNormals: number;
    private readonly _smoothAccumulatedRadiansBetweenNormals: number;
    private readonly _chamferTurnRadians: number;
    ...
    private getCoordinateString(
        ...
        if (showXYZ) {
            if (showFaceSuccessorXYZ) {
                return `${HalfEdge.nodeToIdXYZString(node)} ==> ${HalfEdge.nodeToIdXYZString(
                    node.faceSuccessor
                )}`;
            } else {
    ...
    private assignOffsetByAverageNormalAroundVertex(
        ...
        const maxDeviationRadians = this.computeAverageNormalAndMaxDeviationAroundVertex(
            vertexSeed,
            data
        );
    ...
    private compute3SectorIntersectionDebug(
        ...
        if (OffsetMeshContext.stringDebugFunction !== undefined) {
            OffsetMeshContext.stringDebugFunction(
                `compute3${this.inspectMasks(nodeA)}${this.inspectMasks(nodeB)}${this.inspectMasks(
                    nodeC
                )} `
            );
    ...
    private transferXYZFromNodeToSmoothSector(
        ...
        if (OffsetMeshContext.stringDebugFunction !== undefined) {
            OffsetMeshContext.stringDebugFunction(
                `    ${description} ${this.inspectMasks(sourceNode)} to ${this.inspectMasks(
