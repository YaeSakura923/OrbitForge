...
import { CylindricalRangeQuery } from "../curve/query/cylindrical-range";
import { StrokeCountSection } from "../curve/query/stroke-count-chain";
...
import { BilinearPatch } from "../geometry3d/bilinear-patch";
...
import { type XYZ, Point3d, Vector3d } from "../geometry3d/point3d-vector3d";
import { PolygonOps } from "../geometry3d/polygon-ops";
import { Range1d, Range3d } from "../geometry3d/range";
import { Segment1d } from "../geometry3d/segment1d";
import { Transform } from "../geometry3d/transform";
import { UVSurfaceOps } from "../geometry3d/uv-surface-ops";
...
import { BoxTopology } from "./box-topology";
import { GreedyTriangulationBetweenLineStrings } from "./greedy-triangulation-between-line-strings";
import { type SortableEdgeCluster, SortableEdge } from "./indexed-edge-matcher";
...
import { PolyfaceQuery } from "./polyface-query";
...
class FacetSector {
    ...
    private static readonly _edgeVector: Vector3d = Vector3d.create();
    /**
...
export class PolyfaceBuilder extends NullGeometryHandler {
    ...
    private readonly _options: StrokeOptions;
    ...
    public claimPolyface(
        ...
        if (compress) this._polyface.data.compress(tolerance);
    ...
    private static readonly _workPointFindOrAddA = Point3d.create();
    private static readonly _workVectorFindOrAdd = Vector3d.create();
    private static readonly _workUVFindOrAdd = Point2d.create();
    /**
    ...
    public addRuledSweep(surface: RuledSweep): boolean {
        ...
        for (const contour of contours) {
            sectionMaps.push(
                StrokeCountSection.createForParityRegionOrChain(contour.curves, this._options)
            );
        }
    ...
    public addPolygonGrowableXYZArray(points: GrowableXYZArray) {
        ...
        while (
            numPointsToUse > 2 &&
            Geometry.isSmallMetricDistance(points.distanceIndexIndex(0, numPointsToUse - 1)!)
        ) {
            numPointsToUse--;
        }
    ...
    public addFacetFromIndexedVisitor(visitor: PolyfaceVisitor, indices: number[]): boolean {
        if (indices.length > visitor.pointIndex.length) return false;
        ...
        for (let i = 0; i < indices.length; ++i) {
            ...
            if (index < 0 || index >= visitor.point.length) {
                // all visitor arrays have the same length
                return false;
            }
            xyz.pushXYZ(
            ...
            if (visitor.color && color) color[i] = visitor.color[index];
            if (visitor.edgeVisible && visible) visible[i] = visitor.edgeVisible[index];
        }
    ...
    public addGraph(
        graph: HalfEdgeGraph,
        acceptFaceFunction: HalfEdgeToBooleanFunction = node =>
            HalfEdge.testNodeMaskNotExterior(node),
        isEdgeVisibleFunction: HalfEdgeToBooleanFunction | undefined = node =>
            HalfEdge.testMateMaskExterior(node)
    ) {
        let index = 0;
        const needNormals = this._options.needNormals;
        const needParams = this._options.needParams;
        let normalIndex = 0;
        if (needNormals) normalIndex = this._polyface.addNormalXYZ(0, 0, 1); // big assumption!!!! Is each node.z really the same?
            ...
            if (acceptFaceFunction(seed) && seed.countEdgesAroundFace() > 2) {
                ...
                do {
                    ...
                    this._polyface.addPointIndex(
                        index,
                        isEdgeVisibleFunction === undefined ? true : isEdgeVisibleFunction(node)
                    );
                    if (needParams) {
    ...
    public static pointsToTriangulatedPolyface(
        ...
        if (graph) return PolyfaceBuilder.graphToPolyface(graph, options);
        return undefined;
    }
    ...
    public addSweptIndexedPolyface(
        ...
        for (const edgeOrCluster of baseBoundaryEdges) {
            if (edgeOrCluster instanceof SortableEdge) {
                ...
                for (const edge of edgeOrCluster) this.addSweptFace(edge, numBaseFacets);
            }
        }
