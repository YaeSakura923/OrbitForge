...
import { MultiChainCollector } from "../curve/internal/multi-chain-collector";
...
import {
    type TriangleLocationDetail,
    BarycentricTriangle
} from "../geometry3d/barycentric-triangle";
...
import { type PolygonLocationDetail, PolygonOps } from "../geometry3d/polygon-ops";
import { Range3d } from "../geometry3d/range";
import { type Ray3d } from "../geometry3d/ray3d";
import { Matrix4d } from "../geometry4d/matrix4d";
import { MomentData } from "../geometry4d/moment-data";
import { UnionFindContext } from "../numerics/union-find";
...
import { HalfEdgeGraphFromIndexedLoopsContext } from "../topology/half-edge-graph-from-indexed-loops-context";
import { HalfEdgeGraphSearch, HalfEdgeMaskTester } from "../topology/half-edge-graph-search";
import { HalfEdgeGraphMerge } from "../topology/merging";
import { SpacePolygonTriangulation } from "../topology/space-triangulation";
import {
    type FacetIntersectOptions,
    type FacetLocationDetail,
    ConvexFacetLocationDetail,
    NonConvexFacetLocationDetail,
    TriangularFacetLocationDetail
} from "./facet-location-detail";
import { FacetOrientationFixup } from "./facet-orientation";
import { type SortableEdgeCluster, IndexedEdgeMatcher, SortableEdge } from "./indexed-edge-matcher";
import { IndexedPolyfaceSubsetVisitor } from "./indexed-polyface-visitor";
import { BuildAverageNormalsContext } from "./multiclip/build-average-normals-context";
import { OffsetMeshContext } from "./multiclip/offset-mesh-context";
import { type Range2dSearchInterface } from "./multiclip/range2d-search-interface";
import {
    ClipSweptLineStringContext,
    EdgeClipData,
    SweepLineStringToFacetContext
} from "./multiclip/sweep-line-string-to-facet-context";
import { XYPointBuckets } from "./multiclip/xy-point-buckets";
...
export class SweepLineStringToFacetsOptions {
    ...
    public static create(
        ...
        return new SweepLineStringToFacetsOptions(
            ...
            sideAngle === undefined
                ? Angle.createRadians(Geometry.smallAngleRadians)
                : sideAngle.clone(),
    ...
    public get collectAll() {
        return (
            this.collectOnForwardFacets === true &&
    ...
    public collectFromThisFacetNormal(facetNormal: Vector3d | undefined): boolean {
        if (facetNormal === undefined) return false;
        const theta = facetNormal.angleFromPerpendicular(this.vectorToEye);
        if (theta.isMagnitudeLessThanOrEqual(this.sideAngle)) return this.collectOnSideFacets;
        return facetNormal.dotProduct(this.vectorToEye) > 0
            ? this.collectOnForwardFacets
            : this.collectOnRearFacets;
    }
}
...
export class PolyfaceQuery {
    ...
    public static sumFacetAreas(
        ...
        if (source !== undefined) {
            ...
            while (source.moveToNextFacet()) {
                ...
                s += unitVectorToEye
                    ? scaledNormal.dotProduct(unitVectorToEye)
                    : scaledNormal.magnitude();
            }
        }
        return s;
    }
    ...
    public static announceBoundaryEdges(
        ...
        const badList = [];
        if (includeTypical && bad1.length > 0) badList.push(bad1);
    ...
    public static cloneWithMaximalPlanarFacets(
        ...
        const partitions = PolyfaceQuery.partitionFacetIndicesBySortableEdgeClusters(
            smoothEdges,
            numFacets
        );
    ...
    public static cloneWithDanglingEdgesRemoved(
        source: Polyface | PolyfaceVisitor
    ): IndexedPolyface {
        if (source instanceof Polyface) {
        ...
        const removeFirstOddPalindrome = (indices: number[], data: number[]): boolean => {
            ...
            for (let i = 0; i < n; ++i) {
                ...
                while (palLength + 2 <= n) {
                    ...
                    if (data[indices[iPrev]] !== data[indices[iNext]]) break; // the maximal odd palindrome centered at i has length palLength and spans [i0,i1]
                    i0 = iPrev;
                    i1 = iNext;
                    palLength += 2;
                }
                if (palLength > 1) {
                    // excise the palindrome (but keep i1)
                    if (i0 < i1) {
    ...
    public static sweepLineStringToFacets(
        ...
        if (options.assembleChains) chainContext = ChainMergeContext.create();
        const context = ClipSweptLineStringContext.create(linestringPoints, options.vectorToEye);
        if (context) {
            let visitor: PolyfaceVisitor;
            if (polyfaceOrVisitor instanceof Polyface) visitor = polyfaceOrVisitor.createVisitor(0);
            else visitor = polyfaceOrVisitor;
            const workNormal = Vector3d.createZero();
            for (visitor.reset(); visitor.moveToNextFacet(); ) {
                if (
                    options.collectFromThisFacetNormal(
                        PolygonOps.areaNormalGo(visitor.point, workNormal)
                    )
                ) {
                    context.processPolygon(
                        visitor.point.getArray(),
                        (pointA: Point3d, pointB: Point3d) => {
                            if (chainContext !== undefined) chainContext.addSegment(pointA, pointB);
                            else result.push(LineSegment3d.create(pointA, pointB));
                        }
                    );
                }
            }
    ...
    public static sweepLineStringToFacetsXY(
        ...
        } else lineStringSource = lineStringPoints;
            ...
            if (edgeClipper !== undefined) {
                ...
                searchByReadIndex.searchRange2d(searchRange, (_facetRange, readIndex) => {
                    if (visitor.moveToReadIndex(readIndex)) {
                        edgeClipper.processPolygon(visitor.point, (pointA, pointB) => {
        ...
        for (oldFacetVisitor.reset(); oldFacetVisitor.moveToNextFacet(); ) {
            ...
            for (let i = 0; i + 1 < oldFacetVisitor.point.length; i++) {
                ...
                if (detailArray !== undefined) {
                    detailArray.sort(
                        (a: CurveLocationDetail, b: CurveLocationDetail) => a.fraction - b.fraction
                    );
    ...
    public static announceDuplicateFacetIndices(
        ...
        facetIndexAndVertexIndices.sort((arrayA, arrayB) =>
            this.compareFacetIndexAndVertexIndices(arrayA, arrayB)
        );
        let i0, i1;
    ...
    public static getSingleEdgeVisibility(
        ...
        const index1 = polyface.facetIndex1(facetIndex);
        for (let i = index0; i < index1; i++) {
            if (data.pointIndex[i] === vertexIndex) return data.edgeVisible[i];
        } // return visibility of first edge in the face that starts at this vertex
        return undefined;
    }
    ...
    public static collectEdgesByDihedralAngle(
        ...
        for (const pair of manifoldEdges) {
            if (Array.isArray(pair) && pair.length === 2) {
                ...
                if (
                    undefined !==
                        PolyfaceQuery.computeFacetUnitNormal(mesh, e0.facetIndex, normal0) &&
        ...
        if (sharpEdgeAngle !== undefined) {
            ...
            for (const pair of pairedEdges) {
                if (Array.isArray(pair) && pair.length === 2) {
                    ...
                    if (
                        undefined !==
                            PolyfaceQuery.computeFacetUnitNormal(visitor, e0.facetIndex, normal0) &&
                        undefined !==
                            PolyfaceQuery.computeFacetUnitNormal(visitor, e1.facetIndex, normal1)
                    ) {
                        const edgeAngle = normal0.smallerUnorientedAngleTo(normal1);
