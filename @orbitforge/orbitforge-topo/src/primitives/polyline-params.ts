...
import { type PolylineIndices, PolylineTypeFlags } from "../common/render";
import {
    type PolylineParams,
    type TesselatedPolyline
} from "../common/render/primitives/polyline-params";
...
class PolylineTesselator {
    private readonly _polylines: PolylineIndices[];
    private readonly _doJoints: boolean;
    ...
    public static fromMesh(args: MeshArgs): PolylineTesselator | undefined {
        if (undefined !== args.edges?.polylines.lines && undefined !== args.points) {
            return new PolylineTesselator(
                ...
                args.points,
                wantJointTriangles(args.edges.width, args.is2d === true)
            );
        }
        ...
        for (const line of this._polylines) {
            ...
            for (let i = 0; i < last; ++i) {
                ...
                const jointAt0: boolean =
                    this._doJoints && (isClosed || !isStart) && this._dotProduct(v0) > maxJointDot;
                const jointAt1: boolean =
                    this._doJoints && (isClosed || !isEnd) && this._dotProduct(v1) > maxJointDot;
