...
import {
    ...
    type UVSurface,
    type UVSurfaceIsoParametricDistance
} from "../geometry3d/geometry-handler";
...
export class Cone extends SolidPrimitive implements UVSurface, UVSurfaceIsoParametricDistance {
    ...
    private readonly _localToWorld: Transform; // Transform from local to global.
    private readonly _radiusA: number; // nominal radius at z=0.  skewed axes may make it an ellipse
    private readonly _radiusB: number; // radius at z=1.  skewed axes may make it an ellipse
    private readonly _maxRadius: number; // maximum radius anywhere on the cone.
     ...
     */
    public static createAxisPoints(
        centerA: Point3d,
        centerB: Point3d,
        radiusA: number,
        radiusB: number,
     ...
     * to the axis.
     */
    public static createBaseAndTarget(
        centerA: Point3d,
        centerB: Point3d,
        vectorX: Vector3d,
        vectorY: Vector3d,
    ...
    public getCenterA(): Point3d {
        return this._localToWorld.multiplyXYZ(0, 0, 0);
    }

    /** (Property accessor) */
    public getCenterB(): Point3d {
    ...
    public vFractionToRadius(v: number): number {
        return Geometry.interpolate(this._radiusA, v, this._radiusB);
    }

    /** (Property accessor) test if `other` is an instance of `Cone` */
    public isSameGeometryClass(other: any): boolean {
        return other instanceof Cone;
    }
