...
import { RegionOps } from "../curve/region-ops";
...
export class SweepContour {
    ...
    private constructor(contour: AnyCurve, map: Transform, axis: Ray3d | undefined) {
        if (contour instanceof CurvePrimitive) {
            ...
            contour = contour.startPoint().isAlmostEqual(contour.endPoint())
                ? new Loop()
                : new Path();
    ...
    public isAlmostEqual(other: any): boolean {
        if (!(other instanceof SweepContour)) return false;
        if (!this.curves.isAlmostEqual(other.curves)) return false;
        if (!this.localToWorld.isAlmostEqual(other.localToWorld)) return false;
        if (this.axis && other.axis) {
            if (!this.axis.isAlmostEqual(other.axis)) return false;
        } else if (this.axis || other.axis) return false;
        return true;
    }
    ...
    public computeXYStrokes(options?: StrokeOptions): void {
        ...
        if (worldToLocal) {
            ...
            if (strokes.tryTransformInPlace(worldToLocal)) this._xyStrokes = strokes;
        }
    }
    ...
    public buildFacets(options?: StrokeOptions): void {
        if (this._facets) return;
        if (!this.curves.isAnyRegion()) return;
        const worldToLocal = this.localToWorld.inverse();
        if (!worldToLocal) return;
