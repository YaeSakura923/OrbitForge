...
export class CurveExtendOptions {
    ...
    public static resolveVariantCurveExtendParameterToCurveExtendMode(
        ...
        if (param === false) return CurveExtendMode.None;
        if (param === true) return CurveExtendMode.OnCurve;
        if (Array.isArray(param)) return param[endIndex];
        return param;
    }
    ...
    public static correctFraction(
        ...
        if (fraction < 0) {
            const mode = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
                extendParam,
                0
            );
    ...
    public static resolveRadiansToSweepFraction(
        ...
        if (!sweep.isRadiansInSweep(radians)) {
            ...
            const mode0 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
                extendParam,
                0
            );
            const mode1 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
                extendParam,
                1
            );
            if (mode0 !== CurveExtendMode.None) {
                if (mode1 !== CurveExtendMode.None) {
                    ...
                    if (fraction > 1.0) fraction -= fractionPeriod;
                }
            } else if (mode1 !== CurveExtendMode.None) {
                if (fraction < 0.0) fraction += fractionPeriod;
            } else {
                // both clamped !!!!
