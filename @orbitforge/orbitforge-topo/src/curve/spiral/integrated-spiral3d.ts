...
import { NormalizedTransition } from "./normalized-transition";
import { TransitionConditionalProperties } from "./transition-conditional-properties";
...
export class IntegratedSpiral3d extends TransitionSpiral3d {
    ...
    private readonly _evaluator: NormalizedTransition;
    ...
    public static initWorkSpace() {
        ...
        IntegratedSpiral3d._gaussMapper = (xA, xB, xMapped, wMapped) =>
            Quadrature.setupGauss5(xA, xB, xMapped, wMapped);
    }
    ...
    public static createRadiusRadiusBearingBearing(
        ...
        if (typeName === undefined) typeName = "clothoid";
        ...
        return new IntegratedSpiral3d(
            ...
            new TransitionConditionalProperties(
                radius01.x0,
                radius01.x1,
                bearing01.startAngle.clone(),
                bearing01.endAngle.clone(),
                undefined
            )
        );
    }
    ...
    public setFrom(other: IntegratedSpiral3d): this {
    ...
    public override curveLengthBetweenFractions(fraction0: number, fraction1: number) {
        return (
            this._arcLength01 *
            (this._activeFractionInterval.absoluteDelta() * Math.abs(fraction1 - fraction0))
        );
    }
    ...
    public override isAlmostEqual(other?: GeometryQuery): boolean {
        if (other instanceof IntegratedSpiral3d) {
            return (
                this.radius01.isAlmostEqual(other.radius01) &&
