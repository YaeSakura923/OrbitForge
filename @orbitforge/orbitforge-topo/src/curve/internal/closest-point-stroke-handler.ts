...
export class ClosestPointStrokeHandler extends NewtonRtoRStrokeHandler implements IStrokeHandler {
    ...
    private readonly _spacePoint: Point3d;
    private readonly _extend: VariantCurveExtendParameter;
    ...
    private readonly _newtonSolver: Newton1dUnboundedApproximateDerivative;
    ...
    private announceCandidate(cp: CurvePrimitive, fraction: number, point: Point3d) {
        ...
        if (this._closestPoint && distance > this._closestPoint.a) return;
        this._closestPoint = CurveLocationDetail.createCurveFractionPoint(
            cp,
            fraction,
            point,
            this._closestPoint
    ...
    public announceSegmentInterval(
        ...
        if (!this._extend) localFraction = Geometry.clampToStartEnd(localFraction, 0.0, 1.0);
        else {
            if (fraction0 !== 0.0) localFraction = Math.max(localFraction, 0.0);
            if (fraction1 !== 1.0) localFraction = Math.min(localFraction, 1.0);
        }
    ...
    public evaluate(fraction: number): boolean {
        ...
        if (this._parentCurvePrimitive) curve = this._parentCurvePrimitive;
        if (curve) {
