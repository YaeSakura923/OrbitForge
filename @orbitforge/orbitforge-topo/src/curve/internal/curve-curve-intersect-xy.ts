...
import { CoincidentGeometryQuery } from "../../geometry3d/coincident-geometry-ops";
...
export class BezierBezierIntersectionXYRRToRRD extends NewtonEvaluatorRRtoRRD {
    private readonly _curveA: BezierCurveBase;
    private readonly _curveB: BezierCurveBase;
    private readonly _rayA: Ray3d;
    private readonly _rayB: Ray3d;
    ...
    public evaluate(fractionA: number, fractionB: number): boolean {
        ...
        this.currentF.setOriginAndVectorsXYZ(
            this._rayB.origin.x - this._rayA.origin.x,
            this._rayB.origin.y - this._rayA.origin.y,
            0.0,
            -this._rayA.direction.x,
            -this._rayA.direction.y,
            0.0,
            this._rayB.direction.x,
            this._rayB.direction.y,
            0.0
        );
        return true;
    }
...
export class CurveCurveIntersectXY extends RecurseToCurvesGeometryHandler {
    ...
    private readonly _worldToLocalPerspective: Matrix4d | undefined;
    private readonly _worldToLocalAffine: Transform | undefined;
    private readonly _coincidentGeometryContext: CoincidentGeometryQuery;
    private static readonly _workVector2dA = Vector2d.create();
    private static readonly _workPointA0H = Point4d.create();
    private static readonly _workPointA1H = Point4d.create();
    private static readonly _workPointB0H = Point4d.create();
    private static readonly _workPointB1H = Point4d.create();
    ...
    private static readonly _workPointA0 = Point3d.create();
    private static readonly _workPointA1 = Point3d.create();
    private static readonly _workPointB0 = Point3d.create();
    private static readonly _workPointB1 = Point3d.create();
    ...
    public constructor(
        ...
        if (worldToLocal !== undefined && !worldToLocal.isIdentity()) {
            ...
            if (!this._worldToLocalAffine) this._worldToLocalPerspective = worldToLocal.clone();
        }
    ...
    private acceptFraction(
        ...
        fraction: number,
        extend1: boolean,
        fractionTol: number = 1.0e-12
    ): boolean {
        if (!extend0 && fraction < -fractionTol) return false;
        if (!extend1 && fraction > 1.0 + fractionTol) return false;
        return true;
    }
    ...
    private acceptFractionOnLine(
        ...
        if (!extend0 && fraction < 0) {
            ...
            return Geometry.isDistanceWithinTol(
                (fraction - 1.0) * pointA.distanceXY(pointB),
                tolerance
            );
        }
        return true;
    ...
    private computeSegmentSegment3D(
        ...
        const overlap = this._coincidentGeometryContext.coincidentSegmentRangeXY(
            pointA0,
            pointA1,
            pointB0,
            pointB1,
        ...
        if (overlap) {
            ...
            SmallSystem.lineSegment3dXYTransverseIntersectionUnbounded(
                pointA0,
                pointA1,
                pointB0,
                pointB1,
            ...
            if (
                this.acceptFractionOnLine(
                    extendA0,
                    uv.x,
                    extendA1,
                    pointA0,
                    pointA1,
                    this._coincidentGeometryContext.tolerance
                ) &&
                this.acceptFractionOnLine(
                    extendB0,
                    uv.y,
    ...
    private computeSegmentSegment3DH(
        ...
        const fractionAB = SmallSystem.lineSegment3dHXYTransverseIntersectionUnbounded(
            hA0,
            hA1,
            hB0,
            hB1
        ...
        if (fractionAB !== undefined) {
            ...
            if (
                this.acceptFraction(extendA0, fractionA, extendA1) &&
    ...
    private dispatchSegmentSegment(
        ...
        if (this._worldToLocalAffine) {
            ...
            CurveCurveIntersectXY.setTransformedWorkPoints(
                this._worldToLocalAffine,
                pointA0,
                pointA1,
                pointB0,
                pointB1
    ...
    private dispatchSegmentArc(
        ...
        if (this._worldToLocalPerspective) {
            ...
            for (let i = 0; i < numRoots; i++) {
                ...
                const lineFraction = SmallSystem.lineSegment3dHXYClosestPointUnbounded(
                    pointA0H,
                    pointA1H,
                    arcPoint
                );
    ...
    private dispatchArcArc(
        ...
        if (!this._coincidentGeometryContext) {
            ...
            const pairs = this._coincidentGeometryContext.coincidentArcIntersectionXY(
                cpA,
                cpB,
                true
            );
            if (pairs !== undefined) this.recordPairs(cpA, cpB, pairs, reversed);
        }
    }
                ...
                for (let spanIndex = 0; ; spanIndex++) {
                    ...
                    if (roots) {
                        for (const root of roots) {
                            ...
                            const arcFraction = cpA.sweep.radiansToSignedPeriodicFraction(
                                Math.atan2(s, c)
                            );
                            if (
                                this.acceptFraction(extendA, arcFraction, extendA) &&
                                this.acceptFraction(extendB, fractionB, extendB)
                            ) {
                                this.recordPointWithLocalFractions(
                                    arcFraction,
    ...
    */
        ...
        for (let i = 1; i <= strokeCountA; i++, f0 = f1, this._xyzwA0.setFrom(this._xyzwA1)) {
            ...
            if (roots) {
                for (const r of roots) {
                    ...
                    if (
                        segmentAFraction &&
                        Geometry.isIn01WithTolerance(segmentAFraction, intervalTolerance)
                    ) {
                        let bezierAFraction = Geometry.interpolate(f0, segmentAFraction, f1);
                        const xyMatchingFunction = new BezierBezierIntersectionXYRRToRRD(
                            bezierA,
                            bezierB
                        );
                        ...
                        if (!xyzA0.isAlmostEqualXY(xyzA1)) errors++;
                        if (!xyzB0.isAlmostEqualXY(xyzB1)) errors++;
                        if (errors > 0 && !xyzA0.isAlmostEqual(xyzB0)) errors++;
                        if (errors > 0 && !xyzA1.isAlmostEqual(xyzB1)) errors++;
                        if (
                            this.acceptFraction(false, bcurveAFraction, false) &&
    ...
    private mapNPCPlaneToWorld(npcPlane: Point4d, worldPlane: Point4d) {
        ...
        // hence K = A^ * H
        if (this._worldToLocalAffine) {
            this._worldToLocalAffine.multiplyTransposeXYZW(
                npcPlane.x,
                npcPlane.y,
                npcPlane.z,
                npcPlane.w,
