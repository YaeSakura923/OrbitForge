...
import { type CurveLocationDetailPair, CurveCurveApproachType } from "../curve-location-detail";
...
import { RegionOps } from "../region-ops";
...
class Joint {
    ...
    public static constructArc(
        ...
        if (
            center !== undefined &&
            Geometry.isSameCoordinate(ray0.origin.distance(center), ray1.origin.distance(center))
        ) {
    ...
    public static link(joint0: Joint, joint1: Joint | undefined) {
        ...
        if (joint0.curve1 && joint1 && !joint1.curve0) joint1.curve0 = joint0.curve1;
        else if (!joint0.curve1 && joint1 && joint1.curve0) joint0.curve1 = joint1.curve0;
    }
        ...
        Joint.visitJointsOnChain(
            ...
            (joint: Joint) => {
                ...
                if (joint.curve1 && joint.fraction1 !== undefined) {
                    ...
                    else if (fA < fB) curve1 = joint.curve1.clonePartialCurve(fA, fB); // trimming is done by clonePartialCurve
                    if (curve1) {
                        if (!joint.jointCurve) {
        ...
        if (this.curve0 && this.curve1) {
            ...
            if (intersection.approachType === CurveCurveApproachType.Intersection) {
                if (intersection.detailA.fraction >= 0.0 && intersection.detailB.fraction <= 0.0) {
                    ...
                    if (numChamferPoints > 1) {
                        // create chamfer corner (a line string)
    ...
    public annotateJointMode(options: JointOptions): void {
        if (!this.curve0 && this.curve1) {
            // joint at the start of the chain
            this.flexure = JointMode.Cap;
            this.fraction1 = 0.0;
        } else if (this.curve0 && !this.curve1) {
            // joint at the end of the chain
            this.flexure = JointMode.Cap;
            this.fraction0 = 1.0;
        } else if (this.curve0 && this.curve1) {
            // joints at the middle of the chain
            if (this.curve0.endPoint().isAlmostEqual(this.curve1.startPoint())) {
                // joint between colinear segments
                this.fraction0 = 1.0;
                this.fraction1 = 0.0;
                ...
                if (intersection.approachType === CurveCurveApproachType.Intersection) {
                    ...
                    if (this.fraction0 >= 1.0 && this.fraction1 <= 0.0) {
                        ...
                        // need to fill gap with a single line segment
                        this.flexure = JointMode.Gap;
                        this.jointCurve = LineSegment3d.create(
                            this.curve0.fractionToPoint(1.0),
                            this.curve1.fractionToPoint(0.0)
        ...
        if (jointA) {
          ...
          */
                    ...
                    if (eliminateF && eliminateG) {
                        ...
                        if (newJoint.nextJoint) newJoint.nextJoint.annotateJointMode(options);
                        /*
            if (Checker.noisy.PolygonOffset) {
...
export class PolygonWireOffsetContext {
    /** Construct a context. */
    public constructor() {}
    private static readonly _unitAlong = Vector3d.create();
    private static readonly _unitPerp = Vector3d.create();
    private static readonly _offsetA = Point3d.create();
    private static readonly _offsetB = Point3d.create();
...
export class CurveChainWireOffsetContext {
    ...
    public static constructCurveXYOffset(
        ...
        if (joint0 && previousJoint && curves instanceof Loop) Joint.link(previousJoint, joint0);
        /** annotateChain sets some of the joints attributes (including how to extend curves or fill the gap between curves) */
