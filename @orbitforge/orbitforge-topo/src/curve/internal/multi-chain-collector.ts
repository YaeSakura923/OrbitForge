...
import { BagOfCurves, CurveCollection } from "../curve-collection";
...
import { type ChainTypes, RegionOps } from "../region-ops";
...
export class MultiChainCollector {
     ...
     */
    public constructor(
        gapTolerance = Geometry.smallMetricDistance,
    ...
    public captureCurvePrimitive(candidate: CurvePrimitive) {
        if (this._snapTolerance < this._gapTolerance) {
            if (this.attachPrimitiveToAnyChain(candidate, this._snapTolerance)) return;
        }
        if (this.attachPrimitiveToAnyChain(candidate, this._gapTolerance)) return;
    ...
    public captureCurve(candidate: AnyCurve) {
        if (candidate instanceof CurvePrimitive) this.captureCurvePrimitive(candidate);
            ...
            for (const c of candidate.children) this.captureCurve(c);
        }
    }
    ...
    private static moveHeadOrTail(
        curve0: CurvePrimitive,
        curve1: CurvePrimitive,
        gapTolerance: number
    ): boolean {
        ...
        if (d01 < minShift) return false;
        // try lines and linestrings
        if (d01 < gapTolerance) {
            if (
                this.simpleEndPointMove(curve1, false, xyz0) ||
                this.simpleEndPointMove(curve0, true, xyz1)
            ) {
                return true;
            }
        }
        ...
        if (candidate) {
            ...
            if (connect) {
                if (connect.atEnd) {
                    ...
                    MultiChainCollector.moveHeadOrTail(
                        chain[index0],
                        chain[index0 + 1],
                        this._gapTolerance
                    );
            ...
            if (
                ...
                if (localToWorld) {
                    ...
                    if (worldToLocal) {
                        ...
                        if (
                            this._planeTolerance !== undefined &&
                            range.zLength() <= this._planeTolerance
                        ) {
    ...
    private chainToLineString3d(
        chain: CurvePrimitive[],
        options?: StrokeOptions
    ): LineString3d | undefined {
        if (chain.length === 0) return undefined;
        const linestring = LineString3d.create();
        for (const curve of chain) curve.emitStrokes(linestring, options);
