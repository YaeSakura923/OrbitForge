...
import {
    type HalfEdge,
    type NodeToNumberFunction,
    HalfEdgeGraph,
    HalfEdgeMask
} from "../topology/graph";
import { HalfEdgeGraphSearch } from "../topology/half-edge-graph-search";
import { HalfEdgeGraphMerge } from "../topology/merging";
import { RegularizationContext } from "../topology/regularize-face";
...
import { type GraphCheckPointFunction, RegionBinaryOpType, RegionOps } from "./region-ops";
...
class RegionOpsBinaryBooleanSweepCallbacks extends RegionOpsFaceToFaceSearchCallbacks {
    private readonly _faceSelectFunction: BinaryBooleanAcceptFunction;
...
export class RegionBooleanContext implements RegionOpsFaceToFaceSearchCallbacks {
    ...
    private static readonly _bridgeDirection = Vector3d.createNormalized(1.0, -0.12328974132467)!; // magic unit direction to minimize vertex hits
    /**
    ...
    public addConnectives() {
        ...
        if (this._workSegment) {
            margin *= this._workSegment.point0Ref.distanceXY(this._workSegment.point1Ref);
        ...
        for (const p of maxPoints) {
            ...
            const bridgeLength =
                margin +
                Ray3d.create(p, RegionBooleanContext._bridgeDirection, ray).intersectionWithRange3d(
                    rangeAB
                ).high;
...
export class GraphComponent {
    ...
    public buildFaceData(
        ...
        if (!faceAreaFunction) faceAreaFunction = node => HalfEdgeGraphSearch.signedFaceArea(node);
        for (const f of this.faces) {
...
export class GraphComponentArray {
    ...
    public static create(
        ...
        const componentArrays =
            HalfEdgeGraphSearch.collectConnectedComponentsWithExteriorParityMasks(graph, undefined);
