...
import { MultiChainCollector } from "./internal/multi-chain-collector";
import { CurveChainWireOffsetContext } from "./internal/polygon-offset-context";
...
export class CurveOps {
    ...
    public static sumLengths(curves: AnyCurve | AnyCurve[]): number {
        ...
        if (curves instanceof CurvePrimitive) {
            ...
            for (const data1 of curves) mySum += this.sumLengths(data1);
        }
        return mySum;
    }
    ...
    public static extendRange(range: Range3d, curves: AnyCurve | AnyCurve[]): Range3d {
        if (Array.isArray(curves)) {
            for (const data1 of curves) this.extendRange(range, data1);
        } else {
     ...
     */
    public static appendXYOffsets(
        curves: AnyCurve | AnyCurve[] | undefined,
        offset: number,
        result: AnyCurve[]
    ): number {
        let summedLengths = 0;
        if (curves instanceof CurvePrimitive) {
            ...
            for (const q of curves) summedLengths += this.appendXYOffsets(q, offset, result);
        }
    ...
    public static createSingleOffsetPrimitiveXY(
        ...
        return CurveChainWireOffsetContext.createSingleOffsetPrimitiveXY(
            curve,
