...
} from "../geometry3d/indexed-xyz-collection";
...
import { SortablePolygon } from "../geometry3d/sortable-polygon";
...
import {
    type LineStringDataVariant,
    type MultiLineStringDataVariant,
    Triangulator
} from "../topology/triangulation";
import { BagOfCurves, CurveChain, CurveCollection } from "./curve-collection";
import { CurveCurve } from "./curve-curve";
import { CurveOps } from "./curve-ops";
import { CurvePrimitive } from "./curve-primitive";
import { type AnyCurve, type AnyRegion } from "./curve-types";
import { CurveWireMomentsXYZ } from "./curve-wire-moments-xyz";
import { GeometryQuery } from "./geometry-query";
import { ChainCollectorContext } from "./internal/chain-collector-context";
import { PolygonWireOffsetContext } from "./internal/polygon-offset-context";
...
import { ConsolidateAdjacentCurvePrimitivesContext } from "./query/consolidate-adjacent-primitives-context";
import { CurveSplitContext } from "./query/curve-split-context";
import { PointInOnOutContext } from "./query/in-out-tests";
import { PlanarSubdivision } from "./query/planar-subdivision";
import { RegionMomentsXY } from "./region-moments-xy";
import {
    RegionBooleanContext,
    RegionGroupOpType,
    RegionOpsFaceToFaceSearch
} from "./region-ops-classification-sweeps";
import { type StrokeOptions } from "./stroke-options";
import { UnionRegion } from "./union-region";
...
export class RegionOps {
    ...
    public static splitPathsByRegionInOnOutXY(
        ...
        if (splitPaths instanceof CurveCollection) {
            ...
            const pointOnChild =
                CurveCollection.createCurveLocationDetailOnAnyCurvePrimitive(splitPaths);
            if (pointOnChild) {
                const inOnOut = RegionOps.testPointInOnOutRegionXY(
                    region,
                    pointOnChild.point.x,
    ...
    public static sortOuterAndHoleLoopsXY(loops: Array<Loop | IndexedXYZCollection>): AnyRegion {
        ...
        for (const candidate of loops) {
            if (candidate instanceof Loop) SortablePolygon.pushLoop(loopAndArea, candidate);
    ...
    public static constructAllXYRegionLoops(
        ...
        const primitives = RegionOps.collectCurvePrimitives(
            curvesAndRegions,
            undefined,
            true,
            true
    ...
    public static curveArrayRange(data: any, worldToLocal?: Transform): Range3d {
        ...
        if (data instanceof GeometryQuery) data.extendRange(range, worldToLocal);
            ...
            for (const c of data) {
                ...
                else if (c instanceof GrowableXYZArray) range.extendRange(c.getRange(worldToLocal));
                else if (Array.isArray(c)) range.extendRange(this.curveArrayRange(c, worldToLocal));
            }
        }
        return range;
    }
    ...
    private static triangulateStrokedRegionComponent(
        polygons: MultiLineStringDataVariant
        ...
        if (Array.isArray(polygons)) {
            ...
            if (Point3d.isAnyImmediatePointType(firstEntry)) {
                graph = Triangulator.createTriangulatedGraphFromSingleLoop(
                    polygons as XYAndZ[] | XAndY[] | number[][]
                );
                ...
                if (sortedPolygons.length === 1) {
                    // below requires exactly one outer loop!
                    if (
                        (graph = Triangulator.createTriangulatedGraphFromLoops(sortedPolygons[0]))
                    ) {
                        Triangulator.flipTriangles(graph);
                    }
                }
        ...
        if (!graph) {
            ...
            if (
                (graph = RegionOpsFaceToFaceSearch.doPolygonBoolean(
                    polygons,
                    [],
                    (inA, _inB) => inA
                ))
    ...
    private static strokeRegionComponent(
        ...
        if (strokedComponent instanceof Loop) {
            if (
                strokedComponent.children.length > 0 &&
                strokedComponent.children[0] instanceof LineString3d
            ) {
                polygons.push(strokedComponent.children[0].packedPoints);
            } // expect only 1
        } else if (strokedComponent instanceof ParityRegion) {
            for (const strokedLoop of strokedComponent.children) {
                if (
                    strokedLoop.children.length > 0 &&
                    strokedLoop.children[0] instanceof LineString3d
                ) {
                    polygons.push(strokedLoop.children[0].packedPoints);
                } // expect only 1
            }
        }
        return polygons;
    ...
    public static facetRegionXY(
        ...
        if (!graph) return undefined;
        if (options?.maximizeConvexFacets) HalfEdgeGraphOps.expandConvexFaces(graph);
     ...
     */
    public static convexDecomposePolygonXY(
        polygon: MultiLineStringDataVariant,
        maximize: boolean = true
    ): GrowableXYZArray[] | undefined {
        const graph = RegionOps.triangulateStrokedRegionComponent(polygon);
        if (!graph) return undefined;
        if (maximize) HalfEdgeGraphOps.expandConvexFaces(graph);
            ...
            if (!seed.isMaskSet(HalfEdgeMask.EXTERIOR)) {
                convexPolygons.push(
                    GrowableXYZArray.create(
                        seed.collectAroundFace(node => {
                            return node.getPoint3d();
                        })
                    )
                );
            }
