...
import { type PlaneAltitudeEvaluator, AxisIndex, AxisOrder, Geometry } from "../geometry";
...
import { type ArcBlendData, Arc3d } from "./arc3d";
...
import { IntegratedSpiral3d } from "./spiral/integrated-spiral3d";
...
export class CurveFactory {
    ...
    public static createFilletsInLineString(
        ...
        for (let i = 1; i + 1 < n; i++) {
            ...
            } else blendArray.push({ fraction10: 0.0, fraction12: 0.0, point: pointB.clone() });
    ...
    public static createMiteredSweepSections(
        ...
        const planes = PolylineOps.createBisectorPlanesForDistinctPoints(
            centerline,
            options.wrapIfPhysicallyClosed
        );
        if (planes !== undefined && planes.length > 1) {
            ...
            const doSweepToPlane = function (
                ...
                const sweepVector = Vector3d.createStartEnd(
                    edgePlane0.getOriginRef(),
                    edgePlane1.getOriginRef()
                );
                const transform = Transform.createFlattenAlongVectorToPlane(
                    sweepVector,
                    targetPlane.getOriginRef(),
                    targetPlane.getNormalRef()
                );
                if (transform === undefined) return section;
                const section1 = section.cloneTransformed(transform);
                if (section1 === undefined) return section;
