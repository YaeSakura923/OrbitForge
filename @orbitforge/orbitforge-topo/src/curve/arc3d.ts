...
import {
    type VariantCurveExtendParameter,
    CurveExtendMode,
    CurveExtendOptions
} from "./curve-extend-mode";
...
import { CurveOffsetXYHandler } from "./internal/curve-offset-xy-handler";
...
export class Arc3d extends CurvePrimitive implements BeJSONFunctions {
    ...
    private static readonly _workVectorU = Vector3d.create();
    private static readonly _workVectorV = Vector3d.create();
    ...
    public override closestPoint(
        ...
        let extend0 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
            extend,
            0
        );
        let extend1 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
            extend,
            1
        );
    ...
    public get isCircular(): boolean {
        ...
        return (
            Angle.isPerpendicularDotSet(axx, ayy, axy) && Geometry.isSameCoordinateSquared(axx, ayy)
        );
    }
    ...
    public circularRadiusXY(): number | undefined {
        ...
        if (
            Angle.isPerpendicularDotSet(dotUU, dotVV, dotUV) &&
            Geometry.isSameCoordinateSquared(dotUU, dotVV)
        ) {
        ...
        if (trigPoints !== undefined) {
            ...
            for (xy of trigPoints) {
                ...
                if (Angle.isAlmostEqualRadiansAllowPeriodShift(radians, this._sweep.startRadians)) {
                    ...
                    Angle.isAlmostEqualRadiansAllowPeriodShift(radians, this._sweep.endRadians)
                ) {
                    detail.intervalRole = CurveIntervalRole.isolatedAtVertex;
                }
                result.push(detail);
