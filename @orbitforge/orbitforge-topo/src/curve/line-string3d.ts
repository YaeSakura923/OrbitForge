...
} from "../geometry3d/point-streaming";
...
import { type VariantCurveExtendParameter, CurveExtendOptions } from "./curve-extend-mode";
import { CurveIntervalRole, CurveLocationDetail, CurveSearchStatus } from "./curve-location-detail";
...
import { PlaneAltitudeRangeContext } from "./internal/plane-altitude-range-context";
...
function accumulateGoodUnitPerpendicular(
    ...
    if (stepDirection > 0) {
        ...
        if (baseIndex + 1 >= n) baseIndex = n - 2;
...
export class LineString3d extends CurvePrimitive implements BeJSONFunctions {
    ...
    private static readonly _workPointA = Point3d.create();
    private static readonly _workPointB = Point3d.create();
    private static readonly _workPointC = Point3d.create();
    private static readonly _workRay = Ray3d.createXAxis();
    ...
    private constructor(points?: GrowableXYZArray) {
        ...
        else this._points = new GrowableXYZArray();
    }

    /** Clone this linestring and apply the transform to the clone points. */
    public cloneTransformed(transform: Transform): LineString3d {
    ...
    public addFraction(fraction: number) {
        if (!this._fractions) this._fractions = new GrowableFloat64Array();
    ...
    public addUVParam(uvParam: XAndY) {
        if (!this._uvParams) this._uvParams = new GrowableXYArray();
    ...
    public addDerivative(vector: Vector3d) {
        if (!this._derivatives) this._derivatives = new GrowableXYZArray();
    ...
    public addSurfaceNormal(vector: Vector3d) {
        if (!this._surfaceNormals) this._surfaceNormals = new GrowableXYZArray();
    ...
    public static createRegularPolygonXY(
        ...
        let s;
        let radians;
        if (!radiusToVertices) radius = radius / Math.cos(radiansStep);
    ...
    public static createPoints(points: Point3d[]): LineString3d {
        ...
        let point;
        for (point of points) ls._points.push(point);
        return ls;
    }
     ...
     */
    public globalFractionToSegmentIndexAndLocalFraction(globalFraction: number): {
        index: number;
        fraction: number;
    } {
        const numSegment = this._points.length - 1;
        if (numSegment < 1) return { index: 0, fraction: 0.0 };

        const scaledGlobalFraction = globalFraction * numSegment;
        let segmentIndex: number;
        if (globalFraction < 0) segmentIndex = 0;
        else if (globalFraction > 1) segmentIndex = numSegment - 1;
        // globalFraction in [0,1]
        else segmentIndex = Math.floor(scaledGlobalFraction);
    ...
    public reverseInPlace(): void {
        if (this._points.length >= 2) {
            ...
            if (this._uvParams) this._uvParams.reverseInPlace();
        }
    }
    ...
    public override rangeBetweenFractions(
        ...
        if (fraction1 === fraction0) return range; // 1-point range
        const scaledFraction1 = fraction1 * numSegments;
        const index1 = Math.min(Math.floor(scaledFraction1), numSegments - 1);
        const localFraction1 = scaledFraction1 - index1;
    ...
    public override moveSignedDistanceFromFraction(
        ...
        if (signedDistance > 0.0) {
            ...
            return CurveLocationDetail.createCurveFractionPointDistanceCurveSearchStatus(
                ...
                context.point0,
                signedDistance,
                context.distanceStatus(),
                result
            );
        } else {
            // (moving backwards)
            if (localFraction <= 0.0) leftPointIndex--;
    ...
    public quickUnitNormal(result?: Vector3d): Vector3d | undefined {
        ...
        if (result && result.normalizeInPlace()) return result;
        return undefined;
    }
    ...
    public override closestPoint(
        ...
        const extend0 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
            extend,
            0
        );
        const extend1 = CurveExtendOptions.resolveVariantCurveExtendParameterToCurveExtendMode(
            extend,
            1
        );
            ...
            if (numPoints > 1) {
                ...
                for (let i = 1; i < numPoints; i++) {
                    ...
                    if (segmentFraction < 0) {
                        if (!extend0 || i > 1) segmentFraction = 0.0;
                    } else if (segmentFraction > 1.0) {
                        if (!extend1 || i < lastIndex) segmentFraction = 1.0;
                    }
                    this._points
    ...
    public appendStrokePoint(point: Point3d, fraction?: number): void {
        ...
        if (add) {
            this._points.push(point);
            if (addFraction) this.addFraction(fraction);
        }
    }
    ...
    public removeDuplicatePoints(tolerance: number = Geometry.smallMetricDistance) {
        ...
        if (this._fractions) this._fractions.resize(n1);
        if (this._derivatives) this._derivatives.resize(n1);
    }
    ...
    public clear() {
        this._points.clear();
        if (this._fractions) this._fractions.clear();
        if (this._derivatives) this._derivatives.clear();
    }
     ...
     */
    public appendInterpolatedStrokePoints(
        numStrokes: number,
        point0: Point3d,
        point1: Point3d,
        include01: boolean
    ): void {
        if (include01) this.appendStrokePoint(point0, 0.0);
        if (numStrokes > 1) {
        ...
        if (include01) this.appendStrokePoint(point1, 1.0);
    }
            ...
            if (options && options.hasMaxEdgeLength) {
                for (let i = 1; i < n; i++) {
                    const numStroke = options.applyMaxEdgeLength(
                        ...
                        this._points
                            .getPoint3dAtUncheckedPointIndex(i - 1)
                            .distance(this._points.getPoint3dAtUncheckedPointIndex(i))
                    );
    ...
    private static readonly _indexPoint = Point3d.create(); // private point for addResolvedPoint
    ...
    public override clonePartialCurve(fractionA: number, fractionB: number): LineString3d {
        ...
        const n = this._points.length;
        if (n < 2) return this.clone();
        if (n > 2 && this.isPhysicallyClosed) {
            // don't extend a closed linestring
            if (fractionA < 0) fractionA = 0;
            if (fractionB > 1) fractionB = 1;
        }
        ...
        } else if (fractionA >= 0 && fractionA <= 1) {
        ...
        } else if (fractionB >= 0 && fractionB <= 1) {
    ...
    public getIndexedSegment(index: number): LineSegment3d | undefined {
        if (index >= 0 && index + 1 < this._points.length) {
            return LineSegment3d.create(
                this._points.getPoint3dAtCheckedPointIndex(index)!,
                this._points.getPoint3dAtCheckedPointIndex(index + 1)!
            );
        }
        return undefined;
    }
        ...
        if (
            ...
            for (let k = 0; k + 1 < numParentPoint; k++, pointA.setFromPoint3d(pointB)) {
                ...
                for (let i = 0; i <= m; i++) {
                    ...
                    if (needFractions) destLinestring._fractions!.push(outputFraction);
                    if (needDerivatives) destLinestring._derivatives!.push(vectorAB);
                }
            }
        }
...
class MoveByDistanceContext {
    ...
    public distanceStatus(): CurveSearchStatus {
        return Geometry.isSameCoordinate(this.distance0, this.targetDistance)
            ? CurveSearchStatus.success
            : CurveSearchStatus.stoppedAtBoundary;
    }
    ...
    public announceExtrapolation(
        ...
        if (extensionFraction === undefined) return false;
