...
export class CurveLocationDetail {
    ...
    public collapseToEnd() {
        if (this.fraction1 !== undefined) this.fraction = this.fraction1;
        if (this.point1) this.point = this.point1;
    ...
    public setFR(fraction: number, ray: Ray3d, a: number = 0): void {
        this.setFP(fraction, ray.origin, ray.direction, a);
    }
    ...
    public isSameCurveAndFraction(
        other: CurveLocationDetail | { curve: CurvePrimitive; fraction: number }
    ): boolean {
        return (
            this.curve === other.curve &&
            Geometry.isAlmostEqualNumber(this.fraction, other.fraction)
        );
    }
}
...
export class CurveLocationDetailPair {
     ...
     * @internal
     */
    public static removeAdjacentDuplicates(
