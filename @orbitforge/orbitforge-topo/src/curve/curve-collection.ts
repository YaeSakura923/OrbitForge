...
import { CloneCurvesContext } from "./internal/clone-curves-context";
import { CloneWithExpandedLineStrings } from "./internal/clone-with-expanded-line-strings";
import { CountLinearPartsSearchContext } from "./internal/count-linear-parts-search-context";
import { GapSearchContext } from "./internal/gap-search-context";
import { PlaneAltitudeRangeContext } from "./internal/plane-altitude-range-context";
import { SumLengthsContext } from "./internal/sum-lengths-context";
import { TransformInPlaceContext } from "./internal/transform-in-place-context";
import { LineString3d } from "./line-string3d";
import type { Loop } from "./loop";
import type { Path } from "./path";
import { ProxyCurve } from "./proxy-curve";
...
export abstract class CurveCollection extends GeometryQuery {
    ...
    public get isAnyRegionType(): boolean {
        return (
            this.topoBoundaryType() === 2 ||
            this.topoBoundaryType() === 4 ||
            this.topoBoundaryType() === 5
        );
    }
    ...
    public get isOpenPath(): boolean {
        return this.topoBoundaryType() === 1;
    }
    ...
    public get isClosedPath(): boolean {
        return this.topoBoundaryType() === 2;
    }
     ...
     */
    public abstract topoBoundaryType(): number;
    /**
     * Try to add a child.
...
export abstract class CurveChain extends CurveCollection {
    ...
    public getPackedStrokes(options?: StrokeOptions): GrowableXYZArray | undefined {
        ...
        if (tree instanceof CurveChain) {
            ...
            if (children.length === 1) {
                const ls = children[0];
                if (ls instanceof LineString3d) return ls.packedPoints;
            }
        }
        return undefined;
    }
    ...
    public override extendRange(range: Range3d, transform?: Transform): void {
        for (const curve of this._curves) curve.extendRange(range, transform);
    }
    ...
    public reverseChildrenInPlace(): void {
        for (const curve of this._curves) curve.reverseInPlace();
        ...
        if (alsoSearchProxies ?? false) {
            for (let i = 0; i < this._curves.length; i++) {
                ...
                if (childCurve instanceof ProxyCurve) {
                    if (childCurve.proxyCurve === target) return i;
                }
            }
        }
        return undefined;
...
export class BagOfCurves extends CurveCollection {
    ...
    public topoBoundaryType(): number {
        return 0;
    }
    ...
    public announceToCurveProcessor(
        ...
        processor.announceBagOfCurves(this, indexInParent);
    }
