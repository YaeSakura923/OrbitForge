...
import { type SignedLoops, Loop, LoopCurveLoopCurve } from "../loop";
...
class MapCurvePrimitiveToCurveLocationDetailPairArray {
    ...
    public splitAndAppendMissingClosedPrimitives(
        ...
        for (const p of primitives) {
            ...
            if (p instanceof Arc3d) closedCurveSplitCandidate = p.sweep.isFullCircle;
...
export class PlanarSubdivision {
    ...
    public static assembleHalfEdgeGraph(
        ...
        for (const pair of allPairs) detailByPrimitive.insertPair(pair);
        ...
        for (const entry of detailByPrimitive.primitiveToPair.entries()) {
            ...
            const details = entry[1].reduce(
                (accumulator: CurveLocationDetailPair[], detailPair) => {
                    if (!detailPair.detailA.hasFraction1) return [...accumulator, detailPair];
                    const detail = getDetailOnCurve(detailPair, p)!;
                    const detail0 = CurveLocationDetail.createCurveFractionPoint(
                        p,
                        detail.fraction,
                        detail.point
                    );
                    const detail1 = CurveLocationDetail.createCurveFractionPoint(
                        p,
                        detail.fraction1!,
                        detail.point1!
                    );
    ...
    private static isNullFace(he: HalfEdge): boolean {
        ...
        if (faceHasTwoEdges) {
            ...
            if (!Geometry.isSameCoordinate(c0, c1)) {
                // default tol!
                faceIsBanana = true;
            } // heuristic: we could also check end curvatures, and/or higher derivatives...
        }
    ...
    public static collectSignedLoopSetsInHalfEdgeGraph(
        ...
        const q = HalfEdgeGraphSearch.collectConnectedComponentsWithExteriorParityMasks(
            graph,
            undefined
        );
