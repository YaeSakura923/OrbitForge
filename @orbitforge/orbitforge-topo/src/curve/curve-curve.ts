...
import { CurveCurveCloseApproachXY } from "./internal/curve-curve-close-approach-xy";
import { CurveCurveIntersectXY } from "./internal/curve-curve-intersect-xy";
import { CurveCurveIntersectXYZ } from "./internal/curve-curve-intersect-xyz";
...
export class CurveCurve {
    ...
    public static intersectionXYPairs(
        ...
        return CurveCurve.intersectionProjectedXYPairs(
            undefined,
            curveA,
            extendA,
            curveB,
    ...
    public static closestApproachProjectedXYPair(
        ...
        if (!closeApproaches.length) return undefined;
        let iMin = 0;
        let minDistXY = 2 * maxDistance;
        for (let i = 0; i < closeApproaches.length; ++i) {
            const distXY = closeApproaches[i].detailA.point.distanceXY(
                closeApproaches[i].detailB.point
            );
            if (distXY < minDistXY) {
                iMin = i;
                minDistXY = distXY;
