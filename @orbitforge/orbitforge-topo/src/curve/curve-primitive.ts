...
import { type PlaneAltitudeEvaluator, AxisOrder, Geometry } from "../geometry";
...
import type { CurveChainWithDistanceIndex } from "./curve-chain-with-distance-index";
...
import { AppendPlaneIntersectionStrokeHandler } from "./internal/append-plane-intersection-stroke-handler";
import { ClosestPointStrokeHandler } from "./internal/closest-point-stroke-handler";
import { CurveLengthContext } from "./internal/curve-length-context";
...
import type { DirectSpiral3d } from "./spiral/direct-spiral3d";
import type { IntegratedSpiral3d } from "./spiral/integrated-spiral3d";
 ...
 * @public
 */
export type CurvePrimitiveType =
    | "arc"
    | "lineSegment"
    | "lineString"
    | "bsplineCurve"
 ...
 * @public
 */
export type AnnounceNumberNumberCurvePrimitive = (
    a0: number,
    a1: number,
    cp: CurvePrimitive
) => void;
...
export abstract class CurvePrimitive extends GeometryQuery {
    ...
    public fractionToFrenetFrame(fraction: number, result?: Transform): Transform | undefined {
        ...
        if (axes) return Transform.createRefs(plane.origin, axes, result);
        ...
        const perpVector = Matrix3d.createPerpendicularVectorFavorXYPlane(
            plane.vectorU,
            plane.vectorV
        );
    ...
    public curveLengthBetweenFractions(fraction0: number, fraction1: number): number {
        if (fraction0 === fraction1) return 0.0;
     ...
     * * Default implementation returns the range of the curve from clonePartialCurve
     */
    public rangeBetweenFractionsByClone(
        fraction0: number,
        fraction1: number,
        transform?: Transform
    ): Range3d {
        if (fraction0 === fraction1) return Range3d.create(this.fractionToPoint(fraction0));
        const fragment = this.clonePartialCurve(fraction0, fraction1);
        if (fragment) return fragment.range(transform);
    ...
    public rangeBetweenFractionsByCount(
        ...
        count: number,
        transform?: Transform,
        extrapolationFactor: number = 0.0
    ): Range3d {
        ...
        const evaluateSteps = (fractionA: number, fractionStep: number, countA: number) => {
            ...
            for (let i = 0; i < countA; i++, f += fractionStep) {
                ...
                if (transform) range.extendTransformedPoint(transform, workPoint);
                else range.extendPoint(workPoint);
            }
        };
    ...
    public moveSignedDistanceFromFraction(
        ...
        return this.moveSignedDistanceFromFractionGeneric(
            startFraction,
            signedDistance,
            allowExtension,
            result
    ...
    protected moveSignedDistanceFromFractionGeneric(
        ...
        if (signedDistance === 0.0) {
            return CurveLocationDetail.createCurveEvaluatedFraction(this, startFraction, result);
        } // no movement, just evaluate at startFraction
        if (signedDistance > 0.0) {
            limitFraction = 1.0;
        ...
        for (let iterations = 0; iterations < 10; iterations++) {
            ...
            if (fractionA === fractionB) {
                // YES -- that is an exact equality test.   When it happens, there's no need for confirming with another iteration.
                numConverged = 100;
                break;
            }
     ...
     */
    public appendPlaneIntersectionPoints(
        ...
        for (let candidateIndex = 0; candidateIndex < n0; candidateIndex++) {
            ...
            if (startEndFractionTolerance > 0) {
                ...
                if (Math.abs(fraction - 1.0) < startEndFractionTolerance) {
                    ...
                    if (detail.curve) snappedCoordinates = detail.curve.startPoint(point1);
                }
            }
            ...
            if (accept) {
                if (applySnappedCoordinates) {
                    ...
                    if (snappedCoordinates !== undefined) detail.point.setFrom(snappedCoordinates);
                }
                if (acceptIndex < candidateIndex) details[acceptIndex] = detail;
                acceptIndex++;
            }
        }
        if (acceptIndex < n0) details.length = acceptIndex;
    }
     ...
     */
    public computeAndAttachRecursiveStrokeCounts(
     ...
     * @param parentMap
     */
    public static installStrokeCountMap(
        curve: CurvePrimitive,
        curveMap: StrokeCountMap,
        parentMap?: StrokeCountMap
    ) {
        if (parentMap) parentMap.addToCountAndLength(curveMap.numStroke, curveMap.curveLength);
    ...
    public collectCurvePrimitivesGo(
        ...
        _explodeLinestrings: boolean = false
    ): void {
