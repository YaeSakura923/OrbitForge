...
} from "../bspline/surface-location-detail";
...
import { type AxisIndex, AxisOrder, Geometry } from "../geometry";
import { Point4d } from "../geometry4d/point4d";
import { Order3Bezier } from "../numerics/bezier-polynomials";
...
import { TriDiagonalSystem } from "../numerics/tri-diagonal-system";
...
class EllipsoidComponentExtrema {
    ...
    private readonly _axisRange: Range1d;
    private readonly _trigForm: SineCosinePolynomial;
    ...
    public static createTransformRow(
        ...
        return new EllipsoidComponentExtrema(
            ...
            matrix.at(axisIndex, 0),
            matrix.at(axisIndex, 1),
            matrix.at(axisIndex, 2),
            axisIndex
        );
    }

    public extendRangeForSmoothSurfacePoints(
        range: Range3d,
     ...
     */
    public extendRangeForConstantThetaArc(
        range: Range3d,
        thetaRadians: number,
        phi0Radians: number,
        phi1Radians: number
     ...
     */
    public extendRangeForConstantPhiArc(
        range: Range3d,
        theta0Radians: number,
        theta1Radians: number,
        phiRadians: number
...
export class Ellipsoid implements Clipper {
    ...
    private readonly _unitVectorA: Vector3d;
    private readonly _unitVectorB: Vector3d;
    private readonly _workPointA: Point3d;
    private readonly _workPointB: Point3d;
     ...
     */
    public static createCenterMatrixRadii(
        center: Point3d,
        axes: Matrix3d | undefined,
        radiusX: number,
        radiusY: number,
        radiusZ: number
    ): Ellipsoid {
    ...
    public silhouetteArc(eyePoint: Point4d): Arc3d | undefined {
        ...
        if (localEyePoint !== undefined) {
            ...
            if (localPlaneB) {
                ...
                if (rr > 0.0 && rr <= 1.0) {
                    const arc = Arc3d.createCenterNormalRadius(
                        localPlaneB.getOriginRef(),
                        localPlaneB.getNormalRef(),
                        Math.sqrt(rr)
                    );
                    if (arc.tryTransformInPlace(this._transform)) return arc;
                }
            }
        }
        return undefined;
    ...
    public intersectRay(
        ...
        if (rayFractions) rayFractions.length = 0;
        ...
        if (localRay !== undefined) {
            const n = SphereImplicit.intersectSphereRay(
            ...
            if (rayFractions !== undefined) {
                for (let i = 0; i < rayFractions.length; i++) rayFractions[i] *= scale;
            }
            if (xyz !== undefined) {
    ...
    public patchRangeStartEndRadians(
        theta0Radians: number,
        theta1Radians: number,
        phi0Radians: number,
        phi1Radians: number,
    ...
    public radiansPairToGreatArc(
        thetaARadians: number,
        phiARadians: number,
        thetaBRadians: number,
        phiBRadians: number,
        result?: Arc3d
    ): Arc3d | undefined {
     ...
     */
    public createSectionArcPointPointVectorInPlane(
        pointAnglesA: LongitudeLatitudeNumber,
        pointAnglesB: LongitudeLatitudeNumber,
        ...
        if (localA !== undefined && localB !== undefined && localInPlaneVector !== undefined) {
            const localPlane = Plane3dByOriginAndUnitNormal.createPointPointVectorInPlane(
                localA,
                localB,
                localInPlaneVector
            );
     ...
     */
    public radiansToPointAndDerivatives(
        thetaRadians: number,
        phiRadians: number,
        applyCosPhiFactor = true,
     ...
     * @param result optional plane result
     */
    public radiansToPointAnd2Derivatives(
        thetaRadians: number,
        phiRadians: number,
        point: Point3d,
        d1Theta: Vector3d,
     ...
     */
    public static radiansToUnitNormalRay(
    ...
    public announceClippedSegmentIntervals(
        ...
        if (localA && localB) {
            ...
            if (roots !== undefined && roots.length === 2) {
                ...
                if (f0 < f1) {
                    if (roots[0] < f0) roots[0] = f0;
                    if (f1 < roots[1]) roots[1] = f1;
                    if (roots[0] < roots[1]) {
                        if (announce) announce(roots[0], roots[1]);
                        return true;
                    }
                } else {
                    // f0,f1 are reversed. do the outputs in the same sense
                    if (roots[1] > f0) roots[1] = f0;
                    if (roots[0] < f1) roots[0] = f1;
                    if (roots[1] > roots[0]) {
                        if (announce) announce(roots[1], roots[0]);
                        return true;
                    }
                }
            }
        ...
        if (
            ...
            if (
                ...
                for (const radians of intersectionRadians) {
                    ...
                    if (Geometry.isIn01(fraction)) fractions.push(fraction);
                }
                fractions.sort();
                let f0, f1;
...
export class EllipsoidPatch implements UVSurface {
    ...
    public range(result?: Range3d): Range3d {
        return this.ellipsoid.patchRangeStartEndRadians(
            this.longitudeSweep.startRadians,
            this.longitudeSweep.endRadians,
            this.latitudeSweep.startRadians,
            this.latitudeSweep.endRadians,
            result
        );
    }
    ...
    public intersectRay(
        ...
        convertIntersectionRadiansToFractions: boolean = false
    ): CurveAndSurfaceLocationDetail[] {
    ...
    public uvFractionToAngles(
        ...
        return LongitudeLatitudeNumber.createRadians(
            this.longitudeSweep.fractionToRadians(longitudeFraction),
            this.latitudeSweep.fractionToRadians(phiFraction),
            h,
            result
        );
    }
...
class EllipsoidClosestPoint extends NewtonEvaluatorRRtoRRD {
    ...
    private readonly _surfacePoint: Point3d;
    private readonly _d1Theta: Vector3d;
    private readonly _d2Theta: Vector3d;
    private readonly _d1Phi: Vector3d;
    private readonly _d2Phi: Vector3d;
    private readonly _d2ThetaPhi: Vector3d;
    private readonly _delta: Vector3d;
    ...
    public evaluate(thetaRadians: number, phiRadians: number): boolean {
        ...
        this.currentF.setOriginAndVectorsXYZ(
            ...
            // df/dPhi, dg/dPhi, 0
            q,
            this._d1Phi.dotProduct(this._d1Phi) + this._delta.dotProduct(this._d2Phi),
            0
        );
...
export class GeodesicPathPoint {
    ...
    public evaluateDerivativesAtCurrentAngles(ellipsoid: Ellipsoid) {
        ellipsoid.radiansToPointAnd2Derivatives(
            this.thetaRadians,
            this.phiRadians,
            this.point,
            this.dTheta,
     ...
     */
    public static evaluateNewtonFunction(
        ...
        values[2] =
            pointB.dPhi.tripleProduct(this._vectorCB, pointB.d1Cross) +
...
export class GeodesicPathSolver {
    private readonly _defaultArc: Arc3d;
    private readonly _pathPoints: GeodesicPathPoint[];
     ...
     */
    public static createGeodesicPath(
        originalEllipsoid: Ellipsoid,
        startAngles: LongitudeLatitudeNumber,
        endAngles: LongitudeLatitudeNumber,
        density: number | Angle
    ): GeodesicPathPoint[] | undefined {
        const workEllipsoid1 = originalEllipsoid.radiansPairToEquatorialEllipsoid(
            startAngles.longitudeRadians,
            startAngles.latitudeRadians,
            endAngles.longitudeRadians,
            endAngles.latitudeRadians
        );
        const workArc = originalEllipsoid.radiansPairToGreatArc(
            startAngles.longitudeRadians,
            startAngles.latitudeRadians,
            endAngles.longitudeRadians,
            endAngles.latitudeRadians
        ...
        while (numStep < 15 && numConverged < 2) {
            ...
            solver.setupStep(workEllipsoid);
            if (previousMaxDPhi < dPhiTolerance) numConverged++;
            else numConverged = 0;
            numStep++;
        }
        if (numConverged > 0) {
    ...
    private setupStep(ellipsoid: Ellipsoid) {
        ...
        for (let i = 1; i < lastRow; i++) {
            GeodesicPathPoint.evaluateNewtonFunction(
                this._pathPoints[i - 1],
                this._pathPoints[i],
                this._pathPoints[i + 1],
                values
            );
     ...
     */
    public static approximateMinimumLengthSectionArc(
        ellipsoid: Ellipsoid,
        angleA: LongitudeLatitudeNumber,
        angleB: LongitudeLatitudeNumber,
        numSample: number,
