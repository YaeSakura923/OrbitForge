...
import { PointStringDeepXYZArrayCollector, VariantPointDataStream } from "./point-streaming";
import { Point2d } from "./point2d-vector2d";
import { Point3dArrayCarrier } from "./point3d-array-carrier";
 ...
 */
function selectOptionalClampedMin(
    numA: number,
    ...
    if (numB !== undefined) {
        ...
        if (numC >= 0 && numC <= numA) return numC;
    }
    return numA;
}
...
export class NumberArray {
    ...
    public static isExactEqual(
        dataA: any[] | Float64Array | undefined,
        dataB: any[] | Float64Array | undefined
    ): boolean {
        if (dataA && dataB) {
            if (dataA.length !== dataB.length) return false;
            for (let i = 0; i < dataA.length; i++) if (dataA[i] !== dataB[i]) return false;
            return true;
        }
    ...
    public static isCoordinateInArray(x: number, data: number[] | undefined): boolean {
        if (data) {
            for (const y of data) {
                if (Geometry.isSameCoordinate(x, y)) return true;
            }
        }
        return false;
    }
    ...
    public static maxAbsDiff(
        dataA: number[] | Float64Array,
        dataB: number[] | Float64Array
    ): number {
        let a = 0.0;
    ...
    public static cloneWithStartAndEndMultiplicity(
        ...
        for (let k = 0; k < target0; k++) result.push(knot0);
        for (let k = multiplicity0; k + multiplicity1 < knots.length; k++) result.push(knots[k]);
        for (let k = 0; k < target1; k++) result.push(knot1);
        return result;
    }
    ...
    public static linearCombination(data: number[], scales: number[]): number {
        ...
        let sum = 0;
        for (let i = 0; i < numTerms; ++i) sum += scales[i] * data[i];
        return sum;
    }
...
export class Point2dArray {
    ...
    public static isAlmostEqual(
        dataA: undefined | Point2d[],
        dataB: undefined | Point2d[]
    ): boolean {
        if (dataA && dataB) {
            if (dataA.length !== dataB.length) return false;
            for (let i = 0; i < dataA.length; i++) {
                if (!dataA[i].isAlmostEqual(dataB[i])) return false;
            }
            return true;
        }
        return dataA === undefined && dataB === undefined;
    ...
    public static pointCountExcludingTrailingWraparound(data: XAndY[]): number {
        ...
        while (n > 1) {
            if (
                !Geometry.isSameCoordinate(data[n - 1].x, x0) ||
                !Geometry.isSameCoordinate(data[n - 1].y, y0)
            ) {
                return n;
            }
            n--;
...
export class Point4dArray {
    ...
    public static packPointsAndWeightsToFloat64Array(
        ...
        if (Array.isArray(data) && data[0] instanceof Point3d) {
            ...
            if (points.length !== weights.length) return undefined;
            ...
            if (points.length !== 3 * numPoints) return undefined;
            let i = 0;
            let k;
    ...
    public static unpackFloat64ArrayToPointsAndWeights(
        ...
        points: Point3d[],
        weights: number[],
        pointFormatter: (x: number, y: number, z: number) => any = (x, y, z) =>
    ...
    private static readonly _workPoint4d = Point4d.create();
    /**
    ...
    public static isAlmostEqual(
        dataA: Point4d[] | Float64Array | undefined,
        dataB: Point4d[] | Float64Array | undefined
    ): boolean {
        if (dataA && dataB) {
            if (dataA.length !== dataB.length) return false;
            if (dataA instanceof Float64Array && dataB instanceof Float64Array) {
                for (let i = 0; i < dataA.length; i++) {
                    if (!Geometry.isSameCoordinate(dataA[i], dataB[i])) return false;
                }
    ...
    public static isCloseToPlane(
        ...
        if (Array.isArray(data)) {
            ...
            for (let i = 0; i + 2 < numXYZ; i += 4) {
                if (
                    Math.abs(plane.altitudeXYZW(data[i], data[i + 1], data[i + 2], data[i + 3])) >
                    tolerance
                ) {
                    return false;
                }
...
export class Point3dArray {
     ...
     */
    public static evaluateTrilinearWeights(
    ...
    public static sumWeightedX(weights: Float64Array, points: Point3d[]): number {
        ...
        for (let i = 0; i < n; i++) sum += weights[i] * points[i].x;
        return sum;
    }
    ...
    public static sumWeightedY(weights: Float64Array, points: Point3d[]): number {
        let sum = 0.0;
        const n = weights.length;
        for (let i = 0; i < n; i++) sum += weights[i] * points[i].y;
        return sum;
    }
    ...
    public static sumWeightedZ(weights: Float64Array, points: Point3d[]): number {
        let sum = 0.0;
        const n = weights.length;
        for (let i = 0; i < n; i++) sum += weights[i] * points[i].z;
        return sum;
    }

    private static readonly _weightUVW = new Float64Array(8);
    private static readonly _weightDU = new Float64Array(8);
    private static readonly _weightDV = new Float64Array(8);
    private static readonly _weightDW = new Float64Array(8);
    /**
     ...
     */
    public static evaluateTrilinearPoint(
        points: Point3d[],
        u: number,
        v: number,
        w: number,
     ...
     */
    public static evaluateTrilinearDerivativeTransform(
        points: Point3d[],
        u: number,
        v: number,
        w: number,
     ...
     * @param numPerBlock number of values in each block at first level down
     */
    public static unpackNumbersToNestedArraysIJK(
        data: Float64Array,
        numPerBlock: number,
        numPerRow: number
    ): any[] {
    ...
    public static indexOfMostDistantPoint(
        points: Point3d[],
        spacePoint: XYZ,
        farVector: Vector3d
    ): number | undefined {
    ...
    public static isCloseToPlane(
        ...
        if (Array.isArray(data)) {
            let xyz;
            for (xyz of data) {
                if (Math.abs(plane.altitude(xyz)) > tolerance) return false;
            }
        } else if (data instanceof Float64Array) {
            const numXYZ = data.length;
            for (let i = 0; i + 2 < numXYZ; i += 3) {
     ...
     * @param data points.
     */
    public static sumEdgeLengths(
        data: Point3d[] | Float64Array,
        addClosureEdge: boolean = false,
        ...
        if (Array.isArray(data)) {
            ...
            if (addClosureEdge && n > 0) sum += data[0].distance(data[n]);
        } else if (data instanceof Float64Array) {
            const numXYZ = selectOptionalClampedMin(data.length, maxPointsToUse, 3);
            let i = 0;
            for (; i + 5 < numXYZ; i += 3) {
                // final i points at final point x
     ...
     */
    public static countNonDuplicates(
        points: Point3d[],
        tolerance: number = Geometry.smallMetricDistance
    ): number {
        let n = points.length;
        ...
        while (n > 1) {
            if (points[0].isAlmostEqual(points[n - 1], tolerance)) n--;
            else break;
        }
        for (let i = 0; i + 1 < n; i++) {
            if (points[i].isAlmostEqual(points[i + 1], tolerance)) return 0;
        }
        return n;
    }
    ...
    public static clonePoint3dArray(data: XYZProps[] | Float64Array): Point3d[] {
        ...
        for (const p of data) {
            if (Array.isArray(p)) result.push(Point3d.create(p[0], p[1], p[2]));
            else result.push(Point3d.create(p.x, p.y, p.z));
        }
        return result;
    }
    ...
    public static cloneWithMaxEdgeLength(points: Point3d[], maxEdgeLength: number): Point3d[] {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            for (let k = 1; k < n; k++) result.push(points[i - 1].interpolate(k / n, points[i]));
    ...
    public static cloneDeepJSONNumberArrays(data: MultiLineStringDataVariant): number[][] {
        const collector = new PointStringDeepXYZArrayCollector((x, y, z) =>
            this.xyzToArray(x, y, z)
        );
     ...
     * (no index checking!)
     */
    public static distanceIndexedPointBToSegmentAC(
        points: Point3d[],
        indexA: number,
        indexB: number,
        indexC: number,
    ...
    public static computeConvexHullXY(
        ...
        if (n < 3) {
            for (const p of xy1) hullPoints.push(p);
            if (addClosurePoint && xy1.length > 0) hullPoints.push(xy1[0]);
            return;
        }
        ...
        for (let i = 2; i < n; i++) {
            ...
            while (
                top >= 1 &&
                hullPoints[top - 1].crossProductToPointsXY(hullPoints[top], candidate) <= 0.0
            ) {
        ...
        for (let i = n - 1; i-- > 0; ) {
            ...
            if (i > 0) {
                // don't replicate start !!!
                hullPoints.push(candidate);
            }
        }
        if (addClosurePoint) hullPoints.push(hullPoints[0]);
    }
    ...
    public static minMaxPoints(
        ...
        | { minXPoint: Point3d; maxXPoint: Point3d; minYPoint: Point3d; maxYPoint: Point3d }
        | undefined {
        if (data.length === 0) return undefined;
        const result = {
            minXPoint: data[0].clone(),
            maxXPoint: data[0].clone(),
            minYPoint: data[0].clone(),
            maxYPoint: data[0].clone()
        };
        let q;
