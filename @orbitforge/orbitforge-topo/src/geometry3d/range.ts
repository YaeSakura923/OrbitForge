...
import { type BeJSONFunctions, AxisIndex, Geometry } from "../geometry";
...
import { PointStreamRangeCollector, VariantPointDataStream } from "./point-streaming";
import { type Vector2d, Point2d } from "./point2d-vector2d";
import { type Vector3d, Point3d } from "./point3d-vector3d";
...
export class Range3d extends RangeBase implements LowAndHighXYZ, BeJSONFunctions {
    ...
    private setDirect(
        ...
        this.high.z = zB;
        if (correctToNull) {
            if (this.low.x > this.high.x || this.low.y > this.high.y || this.low.z > this.high.z) {
                this.setNull();
            }
        }
    }
    ...
    public get isSinglePoint(): boolean {
        return (
            this.high.x === this.low.x && this.high.y === this.low.y && this.high.z === this.low.z
        );
    }
     ...
     */
    public ensureMinLengths(min: number = 0.001): void {
...
export class Range2d extends RangeBase implements LowAndHighXY {
    ...
    public isAlmostEqual(other: Range2d): boolean {
        return (
            (this.low.isAlmostEqual(other.low) && this.high.isAlmostEqual(other.high)) ||
            (this.isNull && other.isNull)
        );
    }
