...
import { type Point3d, Vector3d } from "./point3d-vector3d";
...
export class CoincidentGeometryQuery {
    ...
    public clampCoincidentOverlapToSegmentBounds(
        ...
        const rangeA = Segment1d.create(
            overlap.detailA.fraction,
            overlap.detailA.hasFraction1 ? overlap.detailA.fraction1 : overlap.detailA.fraction
        );
        const rangeB = Segment1d.create(
            overlap.detailB.fraction,
            overlap.detailB.hasFraction1 ? overlap.detailB.fraction1 : overlap.detailB.fraction
        );
        ...
        if (haveIntervalA && haveIntervalB) {
            if (
                Geometry.isAlmostEqualNumber(
                    rangeA.absoluteDelta(),
                    rangeB.absoluteDelta(),
                    Geometry.smallFraction
                )
            ) {
                return updateIntervalFromRangesAndInterpolatedPoints();
            } // intersection of partially clamped ranges
            else if (
                rangeA.clampDirectedTo01(true, true, false) &&
                rangeB.clampDirectedTo01(true, true, false)
            ) {
                return updateIntervalFromRangesAndInterpolatedPoints();
            } // intersection of fully clamped ranges
        }

        const collapseToSingleton = (
            pointA: Point3d,
            pointB: Point3d,
            atStartA: boolean,
            atStartB: boolean
        ...
        if (haveSingletonA && haveSingletonB) {
    ...
    public coincidentSegmentRangeXY(
        ...
        if (pointA0.distanceXY(detailA0OnB.point) > this._tolerance) return undefined;
        ...
        if (pointB0.distanceXY(detailB0OnA.point) > this._tolerance) return undefined;
        ...
        return restrictToBounds
            ? this.clampCoincidentOverlapToSegmentBounds(
                  interval,
                  pointA0,
                  pointA1,
                  pointB0,
    ...
    private createDetailPair(
        ...
        if (g0 !== undefined && g1 !== undefined) {
            ...
            if (reverse) detailA.swapFractionsAndPoints();
            ...
            if (matrixBToA) {
                ...
                if (
                    ...
                    const betaStartRadians =
                        alphaB0Radians + sweepDirection * arcB.sweep.startRadians; // arcB start in arcA parameter space
                    ...
                    const fractionSpacesReversed =
                        sweepDirection * arcA.sweep.sweepRadians * arcB.sweep.sweepRadians < 0;
                    ...
                    const fractionB0 = sweepA.radiansToPositivePeriodicFraction(
                        sweepB.startRadians
                    ); // arcB start in arcA fraction space
                     ...
                     */
                    const appendCoincidentIntersection = (
                        arcBInArcAFractionSpace: Segment1d,
                        testStartOfArcA: boolean
                    ): boolean => {
                        const size = result ? result.length : 0;
                        const arcBStart = arcBInArcAFractionSpace.x0;
                        const arcBEnd = arcBInArcAFractionSpace.x1;
                        if (
                            arcBInArcAFractionSpace.clampDirectedTo01() &&
                            !Geometry.isSmallRelative(arcBInArcAFractionSpace.absoluteDelta())
                        ) {
                            ...
                            // test isolated intersection
                            const testStartOfArcB = fractionSpacesReversed
                                ? testStartOfArcA
                                : !testStartOfArcA;
                            const arcAPt = (this._point0 = testStartOfArcA
                                ? arcA.startPoint(this._point0)
                                : arcA.endPoint(this._point0));
                            const arcBPt = (this._point1 = testStartOfArcB
                                ? arcB.startPoint(this._point1)
                                : arcB.endPoint(this._point1));
