...
import { type AngleProps, type BeJSONFunctions, type TrigValues, Geometry } from "../geometry";
...
export class Angle implements BeJSONFunctions {
    ...
    public static cleanupTrigValue(
        value: number,
        tolerance: number = Geometry.smallFloatingPoint
    ): number {
    ...
    public static dotProductsToHalfAngleTrigValues(
        ...
        if (
            favorZero &&
            Math.abs(sin2t0) < Geometry.smallAngleRadians * (Math.abs(dotUU) + Math.abs(dotVV))
        ) {
