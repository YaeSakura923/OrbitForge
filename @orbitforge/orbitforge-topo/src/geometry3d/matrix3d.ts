...
export class Matrix3d implements BeJSONFunctions {
    ...
    private static readonly _productBuffer = new Float64Array(9);
    ...
    */
    ...
    public setFromJSON(json?: Matrix3dProps | Matrix3d): void {
        ...
        if (json.length === 9) {
            ...
            this.setRowValues(json[0], json[1], 0, json[2], json[3], 0, 0, 0, 1);
            return;
        }
    ...
    private applySymmetricJacobi(
        ...
        if (Math.abs(sij) < Geometry.smallFloatingPoint * (sii + sjj)) return 0.0;
     ...
     */
    public static createFlattenAlongVectorToPlane(
    ...
    public multiplyMatrixMatrix(other: Matrix3d, result?: Matrix3d): Matrix3d {
        ...
        if (
            ...
            result.finishInverseCoffs(
                (a, b, _result) => PackedMatrix3dOps.multiplyMatrixMatrix(a, b, _result),
                other.inverseCoffs,
                this.inverseCoffs
            );
        } else if (
    ...
    private static indexedRowCrossProduct(
        ...
        dest[columnStart + 3] =
            source[rowStart0 + 2] * source[rowStart1] - source[rowStart0] * source[rowStart1 + 2];
        dest[columnStart + 6] =
            source[rowStart0] * source[rowStart1 + 1] - source[rowStart0 + 1] * source[rowStart1];
    }
    ...
    private indexedColumnCrossProductInPlace(
        ...
        coffs[colStart2 + 3] =
            coffs[colStart0 + 6] * coffs[colStart1] - coffs[colStart0] * coffs[colStart1 + 6];
        coffs[colStart2 + 6] =
            coffs[colStart0] * coffs[colStart1 + 3] - coffs[colStart0 + 3] * coffs[colStart1];
    }
    ...
    public isRigid(allowMirror: boolean = false): boolean {
        return (
            this.testPerpendicularUnitRowsAndColumns() && (allowMirror || this.determinant() > 0)
        );
    }
