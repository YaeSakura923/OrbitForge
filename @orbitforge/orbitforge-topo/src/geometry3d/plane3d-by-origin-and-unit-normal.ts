...
export class Plane3dByOriginAndUnitNormal extends Plane3d implements BeJSONFunctions {
    ...
    public isAlmostEqual(other: Plane3dByOriginAndUnitNormal): boolean {
        return (
            this._origin.isAlmostEqual(other._origin) && this._normal.isAlmostEqual(other._normal)
        );
    }
    ...
    public cloneTransformed(
        ...
        if (inverse) {
            ...
            if (
                transform.matrix.multiplyTransposeVector(result._normal, result._normal) !==
                    undefined &&
