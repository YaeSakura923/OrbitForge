...
export class PointStreamRangeCollector extends PointStreamXYZHandlerBase {
    ...
    public override handleXYZ(x: number, y: number, z: number): void {
        if (!this._range) this._range = Range3d.createNull();
    ...
    public claimResult(): Range3d {
        ...
        if (!range) return Range3d.createNull();
        return range;
    }
}
...
export class PointStringDeepXYZArrayCollector {
    ...
    private readonly _resultStack: any[];
    private readonly _xyzFunction: (x: number, y: number, z: number) => any;
    /**
     *
        ...
        if (Array.isArray(data)) {
            ...
            if (data.length > 0 && Point3d.isAnyImmediatePointType(data[0])) {
                ...
                for (const p of data) {
                    ...
                    if (x !== undefined && y !== undefined) handler.handleXYZ(x, y, z);
                    numPoint++;
                }
