...
import { PolylineCompressionContext } from "./polyline-compression-by-edge-offset";
...
export class PolylineOps {
    ...
    public static compressShortEdges(
        source: Point3d[] | IndexedXYZCollection,
     ...
     *
     */
    public static compressByPerpendicularDistance(
        source: Point3d[],
        maxDistance: number,
        numPass: number = 2
    ): Point3d[] {
    ...
    private static isDanglerConfiguration(
        ...
        if (indexA < 0 || indexA >= points.length) return false;
        const pointA = points[indexA];
        // simple point match ...
        const d2Q = pointA.distanceSquared(pointQ);
        if (d2Q <= squaredDistanceTolerance) return true;
        if (indexB < 0 || indexB >= points.length) return false;
     ...
     */
    public static compressDanglers(
        source: Point3d[],
        ...
        if (closed) {
            ...
            if (rightIndex + 1 < dest.length) dest.length = rightIndex + 1;
            if (leftIndex > 0) {
    ...
    public static addClosurePoint(data: Point3d[] | Point3d[][]) {
        ...
        if (Array.isArray(q0)) {
            for (const child of data) {
                if (Array.isArray(child)) this.addClosurePoint(child);
            }
            return;
        }
    ...
    public static removeClosurePoint(data: Point3d[] | Point3d[][]) {
        ...
        if (Array.isArray(q0)) {
            for (const child of data) {
                if (Array.isArray(child)) this.removeClosurePoint(child);
            }
            return;
        }
        const q1 = data[data.length - 1];
    ...
    public static createBisectorPlanesForDistinctPoints(
        ...
        const packedPoints = PolylineOps.compressShortEdges(
            centerline,
            2.0 * Geometry.smallMetricDistance
        ); // double the tolerance to ensure normalized vectors exist.
        if (packedPoints.length < 2) return undefined;
        ...
        for (let i = 1; i + 1 < packedPoints.length; i++) {
            ...
            if (
                undefined !==
                Plane3dByOriginAndUnitNormal.create(packedPoints[i], unit01, perpendicular1)
            ) {
                const newBisectorNormal = perpendicular0
                    .getNormalRef()
                    .interpolate(0.5, perpendicular1.getNormalRef());
                const newBisectorPlane = Plane3dByOriginAndUnitNormal.create(
                    packedPoints[i],
                    newBisectorNormal
                );
                if (undefined !== newBisectorPlane) bisectorPlanes.push(newBisectorPlane);
        ...
        bisectorPlanes.push(
            Plane3dByOriginAndUnitNormal.create(
                packedPoints[packedPoints.length - 1],
                perpendicular0.getNormalRef()
            )!
        );
        ...
        if (lastIndex > 0 && wrapIfPhysicallyClosed) {
            ...
            if (Geometry.isSamePoint3d(firstPlane.getOriginRef(), lastPlane.getOriginRef())) {
                ...
                if (undefined !== newBisectorPlane) {
                    ...
                    bisectorPlanes[lastIndex] = Plane3dByOriginAndUnitNormal.create(
                        lastPlane.getOriginRef(),
                        newBisectorNormal
                    )!;
                }
            }
