        ...
        if (edgeIndex + 1 < this.data.length) {
            ...
            if (vector.normalizeInPlace()) {
                ...
                for (let fraction = 1.0e-5; fraction < 3; fraction *= 5.0) {
                    ...
                    if (this.classifyPointXY(candidatePoint) === 1) return candidatePoint;
                }
            }
        }
        return undefined;
...
export class SortablePolygon {
    private readonly _loopCarrier: SimpleRegionCarrier;
    ...
    private static assignParentsAndDepth(loops: SortablePolygon[]): void {
        ...
        loops.sort(
            (loopA: SortablePolygon, loopB: SortablePolygon) => loopB.sortKey - loopA.sortKey
        );
            ...
            if (xy !== undefined) {
                ...
                for (let j = i; j-- > 0; ) {
                    ...
                    if (otherLoop.range.containsXY(xy.x, xy.y)) {
                        if (otherLoop._loopCarrier.classifyPointXY(xy) === 1) {
