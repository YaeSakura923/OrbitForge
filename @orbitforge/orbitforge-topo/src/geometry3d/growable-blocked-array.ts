...
export class GrowableBlockedArray {
    ...
    public addBlock(newData: number[]) {
        ...
        if (numValue > this._blockSize) numValue = this._blockSize;
        for (let i = 0; i < numValue; i++) this._data[k0 + i] = newData[i];
    }
    ...
    public sortIndicesLexical(
        compareBlocks: BlockComparisonFunction = (dataArray, size, iA, iB) =>
            GrowableBlockedArray.compareLexicalBlock(dataArray, size, iA, iB)
    ): Uint32Array {
        const n = this._inUse;
        // let numCompare = 0;
