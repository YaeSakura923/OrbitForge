...
import { XYParitySearchContext } from "../topology/xy-parity-search-context";
...
import { SortablePolygon } from "./sortable-polygon";
...
export class PolygonOps {
    ...
    private static readonly _vector0 = Vector3d.create();
    private static readonly _vector1 = Vector3d.create();
    private static readonly _vector2 = Vector3d.create();
    private static readonly _vectorOrigin = Vector3d.create();
    private static readonly _normal = Vector3d.create();
    private static readonly _matrixA = Matrix4d.createIdentity();
    private static readonly _matrixB = Matrix4d.createIdentity();
    private static readonly _matrixC = Matrix4d.createIdentity();
    ...
    public static closestPointOnBoundary(
        ...
        if (numPoints === 0) return result; // invalid
        if (numPoints === 1) {
    ...
    private static convexBarycentricCoordinatesOnEdge(
        ...
        const pointIsOnPrevEdge =
            !prevNormal.isZero && prevProj.x === 0.0 && Geometry.isIn01(prevProj.y);
        const pointIsOnEdge = !normal.isZero && proj.x === 0.0 && Geometry.isIn01(proj.y);
    ...
    public static convexBarycentricCoordinates(
        ...
        const largestResult =
            tolerance > 0.0 ? 1.0 / (tolerance * tolerance) : Geometry.largeCoordinateResult;
        let coordSum = 0.0;
        for (let i = 0; i < n; ++i) {
            ...
            if (
                ...
                return coords;
            } // point is on vertex or edge; we are done
            ...
            if (projToPrevEdge.x === 0.0 || projToEdge.x === 0.0) continue; // point is on subsequent colinear edge (ASSUMING interior point, convex polygon!); coords[i] = 0; keep previous edge data
...
export class IndexedXYZCollectionPolygonOps {
    private static readonly _xyz0Work: Point3d = Point3d.create();
    private static readonly _xyz1Work: Point3d = Point3d.create();
    private static readonly _xyz2Work: Point3d = Point3d.create();
    /**
...
export class Point3dArrayPolygonOps {
    ...
    public static polygonPlaneCrossings(
        ...
        IndexedXYZCollectionPolygonOps.polygonPlaneCrossings(plane, xyzSource, crossings);
    }
