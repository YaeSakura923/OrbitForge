...
} from "../curve/curve-location-detail";
import { type BeJSONFunctions, AxisOrder, Geometry } from "../geometry";
import { SmallSystem } from "../numerics/polynomials";
import { Matrix3d } from "./matrix3d";
import { type Plane3dByOriginAndUnitNormal } from "./plane3d-by-origin-and-unit-normal";
import { Vector2d } from "./point2d-vector2d";
...
export class Ray3d implements BeJSONFunctions {
    ...
    public isAlmostEqual(other: Ray3d): boolean {
        return (
            this.origin.isAlmostEqual(other.origin) && this.direction.isAlmostEqual(other.direction)
        );
    }
    ...
    public cloneInverseTransformed(transform: Transform, result?: Ray3d): Ray3d | undefined {
        if (!transform.computeCachedInverse(true)) return undefined;
        return Ray3d.create(
    ...
    public toRigidZFrame(result?: Transform): Transform | undefined {
        const axes = (Ray3d._workMatrix = Matrix3d.createRigidHeadsUp(
            this.direction,
            AxisOrder.ZXY,
            Ray3d._workMatrix
        ));
    ...
    public intersectionWithTriangle(
         ...
         */
        if (distanceTol === undefined || distanceTol < 0) {
            // we explicitly allow zero tolerance
            distanceTol = Geometry.smallMetricDistance;
        }
        if (parameterTol === undefined || parameterTol < 0) {
            // we explicitly allow zero tolerance
            parameterTol = Geometry.smallFloatingPoint;
        }
        const edge1 = (Ray3d._workVector0 = Vector3d.createStartEnd(
            vertex0,
            vertex1,
            Ray3d._workVector0
        ));
        const edge2 = (Ray3d._workVector1 = Vector3d.createStartEnd(
            vertex0,
            vertex2,
            Ray3d._workVector1
        ));
        ...
        if (a >= -distanceTol && a <= distanceTol) return undefined; // ray is parallel to the triangle (includes coplanar case)
        const f = 1.0 / a;
        const s = (Ray3d._workVector3 = Vector3d.createStartEnd(
            vertex0,
            this.origin,
            Ray3d._workVector3
        ));
        ...
        if (u < 0.0) {
            if (u > -parameterTol) u = 0.0;
            else return undefined; // ray does not intersect the triangle
        } else if (u > 1.0) {
            if (u < 1.0 + parameterTol) u = 1.0;
            else return undefined; // ray does not intersect the triangle
        }
        const q = (Ray3d._workVector4 = s.crossProduct(edge1, Ray3d._workVector4));
        let v = f * this.direction.dotProduct(q);
        if (v < 0.0) {
            if (v > -parameterTol) v = 0.0;
            else return undefined; // ray does not intersect the triangle
        } else if (u + v > 1.0) {
            if (u + v < 1.0 + parameterTol) v = 1.0 - u;
            else return undefined; // ray does not intersect the triangle
        }
