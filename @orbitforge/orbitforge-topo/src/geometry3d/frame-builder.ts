...
import { Point3dArray } from "./point-helpers";
import { Point3d, Vector3d } from "./point3d-vector3d";
import { PolygonOps } from "./polygon-ops";
...
export class FrameBuilder {
    ...
    private static readonly _workVector0?: Vector3d;
    private static readonly _workVector1?: Vector3d;
    ...
    public getValidatedFrame(
        ...
        if (this._origin && this._vector0 && this._vector1) {
            const createRigidMatrix = (v0: Vector3d, v1: Vector3d): Matrix3d | undefined => {
                return (FrameBuilder._workMatrix = Matrix3d.createRigidFromColumns(
                    v0,
                    v1,
                    AxisOrder.XYZ,
                    FrameBuilder._workMatrix
                ));
            };
            if (!allowLeftHanded) {
                const matrix = createRigidMatrix(this._vector0, this._vector1);
                if (matrix) return Transform.createOriginAndMatrix(this._origin, matrix, result);
    ...
    public static createRightHandedFrame(
        ...
        let frame =
            params.length > 0 && params[params.length - 1] instanceof Transform
                ? (params.pop() as Transform)
                : undefined;
        ...
        for (const data of params) {
            if (data instanceof CurvePrimitive) {
                ...
                for (const curve of children) {
                    if ((frame = evaluatePrimitiveFrame(curve, frame))) return frame;
                }
            }
        }
        return undefined;
    ...
    public static createFrameToDistantPoints(
        ...
        if (points.length > 2) {
            ...
            const matrix = (FrameBuilder._workMatrix = Matrix3d.createRigidFromColumns(
                vector01,
                vector02,
                AxisOrder.XYZ,
                FrameBuilder._workMatrix
    ...
    public static createFrameWithCCWPolygon(
        ...
        if (points.length > 2) {
            ...
            if (ray) return ray.toRigidZFrame(result);
        }
        return undefined;
    }
    ...
    public static createLocalToWorldTransformInRange(
        ...
        if (range.isNull) return Transform.createIdentity(result);
        let a = 1.0;
        let b = 1.0;
        let c = 1.0;
        if (scaleSelect === AxisScaleSelect.LongestRangeDirection) {
            ...
            a =
                Geometry.correctSmallMetricDistance(range.xLength(), defaultAxisLength) *
                Geometry.maxAbsDiff(fractionX, 0, 1);
            b =
                Geometry.correctSmallMetricDistance(range.yLength(), defaultAxisLength) *
                Geometry.maxAbsDiff(fractionY, 0, 1);
            c =
                Geometry.correctSmallMetricDistance(range.zLength(), defaultAxisLength) *
                Geometry.maxAbsDiff(fractionZ, 0, 1);
        }
        const origin = (FrameBuilder._workPoint = range.fractionToPoint(
            fractionX,
            fractionY,
            fractionZ,
            FrameBuilder._workPoint
        ));
        const matrix = (FrameBuilder._workMatrix = Matrix3d.createScale(
            a,
            b,
            c,
            FrameBuilder._workMatrix
