...
import { HalfEdgeGraphSearch, HalfEdgeMaskTester } from "./half-edge-graph-search";
import { HalfEdgeGraphMerge } from "./merging";
import { RegularizationContext } from "./regularize-face";
...
class NodeSortKey {
    ...
    private readonly _node: HalfEdge;
    public get node() {
        return this._node;
    }
...
export class HalfEdgeGraphSpineContext {
    /** The Evolving graph */
    private readonly _spineGraph: HalfEdgeGraph;
    /** mask marking edges that have been paired into quads */
    private readonly _diagonalMask: HalfEdgeMask;
    private readonly _boxMask: HalfEdgeMask;
    public get graph() {
    ...
    private getBoxCorners(diagonalNode: HalfEdge, nodes: HalfEdge[], points: Point3d[]): boolean {
        ...
        if (
            ...
            for (let i = 0; i < 4; i++) points.push(Point3d.create(nodes[i].x, nodes[i].y));
            return true;
        }
        return false;
    }
    ...
    private getSpineEdgesInQuad(
        ...
        for (let i: number = 0; i < 4; i++) {
            bIsBoundary[i] = pNode[i].getMask(HalfEdgeMask.BOUNDARY_EDGE) !== 0;
            if (pNode[i].getMask(HalfEdgeMask.BOUNDARY_EDGE)) iBoundary[numBoundary++] = i;
        ...
        if (numBoundary === 0 || numBoundary === 1) {
            for (let i: number = 0; i < numInterior; i++) {
                if (bIncludeInterior) this.addEdge(xyzOut, midpoint[iInterior[i]], centroid);
            }
            ...
            if (bIncludeInterior) this.addEdge(xyzOut, midpoint[iInterior[0]], centroid);
            if (bIncludeFinal) this.addEdge(xyzOut, centroid, midpoint[iInterior[0] + 2]);
        }
        return true;
    }
    ...
    private static readonly _regularize1 = true;
    private static readonly _regularize2 = false;
    /**
    ...
    public getSpineEdges(
        ...
        this._spineGraph.announceFaceLoops((_graph: HalfEdgeGraph, faceSeed: HalfEdge) => {
            if (!faceSeed.getMask(HalfEdgeMask.EXTERIOR)) {
                if (
                    this.getSpineEdgesInTriangle(faceSeed, xyzOut, bIncludeInterior, bIncludeFinal)
                ) {
                } else if (
    ...
    public consolidateTrianglesToQuads(
        ...
        if (bDeleteDiagonals && numDiagonal > 0) {
            this.graph.yankAndDeleteEdges((node: HalfEdge) => node.getMask(this._diagonalMask));
        }
        return numDiagonal;
    }
}
