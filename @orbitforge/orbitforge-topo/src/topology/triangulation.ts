...
import { InsertAndRetriangulateContext } from "./insert-and-retriangulate-context";
...
export class Triangulator {
    ...
    public static flipTrianglesInEdgeSet(graph: HalfEdgeGraph, edgeSet: MarkedEdgeSet): number {
        const barrierMasks =
            HalfEdgeMask.EXTERIOR | HalfEdgeMask.PRIMARY_EDGE | HalfEdgeMask.BOUNDARY_EDGE;
    ...
    public static createTriangulatedGraphFromSingleLoop(
        ...
        if (!startingNode || graph.countNodes() < 6) return undefined;

        if (!Triangulator.triangulateSingleFace(graph, startingNode)) return undefined;
    ...
    private static getUnwrappedLength(data: LineStringDataVariant): number {
        ...
        while (n > 1) {
            ...
            if (Geometry.isAlmostEqualNumber(x0, x1) && Geometry.isAlmostEqualNumber(y0, y1)) --n;
            else break;
        }
        return n;
    }
    ...
    private static findAroundOrAtVertex(a: HalfEdge, b: HalfEdge): boolean {
        if (a.findAroundVertex(b)) return true;
        return Geometry.isAlmostEqualXAndY(a, b);
    }

    // for reuse over all calls to isEar ....
    private static readonly _edgeInterval = Range1d.createNull();
    private static readonly _earRange = Range2d.createNull();
    private static readonly _edgeRange = Range2d.createNull();
    private static readonly _planes: Plane3dByOriginAndUnitNormal[] = [
    ...
    private static findHoleBridge(hole: HalfEdge, outerNode?: HalfEdge): HalfEdge | undefined {
        ...
        do {
            if (hy <= p.y && hy >= p.faceSuccessor.y && p.faceSuccessor.y !== p.y) {
                const x =
                    p.x + ((hy - p.y) * (p.faceSuccessor.x - p.x)) / (p.faceSuccessor.y - p.y);
                if (x <= hx && x > qx) {
                    qx = x;
                    if (x === hx) {
                        if (hy === p.y) return p;
...
class AssembleXYZXYZChains extends PointStreamXYZXYZHandlerBase {
    ...
    private readonly _graph: HalfEdgeGraph;
