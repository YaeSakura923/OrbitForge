...
import { HalfEdgePriorityQueueWithPartnerArray } from "./half-edge-priority-queue";
import { RegularizationContext } from "./regularize-face";
import { type MultiLineStringDataVariant, Triangulator } from "./triangulation";
...
export class HalfEdgeGraphOps {
        ...
        for (const node of graph.allHalfEdges) {
            if (!node.isMaskSet(visit)) {
                if (!node.isMaskSet(barrier) && !node.edgeMate.isMaskSet(barrier)) {
                    ...
                    const signedAreaTol =
                        Geometry.smallMetricDistanceSquared *
                        (node.signedFaceArea() + node.edgeMate.signedFaceArea());
                    if (
                        this.isSectorConvexAfterEdgeRemoval(node, mark, barrier, signedAreaTol) &&
                        this.isSectorConvexAfterEdgeRemoval(
                            node.edgeMate,
                            mark,
                            barrier,
                            signedAreaTol
    ...
    public static collectRemovableEdgesToExpandConvexFaces(
        ...
        if (this.markRemovableEdgesToExpandConvexFaces(graph, mark, barrier) > 0) {
    ...
    public static expandConvexFaces(
        ...
        if (numRemovedEdges > 0) {
            const numYankedEdges =
                0.5 * graph.yankAndDeleteEdges((node: HalfEdge) => node.getMask(mark));
...
export class HalfEdgeGraphMerge {
    ...
    public static isNullFace(node: HalfEdge): boolean {
        return (
            node.isMaskSet(HalfEdgeMask.NULL_FACE) &&
            node.faceSuccessor.isMaskSet(HalfEdgeMask.NULL_FACE) &&
