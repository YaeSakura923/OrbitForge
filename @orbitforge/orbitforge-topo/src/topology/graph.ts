...
export class HalfEdge implements HalfEdgeUserData {
    ...
    private static readonly _edgePropertyMasks: HalfEdgeMask[] = [
    ...
    public isFaceConvex(tolerance: number = Geometry.smallMetricDistanceSquared): boolean {
        ...
        do {
            if (!node.isSectorConvex(signedAreaTol)) return false;
            node = node.faceSuccessor;
        } while (node !== this);
        return true;
    }
...
export class HalfEdgeGraph {
    ...
    private readonly _maskManager: MaskManager;
    ...
    public yankAndDeleteEdges(deleteThisNode: NodeFunction): number {
        ...
        for (let i = 0; i < numTotal; i++) {
            ...
            if (!deleteThisNode(candidate)) this.allHalfEdges[numAccepted++] = candidate;
            else candidate.yankFromVertexLoop(); // ASSUME callback symmetry so we eventually yank the mate!
        }
