...
import {
    type EdgeParams,
    type EdgeTable,
    type IndexedEdgeParams,
    calculateEdgeTableParams
} from "./edge-params";
import { type MeshParams } from "./mesh-params";
import { type PointStringParams } from "./point-string-params";
import { type PolylineParams, type TesselatedPolyline } from "./polyline-params";
import { type SurfaceMaterial, createSurfaceMaterial } from "./surface-params";
import { VertexIndices } from "./vertex-indices";
import { type VertexTable, computeDimensions } from "./vertex-table";
...
class Node {
    ...
    public constructor(vertexTable: VertexTable, atlas?: AtlasInfo) {
        ...
        if (atlas) {
            const atlasOffset =
                (vertexTable.numVertices * vertexTable.numRgbaPerVertex + atlas.offset) * 4;
        ...
        for (const index of this._input.indices) {
            ...
            if (curState.featureIndex !== featureIndex) {
                ...
                if (undefined === node) {
                    this._nodes.set(
                        nodeId,
                        (node = new Node(this._input.vertices, this._input.atlasInfo))
                    );
                }
...
function remapPolylineEdges(
    ...
    for (const srcIndex of src.indices) {
        if (remapIndex(remappedIndex, srcIndex, nodes)) {
            ...
            const newPrevIndex = remappedIndex.node.remappedIndices.get(prevIndex as number);
    ...
    for (const [id, remappedEdges] of edges) {
        ...
        if (remappedEdges.indexed) {
            ...
            const { width, height, silhouettePadding, silhouetteStartByteIndex } =
                calculateEdgeTableParams(numSegmentEdges, numSilhouettes, maxDimension);
...
export function splitMeshParams(args: SplitMeshArgs): Map<number, MeshParams> {
    ...
    const atlasInfo =
        undefined !== atlasOffset
            ? { offset: atlasOffset, createMaterial: args.createMaterial }
            : undefined;
    ...
    const edges = args.params.edges
        ? splitEdges(args.params.edges, nodes, args.maxDimension)
        : undefined;
