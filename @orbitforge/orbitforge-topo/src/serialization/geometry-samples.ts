...
import { BezierCurve3d } from "../bspline/bezier-curve3d";
import { BezierCurve3dH } from "../bspline/bezier-curve3d-homogeneous";
...
import { BSplineSurface3d, BSplineSurface3dH, WeightStyle } from "../bspline/bspline-surface";
import {
    InterpolationCurve3d,
    InterpolationCurve3dOptions
} from "../bspline/interpolation-curve3d";
import { BSplineWrapMode, KnotVector } from "../bspline/knot-vector";
...
import {
    type AuxChannelDataType,
    AuxChannel,
    AuxChannelData,
    PolyfaceAuxData
} from "../polyface/aux-data";
...
export class Sample {
    ...
    public static createClipPlaneSets(): UnionOfConvexClipPlaneSets[] {
        ...
        result.push(
            UnionOfConvexClipPlaneSets.createConvexSets([quadrant1.clone(), quadrant4.clone()])
        );
        result.push(
    ...
    public static createLineArcPaths(): Path[] {
        ...
        for (const y0 of [0, -1, 1]) {
            for (const x2 of [15, 11, 20, 9, 7]) {
                ...
                path1.tryAddChild(
                    Arc3d.createCircularStartMiddleEnd(point1, Point3d.create(x2, y2, 0), point3)
                );
    ...
    public static createConeBsplineSurface(
        ...
        for (let iV = 0; iV < numVPole; iV++) {
            ...
            controlPoints.push(
                Sample.createBsplineArc90SectionToXYZWArrays(center, axes, r0, r90, false)
            );
        }
    ...
    public static createSimpleLinearSweeps(): LinearSweep[] {
        ...
        result.push(
            LinearSweep.create(Sample.createCappedArcLoop(5, -45, 90), vectorQ, true) as LinearSweep
        );
    ...
    public static createTorusPipes(): TorusPipe[] {
        ...
        result.push(
            TorusPipe.createInFrame(Transform.createIdentity(), 5.0, 0.8, Angle.create360(), false)!
        );
        result.push(
    ...
    public static createBoxes(capped: boolean = true): Box[] {
        ...
        result.push(
            Box.createTopoBox(cornerA, vectorX, vectorY, cornerB, aX, aY, bX, bY, capped) as Box
        );
    ...
    public static createRuledSweeps(
        ...
        allSweeps.push(
            RuledSweep.create([contour0, contour1, contour2, contour3], true) as RuledSweep
        );
    ...
    public static createCurveChainWithDistanceIndex(): CurveChainWithDistanceIndex[] {
        ...
        result.push(
            CurveChainWithDistanceIndex.createCapture(Path.create(LineString3d.create(pointsA)))!
        );
        result.push(
            CurveChainWithDistanceIndex.createCapture(
            ...
            CurveChainWithDistanceIndex.createCapture(
                Path.create(
                    ...
                    InterpolationCurve3d.createCapture(
                        InterpolationCurve3dOptions.create({ fitPoints: [pointsA[0], pointsA[1]] })
                    )!
                )
            )!
        );
    ...
    public static createClosedSolidSampler(
        ...
        result.push(
            RotationalSweep.create(contourA.clone()!, axis.clone(), rotationAngle, capped)!
        );
        if (!capped) {
            result.push(
                RotationalSweep.create(contourZ.clone()!, axis.clone(), rotationAngle, false)!
            );
        }
        ...
        result.push(
            RuledSweep.create([contourA.clone()!, contourB.clone()!, contourC.clone()!], capped)!
        );
        return result;
    }
    ...
    public static createCutPie(
        ...
        for (let i = 1; i < numArcEdges; i++) {
            ...
            points.push(
                Point3d.create(x0 + radius * Math.cos(radians), y0 + radius * Math.sin(radians))
            );
        }
        points.push(pointB);
        for (let i = 1; i < numRadialEdges; i++) {
    ...
    public static createStar(
        ...
        for (let i = 0; i < numPoint; i++) {
            ...
            points.push(
                Point3d.create(cx + r0 * Math.cos(radians), cy + r0 * Math.sin(radians), cz)
            );
            if (r1 !== undefined) {
                radians = radians0 + (2 * i + 1) * angleStepRadians;
                points.push(
                    Point3d.create(cx + r1 * Math.cos(radians), cy + r1 * Math.sin(radians), cz)
                );
            }
        }
        if (close) points.push(points[0].clone());
    ...
    public static createAllGeometryQueryTypes(): GeometryQuery[] {
        ...
        result.push(
            this.createTriangularUnitGridPolyface(pointA, Vector3d.unitX(), Vector3d.unitY(), 4, 5)
        );
        result.push(
    ...
    public static createZigZag(
        ...
        if (Array.isArray(start)) {
            result = start;
            if (result.length === 0) result.push(Point3d.create(0, 0, 0));
        } else {
            result = [start.clone()];
        }
        let numAdded = 0;
        for (;;) {
            for (const step of steps) {
                if (++numAdded > numStroke) return result;
     ...
     */
    public static createMeshFromSmoothSurface(
        size: number,
