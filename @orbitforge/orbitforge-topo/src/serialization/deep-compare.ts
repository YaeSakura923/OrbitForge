...
export class DeepCompare {
    ...
    public propertyCounts: Record<string, number> = {};
    ...
    public compareNumber(a: number, b: number) {
        if (Math.abs(b - a) < this.numberRelTol * (1 + Math.abs(a) + Math.abs(b))) {
            ...
            this.errorTracker.unshift(
                `In ${this.errorTracker[this.errorTracker.length - 1]} property: Mismatched values`
            );
    ...
    private compareArray(a: any[], b: any[]) {
        if (a.length !== b.length) {
            const aCounter: Record<string, any> = {};
            const bCounter: Record<string, any> = {};
