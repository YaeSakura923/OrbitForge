...
} from "../bspline/interpolation-curve3d";
...
import { NumberArray, Point3dArray } from "../geometry3d/point-helpers";
...
export class BGFBReader {
     ...
     */
    public readBSplineSurfaceFromVariant(
        variantHeader: BGFBAccessors.VariantGeometry
    ...
    public readInterpolationCurve3d(
        ...
        if (xyzArray instanceof Float64Array) {
            ...
            const options = new InterpolationCurve3dOptions(
                Point3dArray.clonePoint3dArray(xyzArray),
                knots ? NumberArray.create(knots) : undefined
            );
            ...
            options.captureOptionalProps(
                ...
                startTangent !== null
                    ? Vector3d.create(startTangent.x(), startTangent.y(), startTangent.z())
                    : undefined,
                endTangent !== null
    ...
    public readTransitionSpiral(
        ...
        if (detailHeader) {
            ...
            const spiralTypeName = TopoSpiralTypeQueries.typeCodeToString(
            ...
            if (!directDetailHeader) {
                ...
                if (directSpiral) return directSpiral;
            }
        }
        return undefined;
    }
    ...
    public readCurvePrimitiveFromVariant(
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagLineSegment) {
            ...
            for (let i = 0; i + 2 < numCoordinates; i += 3) {
                result.packedPoints.pushXYZ(
                    offsetToLineString.points(i)!,
                    offsetToLineString.points(i + 1)!,
                    offsetToLineString.points(i + 2)!
                );
            }
            return result;
     ...
     * @param variant read position in the flat buffer.
     */
    public readPolyfaceAuxChannelData(
        channelDataHeader: BGFBAccessors.PolyfaceAuxChannelData | null
    ): AuxChannelData | undefined {
        if (channelDataHeader !== null) {
            const input = channelDataHeader.input();
            const values = channelDataHeader.valuesArray();
            if (values !== null) return new AuxChannelData(input, values);
        }
        return undefined;
    }
    ...
    public readTaggedNumericData(
        ...
        if (accessor) {
            ...
            if (doubleDataArray) {
                ...
                for (const c of doubleDataArray) taggedNumericData.doubleData.push(c);
            }
                ...
                if (meshStyle === 1 && pointF64 && pointIndexI32) {
                    ...
                    if (taggedNumericDataOffset) {
                        ...
                        if (taggedNumericDataAccessor !== undefined) {
                            const taggedNumericData =
                                this.readTaggedNumericData(taggedNumericDataAccessor);
    ...
    public readSolidPrimitiveFromVariant(
        ...
        const geometryType = variant.geometryType();
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoBox) {
            const header = variant.geometry(new BGFBAccessors.TopoBox());
            const detail = header!.detail()!;
            return Box.createTopoBox(
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoSphere) {
            const header = variant.geometry(new BGFBAccessors.TopoSphere());
            const detail = header!.detail()!;
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoCone) {
            const header = variant.geometry(new BGFBAccessors.TopoCone());
            const detail = header!.detail()!;
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoTorusPipe) {
            const header = variant.geometry(new BGFBAccessors.TopoTorusPipe())!;
            const detail = header.detail()!;
            ...
            return TorusPipe.createTopoTorusPipe(
                center,
                vectorX,
                vectorY,
                majorRadius,
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoExtrusion) {
            const header = variant.geometry(new BGFBAccessors.TopoExtrusion())!;
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoRotationalSweep) {
            const header = variant.geometry(new BGFBAccessors.TopoRotationalSweep())!;
        ...
        if (geometryType === BGFBAccessors.VariantGeometryUnion.tagTopoRuledSweep) {
            const header = variant.geometry(new BGFBAccessors.TopoRuledSweep())!;
     ...
     * @param variant read position in the flat buffer.
     */
    public readGeometryQueryFromVariant(
        variant: BGFBAccessors.VariantGeometry
        ...
        switch (rootType) {
            ...
            case BGFBAccessors.VariantGeometryUnion.tagTopoBox:
            case BGFBAccessors.VariantGeometryUnion.tagTopoCone:
            case BGFBAccessors.VariantGeometryUnion.tagTopoTorusPipe:
            case BGFBAccessors.VariantGeometryUnion.tagTopoSphere:
            case BGFBAccessors.VariantGeometryUnion.tagTopoExtrusion:
            case BGFBAccessors.VariantGeometryUnion.tagTopoRotationalSweep:
            case BGFBAccessors.VariantGeometryUnion.tagTopoRuledSweep: {
 ...
 * @internal
 */
export class TopoSpiralTypeQueries {
    ...
    public static typeCodeToString(typeCode: number): string | undefined {
        for (const entry of TopoSpiralTypeQueries.spiralTypeCodeMap) {
            if (entry[0] === typeCode) return entry[1] as string;
        }
        return undefined;
    }
