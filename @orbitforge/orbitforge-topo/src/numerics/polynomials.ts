...
import {
    type OptionalGrowableFloat64Array,
    GrowableFloat64Array
} from "../geometry3d/growable-float64-array";
import { LongitudeLatitudeNumber } from "../geometry3d/longitude-latitude-altitude";
...
export class Degree4PowerPolynomial {
    ...
    public evaluate(x: number): number {
        return (
            this.coffs[0] +
            x * (this.coffs[1] + x * (this.coffs[2] + x * (this.coffs[3] + x * this.coffs[4])))
        );
    }
    ...
    public evaluateDerivative(x: number): number {
        return (
            this.coffs[1] +
            x * (2.0 * this.coffs[2] + x * (3.0 * this.coffs[3] + x * 4.0 * this.coffs[4]))
        );
    }
...
export class AnalyticRoots {
    ...
    private static appendFullCubicSolutions(
        ...
        if (discriminant > 0) {
            ...
            const f = 0.5 / a;
            result.push(
                xN + this.signedCubeRoot(f * (-yN + r)) + this.signedCubeRoot(f * (-yN - r))
            );
  ...
  */
    ...
    public static appendQuarticRoots(c: Float64Array | number[], results: GrowableFloat64Array) {
        ...
        const r: number =
            (-3.0 / 256) * sq_A * sq_A + (1.0 / 16) * sq_A * B - (1.0 / 4) * A * C + D;
...
export class SmallSystem {
    ...
    public static solveBilinearPair(
        ...
        const e1 =
            Geometry.crossProductXYXY(b0, b1, c0, c1) + Geometry.crossProductXYXY(a0, a1, d0, d1);
