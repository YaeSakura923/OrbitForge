...
import { type Point2d, Vector2d } from "../geometry3d/point2d-vector2d";
...
export class ConvexPolygon2d {
    ...
    public distanceOutside(xy: Point2d): number {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (c < 0.0) {
                ...
                let d = 0.0;
                if (s < 0.0) d = xy0.distance(xy);
                else if (s > 1.0) d = xy1.distance(xy);
                else d = xy.distance(ray.fractionToPoint(s));
    ...
    public clipRay(ray: Ray2d): Range1d {
        ...
        if (n < 3) return Range1d.createNull();
        ...
        for (const xy1 of this._hullPoints) {
            ...
            if (hasIntersection) {
                if (cross > 0.0) {
                    if (fraction < distanceB) distanceB = fraction;
                } else {
                    if (fraction > distanceA) distanceA = fraction;
                }
                if (distanceA > distanceB) return Range1d.createNull();
            } else {
