...
import { type JsonExpr, type Value, type ValueMap, CallExpr, Env, Expr, MapEnv } from "../src/Expr";
import { type Definitions } from "../src/Theme";
...
function evaluate(expr: string | JsonExpr | Expr, env?: Env | ValueMap): Value {
    ...
    return (
        Expr.isExpr(expr) ? expr : typeof expr === "string" ? Expr.parse(expr) : Expr.fromJSON(expr)
