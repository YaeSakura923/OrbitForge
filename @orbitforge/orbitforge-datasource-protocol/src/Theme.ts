...
import { type Vector3Like } from "@orbitforge/orbitforge-geoutils/math/Vector3Like";
...
import {
    ...
    type ExtrudedPolygonTechniqueParams,
    type FillTechniqueParams,
    type LineTechniqueParams,
    type MarkerTechniqueParams,
    type Pickability,
    type PointTechniqueParams,
    type SegmentsTechniqueParams,
    type ShaderTechniqueParams,
    type SolidLineTechniqueParams,
    type StandardExtrudedLineTechniqueParams,
    type StandardTechniqueParams,
    type TerrainTechniqueParams,
    type TextTechniqueParams,
...
export interface Theme {
    ...
    /**
     * Optional environment map for reflections and lighting.
     *
     * @remarks
     * Defines a global environment map that can be used for reflections and lighting
     * in the scene. Can be either a cube map or an equirectangular map.
     */
    environment?: EnvironmentMapDefinition | EquirectangularEnvironmentMapDefinition;
    ...
    /**
     * Post effects configuration for the map rendering manager.
     */
 ...
 */
export type Definitions = Record<string, Definition | VerboseDefinition>;
 ...
 * {@link @orbitforge/orbitforge-mapview#DataSource} should be rendered.
 *
 * @remarks
 ...
 * {@link @orbitforge/orbitforge-mapview#MapView} via {@link @orbitforge/orbitforge-mapview#MapViewtheme}.
 */
export type StyleSet = Style[];

/**
 * The object that defines what way an item of a {@link @orbitforge/orbitforge-mapview#DataSource}
 ...
 */
export type Styles = Record<string, StyleSet>;
...
export type Tile3DBatchMeshStyle = BaseStyle<"tile3d", Tile3DBatchMeshTechniqueParams>;
...
export interface NoneStyle extends BaseStyle<"none", Record<string, any>> {
    [name: string]: any;
}
...
export interface Fog {
    ...
    /** Distance ratio to far plane at which the linear fog ends. */
    range: number;
}
...
export type ImageDefinitions = Record<string, ImageDefinition>;
...
/**
 * Define an environment map (cubemap) for reflections and lighting.
 */
export interface EnvironmentMapDefinition {
    /**
     * The URLs of the six cube map faces.
     *
     * @remarks
     * Order: [ +X, -X, +Y, -Y, +Z, -Z ]
     */
...
/**
 * Define an equirectangular environment map for reflections and lighting.
 */
export interface EquirectangularEnvironmentMapDefinition {
    /**
     * The URL of the equirectangular environment map.
     */
    url: string;
