...
import { type DecoderOptions } from "./ITileDecoder";
...
import { type Definitions, type Style, type StyleSet } from "./Theme";
...
class OptimizedSubSetKey {
    ...
    set(layer: string | undefined, geometryType: string | undefined, env?: Env): this {
...
export class StyleSetEvaluator {
    ...
    /**
     * Find style by its identifier.
     *
     * @param id - The style identifier.
     * @returns The found style or `undefined` if not found.
     */
    getStyleById(id: string): InternalStyle | undefined {
        return this.styleSet.find(style => style.id === id);
    }
    ...
    updateStyleById(id: string, updates: Partial<Style>): boolean {
        const styleIndex = this.styleSet.findIndex(style => style.id === id);
        if (styleIndex === -1) {
            return false;
        }
        ...
        const updatedStyle = {
            ...originalStyle,
            ...updates,
            // Ensure internal optimization properties are preserved
            id: originalStyle.id,
            _styleSetIndex: originalStyle._styleSetIndex,
            _geometryType: originalStyle._geometryType,
            // Clear compiled expressions to force recompilation
            _whenExpr: undefined,
            _minZoomLevelExpr: undefined,
            _maxZoomLevelExpr: undefined,
            _staticAttributes: undefined,
            _dynamicTechniqueAttributes: undefined,
            _dynamicFeatureAttributes: undefined,
            _dynamicForwardedAttributes: undefined,
            _dynamicTechniques: undefined,
            _staticTechnique: undefined,
            _usesFeatureState: undefined
        } as InternalStyle;
     ...
     */
    addStyle(style: Style): InternalStyle {
        // Ensure style has unique identifier
        const styleWithId = {
            ...style,
            id: style.id || this.generateStyleId()
        } as InternalStyle;
    ...
    /**
     * Recompute style set indices and render order.
     */
    private recomputeStyleSetIndices() {
        computeDefaultRenderOrder(this.styleSet);

        this.styleSet.forEach((style, index) => {
            style._styleSetIndex = index;
        });
    }
     ...
     * Invalidate caches that depend on style set structure.
     */
    private invalidateStyleSetCaches() {
        this.m_subStyleSetCache.clear();
        this.m_previousResult = undefined;
        this.m_previousEnv = undefined;
        this.m_tmpOptimizedSubSetKey.cachedStyleSet = undefined;
    }

    /**
     * Invalidate caches that depend on specific style properties.
     */
    private invalidateStyleCaches() {
        this.m_subStyleSetCache.clear();
        this.m_previousResult = undefined;
        this.m_previousEnv = undefined;
        this.m_tmpOptimizedSubSetKey.cachedStyleSet = undefined;
        // Keep m_cachedResults as it contains expression evaluation results that may still be valid
        // Keep m_definitionExprCache as it contains definition expressions that don't change
    }
}
