...
export interface DecodedTile {
    ...
    /**
     * List of {@link @orbitforge/orbitforge-geoutils#TileKey}s stored as mortonCodes representing
     * {@link @orbitforge/orbitforge-mapview#Tile}s that have geometry covering this `Tile`.
     */
    dependencies?: number[];
}
 ...
 * deprecated).
 */
export type AttributeMap = Record<string, unknown> | string | number;
...
export function getProjectionName(projection: Projection): string | never {
    ...
    } else if (projection === ellipsoidProjection) {
        return "ellipsoid";
    }
