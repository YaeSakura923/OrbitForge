...
const nodeDependencyWhitelist: Record<string, boolean> = {
    http: true,
    https: true,
    util: true,
    url: true,
...
function checkImports() {
    ...
    const packageDependencies: Record<string, any> = {};
    const packageConfigs: Record<string, any> = {};
    const packageJsonFiles = [...glob.sync(__dirname + "/../@orbitforge/*/package.json")];

    for (const packageJsonFile of packageJsonFiles) {
        const moduleName = "@orbitforge/" + packageJsonFile.split(path.sep).slice(-2, -1)[0];
    ...
    const sourceFiles = glob
        .sync(__dirname + "/../@orbitforge/**/*.ts")
        .filter(
            sourcePath =>
                !sourcePath.includes("node_modules") && !sourcePath.includes("generator-OrbitForge")
        );

    // regular expression catching all imported modules
    const importRE = /(.*)?import\s*{[\s\S]*?}\s*from\s+["'](.*)["']/gi;
    const environmentRE = /@orbitforge:check-imports:environment:(.*)/gi;
    ...
    for (const sourceFile of sourceFiles) {
        ...
        let env = "browser";
        const relativePath = path.relative(__dirname + "/../@orbitforge", sourceFile);
        const moduleName = "@orbitforge/" + relativePath.split(path.sep)[0];
        ...
        if (environmentMatch) {
            ...
            if (env !== "browser" && env !== "node") {
                errors.push(
                    `Error: ${relativePath} unknown '@orbitforge:environment:' type: ${env}. Supported types: 'node', 'browser'`
                );
                env = "browser";
            }
        }
        ...
        while ((matches = importRE.exec(contents)) != null) {
            ...
            // 3) Make sure we don't include other @orbitforge modules via relative paths
    ...
    function detectCircularDependencies() {
        ...
        for (const moduleName in packageConfigs) {
            const dependencies = Object.keys(packageDependencies[moduleName]).filter(
                dependencyName =>
                    dependencyName.startsWith("@orbitforge/") && dependencyName !== moduleName
            );
